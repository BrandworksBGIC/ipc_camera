...
static int ty_chip_74hc595_write(struct ty_motor_dev *ty_motor_dev, void *data)
{
	...
	for (i = 0; i < 8; i++) {
		...
		gpio_set_value(ty_motor_dev->used_pins[MOTOR_SH_PIN].gpio, 	1); //输入数据时钟
		gpio_set_value(ty_motor_dev->used_pins[MOTOR_SH_PIN].gpio, 	0);
	}
	gpio_set_value(ty_motor_dev->used_pins[MOTOR_ST_PIN].gpio, 	1); //输出数据时钟
	gpio_set_value(ty_motor_dev->used_pins[MOTOR_ST_PIN].gpio, 	0);
...
//        MS_DEBUG("gpio %d, val %d", ty_motor_dev->used_pins[i].gpio, val);
	}
...
long  set_ptz_cmd(struct ty_motor_dev *phdl, DWORD arg)
{
	...
	if (phdl->m_status.pan_status | phdl->m_status.tilt_status) {
		if (g_motor_init.soc == 3903) {
			//ktime_t ktime_set(const long secs, const unsigned long nsecs)；
			phdl->kt = ktime_set(0, (phdl->base_period) * 1000);    //base_period hrtimer实际时间不太稳定
			rts_hrtimer_enable(&phdl->ms_timer, phdl->kt);
		} else {
			/* 硬件timer 非常稳定
			* 500us=0.5ms 电机物理极限，无法转动
			* 800us=0.8ms, 转一圈3.5s
			* 1000us=1ms, 转一圈4.3s
			* 2000us=2ms，转一圈8.6s */
			phdl->hard_timer.timer_period = US_TO_PERIOD_VALUE(phdl->base_period); //us
			rts_xb2_timer_enable(&phdl->hard_timer, &phdl->xb2_timer_reg);
		}
	}
...
static void ty_do_motor(struct ty_motor_dev *ty_motor_dev)
{
    BYTE			dir_val		= 0;
...
static enum hrtimer_restart hr_ms_timer_func(struct hrtimer *timer)
{
    struct ty_motor_dev *ty_motor_dev = container_of(timer, struct ty_motor_dev, ms_timer);

    ty_do_motor(ty_motor_dev);
    ...
    rts_hrtimer_forward(timer, ty_motor_dev->kt);

    return HRTIMER_RESTART;
}

static void xb2_ms_timer_func(struct ty_motor_dev *ty_motor_dev)
{
    ty_do_motor(ty_motor_dev);

    if (ty_motor_dev->m_status.tilt_status == 0 && ty_motor_dev->m_status.pan_status == 0) {
		ty_motor_shutdown(ty_motor_dev);
		rts_xb2_timer_disable(&ty_motor_dev->hard_timer, &ty_motor_dev->xb2_timer_reg);
    }

    return;
}

static irqreturn_t timer_irq_handler(int irq, void *arg)
{
    struct ty_motor_dev *dev = (struct ty_motor_dev *)arg;

    if (!rts_xb2_timer_int_sts(&dev->hard_timer, &dev->xb2_timer_reg))
        return IRQ_NONE;
    else
		rts_xb2_timer_clr_int_sts(&dev->hard_timer, &dev->xb2_timer_reg);

	xb2_ms_timer_func(dev);

	return IRQ_HANDLED;
}

static int xb2_reg_irq_init(struct ty_motor_dev *motor_dev)
{
    int ret = 0;

	if (g_motor_init.soc == 3917) {
		unsigned int phy_addr = 0;
		unsigned int addr_len = 0;
		int irq;
		struct device_node *of_timer_node = NULL;
		//set rts xb2 timer register
		motor_dev->xb2_timer_reg.rts_timer0_en = 0x00;
		motor_dev->xb2_timer_reg.rts_timer0_compare = 0x04;
		motor_dev->xb2_timer_reg.rts_timer0_mode = 0x0c;
		motor_dev->xb2_timer_reg.rts_timer0_int_en = 0x10;
		motor_dev->xb2_timer_reg.rts_timer0_int_sts = 0x14;
		motor_dev->xb2_timer_reg.rts_irq_timer0 = 0x10;

		of_timer_node = of_find_node_by_name(NULL, "rts_timer");
		if (IS_ERR(of_timer_node)) {
			MS_ERROR("can't find device tree node timer\n");
			return ret;
		}

		ret = of_property_read_u32_index(of_timer_node, "reg", 0, &phy_addr);
		if (ret < 0) {
			MS_ERROR("can't find property reg\n");
			return ret;
		} else
			MS_DEBUG("=====vaddr %d 0x%x=====", phy_addr, phy_addr);

		ret = of_property_read_u32_index(of_timer_node, "reg", 1, &addr_len);
		if (ret < 0) {
			MS_ERROR("can't find property reg\n");
			return ret;
		} else
			MS_DEBUG("=====reg len  %d 0x%x=====\n", addr_len, addr_len);

		irq = irq_of_parse_and_map(of_timer_node, 0);
		if (irq < 0) {
			MS_ERROR("can't find property interrupts\n");
			return irq;
		} else {
			motor_dev->hard_timer.irq = irq;
			MS_DEBUG("=====interrupts %d 0x%x=====", motor_dev->hard_timer.irq, motor_dev->hard_timer.irq);
		}

		motor_dev->hard_timer.io_base = ioremap((phys_addr_t)phy_addr, addr_len);

		ret = devm_request_irq(motor_dev->misc_dev.this_device, motor_dev->hard_timer.irq, timer_irq_handler,
				IRQF_SHARED, "step_ptz_motor", motor_dev);
		if (ret != 0) {
			MS_ERROR("fail to request irq %d\n", motor_dev->hard_timer.irq);
			return ret;
		}
	} else if (g_motor_init.soc == 3915) {
		//set rts xb2 timer register
		motor_dev->xb2_timer_reg.rts_timer0_en = 0x00;
		motor_dev->xb2_timer_reg.rts_timer0_compare = 0x04;
		motor_dev->xb2_timer_reg.rts_timer0_mode = 0x0c;
		motor_dev->xb2_timer_reg.rts_timer0_int_en = 0x40;
		motor_dev->xb2_timer_reg.rts_timer0_int_sts = 0x44;
		motor_dev->xb2_timer_reg.rts_irq_timer0 = 0x10;

		motor_dev->hard_timer.io_base = ioremap((phys_addr_t)XB2_PHY_BASE_ADDR, XB2_PHY_ADDR_LEN);
...
static int ptz_of_parse(void)
{
	...
	np = of_find_node_by_name(NULL, "realtek-ptz");
	if (ret < 0) {
		MS_ERROR("can't find ptz\n");
		return ret;
	}

	ret = of_property_read_u32(np, "soc", &g_motor_init.soc);
	if (ret < 0) {
		MS_ERROR("can't find soc\n");
		return ret;
	}

	ret = of_property_read_string(np, "ptz-chip", &g_motor_init.ptz_chip);
	if (ret < 0) {
		MS_ERROR("can't find ptz-chip\n");
		return ret;
	}
	ret = of_property_read_u32(np, "base-period", &g_motor_init.base_period);
	if (ret < 0) {
		MS_ERROR("can't find base-period\n");
		return ret;
	}
	ret = of_property_read_u32(np, "x-step-max", &g_motor_init.pan_step_max);
	if (ret < 0) {
		MS_ERROR("can't find x-step-max\n");
		return ret;
	}
	ret = of_property_read_u32(np, "y-step-max", &g_motor_init.tilt_step_max);
	if (ret < 0) {
		MS_ERROR("can't find y-step-max\n");
		return ret;
	}

	if (!strcmp(g_motor_init.ptz_chip, "darlington_8pin")) {
		g_motor_init.chip = CHIP_DARLINGTON;
		g_motor_init.chip_pins_num = 8;
		child_np = of_find_node_by_name(np, "darlington-8pin");
		if (ret < 0) {
			MS_ERROR("can't find darlington_8pin\n");
			return ret;
		}

		ret = of_property_read_u32_index(child_np, "gpio-y1", 1, &g_motor_init.used_pins[0].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-y1\n");
			return ret;
		}
		g_motor_init.used_pins[0].pin_name = "y_motor_1";
		g_motor_init.used_pins[0].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-y2", 1, &g_motor_init.used_pins[1].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-y2\n");
			return ret;
		}
		g_motor_init.used_pins[1].pin_name = "y_motor_2";
		g_motor_init.used_pins[1].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-y3", 1, &g_motor_init.used_pins[2].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-y3\n");
			return ret;
		}
		g_motor_init.used_pins[2].pin_name = "y_motor_3";
		g_motor_init.used_pins[2].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-y4", 1, &g_motor_init.used_pins[3].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-y4\n");
			return ret;
		}
		g_motor_init.used_pins[3].pin_name = "y_motor_4";
		g_motor_init.used_pins[3].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-x1", 1, &g_motor_init.used_pins[4].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-x1\n");
			return ret;
		}
		g_motor_init.used_pins[4].pin_name = "x_motor_1";
		g_motor_init.used_pins[4].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-x2", 1, &g_motor_init.used_pins[5].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-x2\n");
			return ret;
		}
		g_motor_init.used_pins[5].pin_name = "x_motor_2";
		g_motor_init.used_pins[5].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-x3", 1, &g_motor_init.used_pins[6].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-x3\n");
			return ret;
		}
		g_motor_init.used_pins[6].pin_name = "x_motor_3";
		g_motor_init.used_pins[6].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-x4", 1, &g_motor_init.used_pins[7].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio-x4\n");
			return ret;
		}
		g_motor_init.used_pins[7].pin_name = "x_motor_4";
		g_motor_init.used_pins[7].direction = OUTPUT_PIN;

	} else if (!strcmp(g_motor_init.ptz_chip, "74hc595")) {
		g_motor_init.chip = CHIP_74HC595;
		g_motor_init.chip_pins_num = 3;
		child_np = of_find_node_by_name(np, "74hc595");
		if (ret < 0) {
			MS_ERROR("can't find 74hc595\n");
			return ret;
		}

		ret = of_property_read_u32_index(child_np, "gpio-1", 1, &g_motor_init.used_pins[0].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio_1\n");
			return ret;
		}
		g_motor_init.used_pins[0].pin_name = "y_motor_1";
		g_motor_init.used_pins[0].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-2", 1, &g_motor_init.used_pins[1].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio_2\n");
			return ret;
		}
		g_motor_init.used_pins[1].pin_name = "y_motor_2";
		g_motor_init.used_pins[1].direction = OUTPUT_PIN;

		ret = of_property_read_u32_index(child_np, "gpio-3", 1, &g_motor_init.used_pins[2].gpio);
		if (ret < 0) {
			MS_ERROR("can't find gpio_3\n");
			return ret;
		}
		g_motor_init.used_pins[2].pin_name = "y_motor_3";
		g_motor_init.used_pins[2].direction = OUTPUT_PIN;
...
int ty_motor_init(void)
{
	...
	g_motor_init.infi_rot_en = 0;
	g_motor_init.motor_select = 0;
	g_motor_init.pan_dir = 0;
	g_motor_init.tilt_dir = 0;
	g_motor_init.pan_speed = 1;
	g_motor_init.tilt_speed = 1;
	g_motor_init.pan_roll_step = 4000;

	ret = ptz_of_parse();
	if (ret < 0) {
		MS_ERROR("parse ptz param failed\n");
		return ret;
	}
	...
	g_motor_dev->driver.of_match_table  = of_match_ptr(ptz_ids);
	...
	if (g_motor_init.soc == 3903) {
		rts_hrtimer_init(&g_motor_dev->ms_timer);
		g_motor_dev->ms_timer.function = hr_ms_timer_func;
	} else
		xb2_reg_irq_init(g_motor_dev);
...
void ty_motor_exit(void)
{
	...
	if (g_motor_init.soc == 3903) {
		rts_hrtimer_disable(&g_motor_dev->ms_timer);
	} else
		rts_xb2_timer_disable(&g_motor_dev->hard_timer, &g_motor_dev->xb2_timer_reg);
