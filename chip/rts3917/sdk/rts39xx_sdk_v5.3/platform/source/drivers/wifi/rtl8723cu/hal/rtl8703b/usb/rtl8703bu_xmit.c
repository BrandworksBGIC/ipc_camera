...
#define _RTL8703BU_XMIT_C_

#include <rtl8703b_hal.h>


s32	rtl8703bu_init_xmit_priv(_adapter *padapter)
{
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;

#ifdef PLATFORM_LINUX
	tasklet_init(&pxmitpriv->xmit_tasklet,
		     (void(*)(unsigned long))rtl8703bu_xmit_tasklet,
		     (unsigned long)padapter);
#endif
	return _SUCCESS;
}

void	rtl8703bu_free_xmit_priv(_adapter *padapter)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz, u8 bagg_pkt)
{
	...
	rtl8703b_update_txdesc(pxmitframe, (u8 *)ptxdesc);
...
static s32 rtw_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		/* rtl8703b_update_txdesc(pxmitframe, mem_addr+PACKET_OFFSET_SZ); */
...
s32 rtl8703bu_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe)
{
 ...
 */
s32 rtl8703bu_hal_xmit(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	return pre_xmitframe(padapter, pxmitframe);
}

#ifdef CONFIG_RTW_MGMT_QUEUE
s32 rtl8703bu_hal_mgmt_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
static void rtl8703bu_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8703bu_hostap_mgnt_xmit_entry(_adapter *padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
	...
	rtl8703b_cal_txdesc_chksum(ptxdesc);
	/* ----- end of fill tx desc ----- */
	...
	usb_fill_bulk_urb(urb, pdvobj->pusbdev, pipe,
		pxmit_skb->data, pxmit_skb->len, rtl8703bu_hostap_mgnt_xmit_cb, pxmit_skb);
