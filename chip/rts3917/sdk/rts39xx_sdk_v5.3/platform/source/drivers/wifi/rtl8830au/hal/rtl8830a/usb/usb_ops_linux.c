...
void interrupt_handler_8830au(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	do {
		...
		rtl8830a_query_rx_desc_status(precvframe, prxstat);
...
#ifdef CONFIG_SUPPORT_USB_INT
				interrupt_handler_8830au(padapter, pattrib->pkt_len, precvframe->u.hdr.rx_data);
#endif
			}
			rtw_free_recvframe(precvframe, pfree_recv_queue);
...
void rtl8830au_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8830au_xmitframe_complete(padapter, pxmitpriv, NULL);
...
void rtl8830au_set_hw_type(struct dvobj_priv *pdvobj)
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8830AU;
	RTW_INFO("CHIP TYPE: RTL8830A\n");
}
