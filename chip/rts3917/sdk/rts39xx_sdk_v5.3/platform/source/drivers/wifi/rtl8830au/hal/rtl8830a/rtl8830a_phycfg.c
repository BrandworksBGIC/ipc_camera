 ...
 *
 *---------------------------------------------------------------------------*/
s32 PHY_MACConfig8830A(PADAPTER Adapter)
{
...
)
{
	...
	if (Adapter->registrypriv.mp_mode == 1) {
		...
		if (rtStatus != _SUCCESS) {
			RTW_INFO("phy_BB8830A_Config_ParaFile():Write BB Reg MP Fail!!");
			goto phy_BB8190_Config_ParaFile_Fail;
		}
	}
...
)
{
	...
	/* Config BB and AGC */
	/*  */
	rtStatus = phy_BB8830A_Config_ParaFile(Adapter);
...
)
{
	...
	/* RF config */
	/*  */
	rtStatus = PHY_RF6052_Config8830A(Adapter);
	return rtStatus;
}
 ...
 *---------------------------------------------------------------------------*/
int
rtl8830a_PHY_ConfigRFWithParaFile(
		PADAPTER		Adapter,
		u8				*pFileName,
		enum rf_path		eRFPath
)
...
s8 phy_get_txpwr_target_extra_bias_8830a(_adapter *adapter, enum rf_path rfpath
...
)
{
	...
	/* DbgPrint("===> phy_SpurCalibration_8830A  current_channel_bw = %d, current_channel = %d\n", pHalData->current_channel_bw, pHalData->current_channel);*/
...
)
{
	...
	case RF_6052:
		rtl8830a_PHY_RF6052SetBandwidth(Adapter, pHalData->current_channel_bw);
		break;
 ...
 *	} */
	...
	if (!RTW_CANNOT_RUN(Adapter)) {
		...
		if (IS_VENDOR_8830A_I_CUT_SERIES(Adapter))
			phy_SpurCalibration_8830A(Adapter);
#endif
	} else {
		/* pHalData->SetBWModeInProgress= FALSE; */
...
static void _PHY_SwChnl8830A(PADAPTER Adapter, u8 channel)
{
	enum rf_path eRFPath;
	u32 param1, param2;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
void
PHY_SwChnl8830A(/* Call after initialization */
		PADAPTER	Adapter,
		u8		channel
)
{
...
#else
		_PHY_SwChnl8830A(Adapter, channel);
#endif
...
)
{
	...
	PHY_SwChnl8830A(Adapter, channel);
	PHY_SetBWMode8830A(Adapter, Bandwidth, Offset40);
