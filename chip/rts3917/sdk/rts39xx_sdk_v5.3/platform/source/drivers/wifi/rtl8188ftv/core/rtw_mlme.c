...
#define RTW_ROAM_DICONNECT_DELAY	20
	...
	for (i = 0; i < sizeof(pmlmepriv->roam_scan_order); i++)
		pmlmepriv->roam_scan_order[i] =
			(str[i]=='2')?BAND_ON_24G:(str[i]=='5')?BAND_ON_5G:BAND_MAX;
...
*/
static bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target,
...
void rtw_add_network(_adapter *adapter, WLAN_BSSID_EX *bss, struct wlan_network **pnetwork)
{
	bool update_ie;
	/* _queue	*queue	= &(pmlmepriv->scanned_queue); */
	...
	update_ie = rtw_update_scanned_network(adapter, bss, pnetwork);
...
#define SURVEY_INFO_VALUE_ARG_80211D
	...
	while (!rtw_end_of_queue_search(phead, plist)) {
		...
		RTW_PRINT_SEL(sel, "%5d %s %4s %3d %4d %4d %5d %4d"
			SURVEY_INFO_VALUE_FMT_80211D
			" %5d %-32s %s\n"
			, ++index
			, get_macaddr_str(mac_addr_str, sel, pnetwork->network.MacAddress)
...
void dump_unassoc_sta(void *sel, _adapter *adapter)
{
	...
	for (i = 0; i < unassoc_sta_cnt; i++) {
		...
		RTW_PRINT_SEL(sel, "%c %s %18u %6d\n"
			...
			, get_macaddr_str(mac_addr_str, sel, unassoc_sta->addr), measure_delta_time, unassoc_sta->recv_signal_power);
	}
...
void rtw_survey_event_callback(_adapter	*adapter, u8 *pbuf)
{
	...
	if ((check_fwstate(pmlmepriv, WIFI_UNDER_LINKING)) == _FALSE) {
		...
		rtw_add_network(adapter, bss, &pnetwork);
...
#ifdef CONFIG_LAYER2_ROAMING
		if (rtw_chk_roam_flags(adapter, RTW_ROAM_QUICK_SCAN) && pmlmepriv->need_to_roam &&
			pnetwork && (pnetwork != pmlmepriv->cur_network_scanned)) {
			if (rtw_check_roaming_candidate(pmlmepriv, &pmlmepriv->roam_scan_candidate, pnetwork)) {
...
void rtw_reset_bcn_cnt_info(_adapter *adapter)
{
	struct bcn_cnt_priv *bcn_cntpriv = &adapter->bcn_cntpriv;
	if (bcn_cntpriv) {
		bcn_cntpriv->bcn_tsf = 0;
        bcn_cntpriv->bcn_ts = 0;
	}
}
#endif
...
#ifdef CONFIG_BCN_CNT
				if (!rtw_mi_get_assoced_sta_num(adapter))
					adapter->is_first_ld_sta = _TRUE;
#endif
				rtw_indicate_connect(adapter);
			}
...
#ifdef CONFIG_LAYER2_ROAMING
		dequeuq_roam_pkt(adapter, _TRUE);
#endif
...
void rtw_sta_media_status_rpt(_adapter *adapter, struct sta_info *sta, bool connected)
{
	...
	struct _ADAPTER *adapter_dftport = NULL;
#endif
...
static void rtw_mlme_reset_auto_scan_int(_adapter *adapter, u8 *reason, u32 *scan_int_ms)
{
	...
	*reason = RTW_AUTO_SCAN_REASON_UNSPECIFIED;
	...
	if (hal_chk_bw_cap(adapter, BW_CAP_40M)
		...
		if (ALINK_GET_BAND(GET_PRIMARY_LINK(adapter)) == BAND_ON_24G
...
#ifdef CONFIG_80211D
	if (rtw_cis_scan_needed(adapter_to_rfctl(adapter), &urgent)) {
		interval_ms = rtw_min(interval_ms, urgent ? RTW_ASCAN_INT_URGENT : RTW_ASCAN_INT_ASAP);
	...
	if (interval_ms == U32_MAX)
		interval_ms = RTW_ASCAN_INT_NONE;

	*scan_int_ms = interval_ms;
}
...
static void rtw_auto_scan_ch_list_set_flags(struct sitesurvey_parm *parm, u8 band_bmp, u8 flags)
{
	...
	for (i = 0; i < parm->ch_num; i++) {
		if (BIT(rtw_get_band_type(parm->ch[i].hw_value)) & band_bmp)
			parm->ch[i].flags = flags;
	}

}

u8 rtw_drv_scan_by_self(_adapter *padapter, u8 reason, bool urgent)
{
	struct sitesurvey_parm *parm = NULL;
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	int i;
	u8 ssc_chk, rtn = _FALSE;
...
static void rtw_auto_scan_handler(_adapter *padapter)
{
	...
	rtw_mlme_reset_auto_scan_int(padapter, &reason, &scan_int_ms);
	...
	if (is_client_associated_to_ap(padapter) &&
		...
		if (precvpriv->beacon_strength < pmlmepriv->roam_rssi_threshold) {
			pmlmepriv->need_to_roam = _TRUE;
			if (rtw_get_passing_time_ms(pmlmepriv->last_roaming) >= pmlmepriv->roam_scan_int*2000) {
				reason = RTW_AUTO_SCAN_REASON_ROAM_ACTIVE;
				if (rtw_drv_scan_by_self(padapter, reason, false))
					pmlmepriv->last_roaming = rtw_get_current_time();
				return;
			}
		} else
	...
	if (scan_int_ms == RTW_ASCAN_INT_NONE
		|| (scan_int_ms > RTW_ASCAN_INT_ASAP
			&& rtw_get_passing_time_ms(pmlmepriv->scan_start_time) < scan_int_ms)
	)
		goto exit;

	rtw_drv_scan_by_self(padapter, reason, scan_int_ms == RTW_ASCAN_INT_URGENT);
	...
	, struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if (competitor == mlme->cur_network_scanned) {
		result ="self";
		goto exit;
	}
	goto update; /* skip rssi check to force roam */
#else
...
exit:
	if (result)
		RTW_INFO("candidate:%s %s("MAC_FMT", ch%3u) rssi:%d dBm, age:%5d [%5s]\n",
...
void dump_arp_pkt(void *sel, u8 *da, u8 *sa, u8 *arp, bool tx)
{
	...
	char mac_addr_str_arp_da[MAC_FMT_LEN];

	RTW_PRINT_SEL(sel, "%s ARP da=%s, sa=%s\n"
		, tx ? "send" : "recv", get_macaddr_str(mac_addr_str_da, sel, da), get_macaddr_str(mac_addr_str_sa, sel, sa));
	...
	RTW_PRINT_SEL(sel, "sha=%s, spa="IP_FMT"\n"
		, get_macaddr_str(mac_addr_str_arp_sa, sel, ARP_SENDER_MAC_ADDR(arp)), IP_ARG(ARP_SENDER_IP_ADDR(arp)));
	RTW_PRINT_SEL(sel, "tha=%s, tpa="IP_FMT"\n"
		, get_macaddr_str(mac_addr_str_arp_da, sel, ARP_TARGET_MAC_ADDR(arp)), IP_ARG(ARP_TARGET_IP_ADDR(arp)));
}
