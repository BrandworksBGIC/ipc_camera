...
static BOOLEAN HalUsbSetQueuePipeMapping8703BUsb(
		PADAPTER	pAdapter,
		u8		NumInPipe,
		u8		NumOutPipe
)
{
	...
	_ConfigChipOutEP_8703(pAdapter, NumOutPipe);
...
void rtl8703bu_interface_configure(_adapter *padapter)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(padapter);
	...
	HalUsbSetQueuePipeMapping8703BUsb(padapter,
...
static u32 _InitPowerOn_8703BU(PADAPTER padapter)
{
	u32 status = _SUCCESS;
	u16 value16 = 0;
	u8 value8 = 0;
	...
	if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8703B_card_enable_flow))
		return _FAIL;

	/* Enable MAC DMA/WMAC/SCHEDULE/SEC block */
	/* Set CR bit10 to enable 32k calibration. Suggested by SD1 Gimmy. Added by tynli. 2011.08.31. */
	rtw_write8(padapter, REG_CR, 0x00);  /* suggseted by zhouzhou, by page, 20111230 */
...
static void _InitPageBoundary(PADAPTER padapter)
{
	...
	rtw_write16(padapter, REG_TRXFF_BNDY + 2, RX_DMA_BOUNDARY_8703B);
}
...
)
{
	...
	rcr = (
		...
		| RCR_CBSSID_DATA | RCR_CBSSID_BCN | RCR_APP_ICV
		| RCR_AMF | RCR_HTC_LOC_CTRL | RCR_APP_MIC | RCR_APP_PHYST_RXFF
	);
	...
	value16 = 0xFFFF;
	rtw_write16(Adapter, REG_RXFLTMAP2_8703B, value16);
	...
	value16 = 0x400;
	rtw_write16(Adapter, REG_RXFLTMAP1_8703B, value16);

	/* Accept all management frames */
	value16 = 0xFFFF;
	rtw_write16(Adapter, REG_RXFLTMAP0_8703B, value16);
...
static void
_InitRDGSetting_8703bu(
		PADAPTER Adapter
)
{
	rtw_write8(Adapter, REG_RD_CTRL_8703B, 0xFF);
	rtw_write16(Adapter, REG_RD_NAV_NXT_8703B, 0x200);
	rtw_write8(Adapter, REG_RD_RESP_PKT_TH_8703B, 0x05);
}

/*
static void
_InitRxSetting_8703bu(
		PADAPTER Adapter
)
{
...
static void _InitBurstPktLen(PADAPTER padapter)
{
	...
	tmp8 = rtw_read8(padapter, REG_RXDMA_MODE_CTRL_8703B);
	tmp8 &= ~(BIT(4) | BIT(5));
	...
	rtw_write8(padapter, REG_RXDMA_MODE_CTRL_8703B, tmp8);

	pHalData->bSupportUSB3 = _FALSE;

	tmp8 = rtw_read8(padapter, REG_HT_SINGLE_AMPDU_8703B);
	tmp8 |= BIT(7); /* enable single pkt ampdu */
	rtw_write8(padapter, REG_HT_SINGLE_AMPDU_8703B, tmp8);
	rtw_write16(padapter, REG_MAX_AGGR_NUM, 0x0C14);
	rtw_write8(padapter, REG_AMPDU_MAX_TIME_8703B, 0x5E);
	rtw_write32(padapter, REG_AMPDU_MAX_LENGTH_8703B, 0xffffffff);
	if (pHalData->AMPDUBurstMode)
		rtw_write8(padapter, REG_AMPDU_BURST_MODE_8703B, 0x5F);
	...
	rtw_write8(padapter, REG_USTIME_TSF_8703B, 0x50);
	rtw_write8(padapter, REG_USTIME_EDCA_8703B, 0x50);
...
)
{
	...
	if (pHalData->UsbTxAggMode) {
		value32 = rtw_read32(Adapter, REG_DWBCN0_CTRL_8703B);
		value32 = value32 & ~(BLK_DESC_NUM_MASK << BLK_DESC_NUM_SHIFT);
		value32 |= ((pHalData->UsbTxAggDescNum & BLK_DESC_NUM_MASK) << BLK_DESC_NUM_SHIFT);

		rtw_write32(Adapter, REG_DWBCN0_CTRL_8703B, value32);
		rtw_write8(Adapter, REG_DWBCN1_CTRL_8703B, pHalData->UsbTxAggDescNum << 1);
	}
...
static void usb_AggSettingRxUpdate(PADAPTER padapter)
{
	...
	case RX_AGG_DMA:
		...
		if (agg_size > RX_DMA_BOUNDARY_8703B)
			agg_size = RX_DMA_BOUNDARY_8703B >> 1;
...
void _InitBBRegBackup_8703BU(PADAPTER	Adapter)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(Adapter);
...
u32 rtl8703bu_hal_init(PADAPTER padapter)
{
	...
	static const char *const hal_init_stages_str[] = {
...
#define HAL_INIT_PROFILE_TAG(stage) do {} while (0)
	...
	value8 = rtw_read8(padapter, REG_SYS_CLKR_8703B + 1);
	...
	if (!pregistrypriv->wifi_spec)
		boundary = TX_PAGE_BOUNDARY_8703B;
	else {
		/* for WMM */
		boundary = WMM_NORMAL_TX_PAGE_BOUNDARY_8703B;
	}

	status =  rtl8703b_InitLLTTable(padapter);
	if (status == _FAIL) {
		goto exit;
	}
	...
	if (pHalData->bRDGEnable)
		_InitRDGSetting_8703bu(padapter);
	...
	if (pHalData->AMPDUBurstMode) {
		...
		value8 = rtw_read8(padapter, REG_EARLY_MODE_CONTROL_8703B);
#if RTL8703B_EARLY_MODE_PKT_NUM_10 == 1
		value8 = value8 | 0x1f;
#else
		value8 = value8 | 0xf;
#endif
		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8703B, value8);

		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8703B + 3, 0x80);

		value8 = rtw_read8(padapter, REG_TCR_8703B + 1);
		value8 = value8 | 0x40;
		rtw_write8(padapter, REG_TCR_8703B + 1, value8);
	} else
#endif
		rtw_write8(padapter, REG_EARLY_MODE_CONTROL_8703B, 0);
	...
	if (padapter->registrypriv.mp_mode == 0) {
		status = rtl8703b_FirmwareDownload(padapter, _FALSE);
		if (status != _SUCCESS) {
	...
	PHY_InitAntennaSelection8703B(padapter);



	HAL_INIT_PROFILE_TAG(HAL_INIT_STAGES_MAC);
#if (HAL_MAC_ENABLE == 1)
	status = PHY_MACConfig8703B(padapter);
	if (status == _FAIL) {
		RTW_INFO("PHY_MACConfig8703B fault !!\n");
		goto exit;
	}
#endif
...
#if (HAL_BB_ENABLE == 1)
	status = PHY_BBConfig8703B(padapter);
	if (status == _FAIL) {
		RTW_INFO("PHY_BBConfig8703B fault !!\n");
		goto exit;
	}
#endif
...
#if (HAL_RF_ENABLE == 1)
	status = PHY_RFConfig8703B(padapter);

	if (status == _FAIL) {
		RTW_INFO("PHY_RFConfig8703B fault !!\n");
		goto exit;
	}
#endif

	_InitBBRegBackup_8703BU(padapter);
	...
	_InitTransferPageSize_8703bu(padapter);
...
static void rtl8703bu_hw_power_down(_adapter *padapter)
{
	u8	u1bTmp;

	RTW_INFO("PowerDownRTL8703U\n");
...
/*
 * Description: RTL8703e card disable power sequence v003 which suggested by Scott.
 * First created by tynli. 2011.01.28.
 */
void
CardDisableRTL8703BU(
	PADAPTER			Adapter
)
{
	u8		u1bTmp;
	...
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8703B_enter_lps_flow);

	if ((rtw_read8(Adapter, REG_MCUFWDL_8703B) & BIT7) &&
	    GET_HAL_DATA(Adapter)->bFWReady) /*8051 RAM code */
		rtl8703b_FirmwareSelfReset(Adapter);

	/* Reset MCU. Suggested by Filen. 2011.01.26. by tynli. */
	u1bTmp = rtw_read8(Adapter, REG_SYS_FUNC_EN_8703B + 1);
	rtw_write8(Adapter, REG_SYS_FUNC_EN_8703B + 1, (u1bTmp & (~BIT2)));

	/* MCUFWDL 0x80[1:0]=0 */	/* reset MCU ready status */
	rtw_write8(Adapter, REG_MCUFWDL_8703B, 0x00);

	/* Card disable power action flow */
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, rtl8703B_card_disable_flow);

	GET_HAL_DATA(Adapter)->bFWReady = _FALSE;
}


u32 rtl8703bu_hal_deinit(PADAPTER padapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(padapter);
...
#ifdef CONFIG_FWLPS_IN_IPS
	if (rtw_is_fw_ips_mode(padapter) == _TRUE) {
		if (rtw_fw_ips_deinit(padapter) == _SUCCESS)
			goto exit;
	}
#endif
...
unsigned int rtl8703bu_inirp_init(PADAPTER Adapter)
{
...
unsigned int rtl8703bu_inirp_deinit(PADAPTER Adapter)
{
#ifdef CONFIG_USB_INTERRUPT_IN_PIPE
	u32(*_read_interrupt)(struct intf_hdl *pintfhdl, u32 addr);
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
...
static void _ReadPSSetting(PADAPTER Adapter, u8 *PROMContent, u8	AutoloadFail)
{
	...
	if (AutoloadFail) {
		...
		if (Adapter->registrypriv.hwpdn_mode == 2)
			pwrctl->bHWPowerdown = (PROMContent[EEPROM_FEATURE_OPTION_8703B] & BIT4);
		else
...
)
{
	...
	if (AutoLoadFail) {
		...
		/* VID, PID */
		pHalData->EEPROMVID = le16_to_cpu(*(u16 *)&hwinfo[EEPROM_VID_8703BU]);
		pHalData->EEPROMPID = le16_to_cpu(*(u16 *)&hwinfo[EEPROM_PID_8703BU]);
...
static u8
InitAdapterVariablesByPROM_8703BU(
		PADAPTER	padapter
)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	...
	Hal_EfuseParsePIDVID_8703BU(padapter, hwinfo, pHalData->bautoload_fail_flag);
	...
	if (Hal_ReadRFEType_8703B(padapter, pHalData->efuse_eeprom_data,
	...
	/*hal_CustomizedBehavior_8703U(Adapter); */
...
)
{
	...
	if (InitAdapterVariablesByPROM_8703BU(Adapter) != _SUCCESS)
		goto exit;
...
static u8 ReadAdapterInfo8703BU(PADAPTER Adapter)
{
	u8 ret = _FAIL;

	/* Read EEPROM size before call any EEPROM function */
	Adapter->EepromAddressSize = GetEEPROMSize8703B(Adapter);
...
static void rtl8703bu_trigger_gpio_0(_adapter *padapter)
{
 ...
 * some variables will be processed in SetHwReg8703A()
 */
u8 SetHwReg8703BU(PADAPTER Adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
	...
	case HW_VAR_TRIGGER_GPIO_0:
		rtl8703bu_trigger_gpio_0(Adapter);
		break;

	default:
		ret = SetHwReg8703B(Adapter, variable, val);
		break;
	}
...
/*
 * If variable not handled here,
 * some variables will be processed in GetHwReg8703A()
 */
void GetHwReg8703BU(PADAPTER Adapter, u8 variable, u8 *val)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
...
#endif /* CONFIG_LPS_LCLK */
	default:
		GetHwReg8703B(Adapter, variable, val);
		break;
	}
...
static u8 rtl8703bu_ps_func(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id, u8 *val)
{
	u8 bResult = _TRUE;
...
void rtl8703bu_set_hal_ops(_adapter *padapter)
{
	...
	pHalFunc->hal_power_on = &_InitPowerOn_8703BU;
	pHalFunc->hal_power_off = &CardDisableRTL8703BU;

	pHalFunc->hal_init = &rtl8703bu_hal_init;
	pHalFunc->hal_deinit = &rtl8703bu_hal_deinit;

	pHalFunc->inirp_init = &rtl8703bu_inirp_init;
	pHalFunc->inirp_deinit = &rtl8703bu_inirp_deinit;

	pHalFunc->init_xmit_priv = &rtl8703bu_init_xmit_priv;
	pHalFunc->free_xmit_priv = &rtl8703bu_free_xmit_priv;

	pHalFunc->init_recv_priv = &rtl8703bu_init_recv_priv;
	pHalFunc->free_recv_priv = &rtl8703bu_free_recv_priv;
#ifdef CONFIG_RTW_SW_LED
	pHalFunc->InitSwLeds = &rtl8703bu_InitSwLeds;
	pHalFunc->DeInitSwLeds = &rtl8703bu_DeInitSwLeds;
#endif/*CONFIG_RTW_SW_LED */

	pHalFunc->init_default_value = &rtl8703b_init_default_value;
	pHalFunc->intf_chip_configure = &rtl8703bu_interface_configure;
	pHalFunc->read_adapter_info = &ReadAdapterInfo8703BU;

	pHalFunc->set_hw_reg_handler = &SetHwReg8703BU;
	pHalFunc->GetHwRegHandler = &GetHwReg8703BU;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8703BUsb;
	pHalFunc->SetHalDefVarHandler = &SetHalDefVar8703BUsb;

	pHalFunc->hal_xmit = &rtl8703bu_hal_xmit;
	pHalFunc->mgnt_xmit = &rtl8703bu_mgnt_xmit;
#ifdef CONFIG_RTW_MGMT_QUEUE
	pHalFunc->hal_mgmt_xmitframe_enqueue = &rtl8703bu_hal_mgmt_xmitframe_enqueue;
#endif
	pHalFunc->hal_xmitframe_enqueue = &rtl8703bu_hal_xmitframe_enqueue;

#ifdef CONFIG_HOSTAPD_MLME
	pHalFunc->hostap_mgnt_xmit_entry = &rtl8703bu_hostap_mgnt_xmit_entry;
#endif
	pHalFunc->interface_ps_func = &rtl8703bu_ps_func;

#ifdef CONFIG_XMIT_THREAD_MODE
	pHalFunc->xmit_thread_handler = &rtl8703bu_xmit_buf_handler;
#endif
#ifdef CONFIG_SUPPORT_USB_INT
	pHalFunc->interrupt_handler = interrupt_handler_8703bu;
#endif
