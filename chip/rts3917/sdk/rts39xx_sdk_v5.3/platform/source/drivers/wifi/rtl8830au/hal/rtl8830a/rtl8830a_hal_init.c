...
#include "hal8830a_s_fw.h"
#endif
#include "hal8830a_t_fw.h"
...
s32 rtl8830a_iol_efuse_patch(PADAPTER padapter)
{
	s32	result = _SUCCESS;
	printk("==> %s\n", __FUNCTION__);
...
int rtl8830a_IOL_exec_cmds_sync(ADAPTER *adapter, struct xmit_frame *xmit_frame, u32 max_wating_ms, u32 bndy_cnt)
{
...
)
{
...
#ifdef CONFIG_USB_HCI
		ret = rtw_writeN(padapter, (FW_8830A_START_ADDRESS + i * blockSize_p1), blockSize_p1, (bufferPtr + i * blockSize_p1));
#else
		ret = rtw_write32(padapter, (FW_8830A_START_ADDRESS + i * blockSize_p1), le32_to_cpu(*((u32 *)(bufferPtr + i * blockSize_p1))));
#endif
	...
	if (remainSize_p1) {
		...
		case 1:
			remainFW[0] = *(p);
			ret = rtw_write32(padapter, (FW_8830A_START_ADDRESS + blockCount_p1 * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter, (FW_8830A_START_ADDRESS + offset + i * blockSize_p2), blockSize_p2, (bufferPtr + offset + i * blockSize_p2));
	...
	if (remainSize_p2) {
		...
		for (i = 0 ; i < blockCount_p3 ; i++) {
			ret = rtw_write8(padapter, (FW_8830A_START_ADDRESS + offset + i), *(bufferPtr + offset + i));
...
#define IS_FW_81xxC(padapter)	(((GET_HAL_DATA(padapter))->FirmwareSignature & 0xFFF0) == 0x88C0)
	...
	u8	FwBuffer8830A[FW_8830A_SIZE];
 ...
 *
 *   */
s32 rtl8830a_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	PRT_FIRMWARE_8830A	pFirmware = NULL;
	PRT_8830A_FIRMWARE_HDR		pFwHdr = NULL;
	...
	pFirmware = (PRT_FIRMWARE_8830A)rtw_zmalloc(sizeof(RT_FIRMWARE_8830A));
	if (!pFirmware) {
		rtStatus = _FAIL;
		goto exit;
	}
...
#ifdef CONFIG_FILE_FWIMG
		rtStatus = rtw_retrieve_from_file(fwfilepath, FwBuffer8830A, FW_8830A_SIZE);
		pFirmware->ulFwLength = rtStatus >= 0 ? rtStatus : 0;
		pFirmware->szFwBuffer = FwBuffer8830A;
#endif /* CONFIG_FILE_FWIMG */
		break;
	case FW_SOURCE_HEADER_FILE:
		if (bUsedWoWLANFw) {
	...
	#ifdef CONFIG_SFW_SUPPORTED
				if (IS_VENDOR_8830A_I_CUT_SERIES(padapter)) {
					pFirmware->szFwBuffer = array_mp_8830a_s_fw_wowlan;
					pFirmware->ulFwLength = array_length_mp_8830a_s_fw_wowlan;
				} else
	#endif
				{
					pFirmware->szFwBuffer = array_mp_8830a_t_fw_wowlan;
					pFirmware->ulFwLength = array_length_mp_8830a_t_fw_wowlan;
				}
			...
			if (pwrpriv->wowlan_ap_mode) {
					pFirmware->szFwBuffer = array_mp_8830a_t_fw_ap;
					pFirmware->ulFwLength = array_length_mp_8830a_t_fw_ap;
...
#ifdef CONFIG_SFW_SUPPORTED
			if (IS_VENDOR_8830A_I_CUT_SERIES(padapter)) {
				pFirmware->szFwBuffer = array_mp_8830a_s_fw_nic;
				pFirmware->ulFwLength = array_length_mp_8830a_s_fw_nic;
			} else
#endif
			{
				pFirmware->szFwBuffer = array_mp_8830a_t_fw_nic;
				pFirmware->ulFwLength = array_length_mp_8830a_t_fw_nic;
			}
	...
	tmp_fw_len = IS_VENDOR_8830A_I_CUT_SERIES(padapter) ? FW_8830A_SIZE_2 : FW_8830A_SIZE;
	...
	pFwHdr = (PRT_8830A_FIRMWARE_HDR)pFirmwareBuf;
	...
	_FWDownloadEnable_8830A(padapter, _TRUE);
	...
	_FWDownloadEnable_8830A(padapter, _FALSE);
...
exit:
	if (pFirmware)
		rtw_mfree((u8 *)pFirmware, sizeof(RT_FIRMWARE_8830A));

	rtl8830a_InitializeFirmwareVars(padapter);

	return rtStatus;
}

void rtl8830a_InitializeFirmwareVars(PADAPTER padapter)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	struct pwrctrl_priv *pwrpriv = adapter_to_pwrctl(padapter);
...
static void
rtl8830a_EfusePowerSwitch(
		PADAPTER	pAdapter,
		u8		bWrite,
		u8		PwrState)
{
	hal_EfusePowerSwitch_RTL8830A(pAdapter, bWrite, PwrState);
}
...
static void
rtl8830a_EFUSE_GetEfuseDefinition(
			PADAPTER	pAdapter,
			u8		efuseType,
			u8		type,
			void		*pOut,
...
static u8
rtl8830a_Efuse_WordEnableDataWrite(PADAPTER	pAdapter,
					u16		efuse_addr,
					u8		word_en,
					u8		*data,
					BOOLEAN		bPseudoTest)
...
static u16
hal_EfuseGetCurrentSize_8830a(PADAPTER	pAdapter,
						BOOLEAN			bPseudoTest)
{
						...
						BOOLEAN			bPseudoTest)
{
	...
	ret = hal_EfuseGetCurrentSize_8830a(pAdapter, bPseudoTest);
				...
				BOOLEAN			bPseudoTest)
{
	...
	ret = hal_EfusePgPacketRead_8830a(pAdapter, offset, data, bPseudoTest);
...
static int
rtl8830a_Efuse_PgPacketRead(PADAPTER	pAdapter,
					u8			offset,
					u8			*data,
					BOOLEAN		bPseudoTest)
{
...
)
{
	...
	/* Change to check TYPE_EFUSE_MAP_LEN ,beacuse 8830A raw 256,logic map over 256. */
					...
					BOOLEAN		bPseudoTest)
{
	...
	ret = hal_EfusePgPacketWrite_8830a(pAdapter, offset, word_en, data, bPseudoTest);
...
static int
rtl8830a_Efuse_PgPacketWrite(PADAPTER	pAdapter,
					u8			offset,
					u8			word_en,
					u8			*data,
					BOOLEAN		bPseudoTest)
...
static void read_chip_version_8830a(PADAPTER padapter)
{
	u32				value32;
	...
	pHalData->version_id.ICType = CHIP_8830A;
...
void rtl8830a_start_thread(_adapter *padapter)
{
	...
	if (xmitpriv->SdioXmitThread == NULL) {
		...
		xmitpriv->SdioXmitThread = kthread_run(rtl8830as_xmit_thread, padapter, "RTWHALXT");
		if (IS_ERR(xmitpriv->SdioXmitThread)) {
			RTW_ERR("%s: start rtl8830as_xmit_thread FAIL!!\n", __func__);
...
void rtl8830a_stop_thread(_adapter *padapter)
{
#if defined(CONFIG_SDIO_HCI) || defined(CONFIG_GSPI_HCI)
#ifndef CONFIG_SDIO_TX_TASKLET
	struct xmit_priv *xmitpriv = &padapter->xmitpriv;
...
void hal_notch_filter_8830a(_adapter *adapter, bool enable)
{
	if (enable) {
...
void init_hal_spec_8830a(_adapter *adapter)
{
	struct hal_spec_t *hal_spec = GET_HAL_SPEC(adapter);

	hal_spec->ic_name = "rtl8830a";
...
bool rtl8830a_gpio_radio_on_off_check(_adapter *adapter, u8 *valid)
{
	u32 tmp32;
	bool ret;
...
void InitBeaconParameters_8830a(_adapter *adapter)
{
	...
	rtw_write8(adapter, REG_DRVERLYINT, DRIVER_EARLY_INT_TIME_8830A); /* 5ms */
	rtw_write8(adapter, REG_BCNDMATIM, BCN_DMA_ATIME_INT_TIME_8830A); /* 2ms */
...
void SetBeaconRelatedRegisters8830A(PADAPTER padapter)
{
	u32	value32;
	...
	InitBeaconParameters_8830a(padapter);
...
void rtl8830a_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	GetHwReg8830A(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	GetHwReg8830A(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	GetHwReg8830A(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	GetHwReg8830A(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void rtl8830a_set_hal_ops(struct hal_ops *pHalFunc)
{
	pHalFunc->dm_init = &rtl8830a_init_dm_priv;
	pHalFunc->dm_deinit = &rtl8830a_deinit_dm_priv;

	pHalFunc->read_chip_version = read_chip_version_8830a;

	pHalFunc->set_chnl_bw_handler = &PHY_SetSwChnlBWMode8830A;

	pHalFunc->set_tx_power_level_handler = &PHY_SetTxPowerLevel8830A;
	pHalFunc->set_tx_power_index_handler = PHY_SetTxPowerIndex_8830A;
	pHalFunc->get_tx_power_index_handler = hal_com_get_txpwr_idx;
	pHalFunc->get_txpwr_target_extra_bias = phy_get_txpwr_target_extra_bias_8830a;

	pHalFunc->hal_dm_watchdog = &rtl8830a_HalDmWatchDog;

	pHalFunc->run_thread = &rtl8830a_start_thread;
	pHalFunc->cancel_thread = &rtl8830a_stop_thread;

	pHalFunc->read_bbreg = &PHY_QueryBBReg8830A;
	pHalFunc->write_bbreg = &PHY_SetBBReg8830A;
	pHalFunc->read_rfreg = &PHY_QueryRFReg8830A;
	pHalFunc->write_rfreg = &PHY_SetRFReg8830A;

	pHalFunc->read_wmmedca_reg = &rtl8830a_read_wmmedca_reg;
	
	/* Efuse related function */
	pHalFunc->EfusePowerSwitch = &rtl8830a_EfusePowerSwitch;
	pHalFunc->ReadEFuse = &rtl8830a_ReadEFuse;
	pHalFunc->EFUSEGetEfuseDefinition = &rtl8830a_EFUSE_GetEfuseDefinition;
	pHalFunc->EfuseGetCurrentSize = &rtl8830a_EfuseGetCurrentSize;
	pHalFunc->Efuse_PgPacketRead = &rtl8830a_Efuse_PgPacketRead;
	pHalFunc->Efuse_PgPacketWrite = &rtl8830a_Efuse_PgPacketWrite;
	pHalFunc->Efuse_WordEnableDataWrite = &rtl8830a_Efuse_WordEnableDataWrite;

#ifdef DBG_CONFIG_ERROR_DETECT
	pHalFunc->sreset_init_value = &sreset_init_value;
	pHalFunc->sreset_reset_value = &sreset_reset_value;
	pHalFunc->silentreset = &sreset_reset;
	pHalFunc->sreset_xmit_status_check = &rtl8830a_sreset_xmit_status_check;
	pHalFunc->sreset_linked_status_check  = &rtl8830a_sreset_linked_status_check;
...
#ifdef CONFIG_IOL
	pHalFunc->IOL_exec_cmds_sync = &rtl8830a_IOL_exec_cmds_sync;
#endif

	pHalFunc->hal_notch_filter = &hal_notch_filter_8830a;
	pHalFunc->fill_h2c_cmd = &FillH2CCmd_88E;
	pHalFunc->fill_fake_txdesc = &rtl8830a_fill_fake_txdesc;
	pHalFunc->fw_dl = &rtl8830a_FirmwareDownload;
	pHalFunc->hal_get_tx_buff_rsvd_page_num = &GetTxBufferRsvdPageNum8830A;

#ifdef CONFIG_GPIO_API
        pHalFunc->hal_gpio_func_check = &rtl8830a_GpioFuncCheck;
#endif
#ifdef CONFIG_RFKILL_POLL
	pHalFunc->hal_radio_onoff_check = rtl8830a_gpio_radio_on_off_check;
#endif
}

u8 GetEEPROMSize8830A(PADAPTER padapter)
{
	u8 size = 0;
	u32	cr;
...
s32 InitLLTTable(PADAPTER padapter, u8 txpktbuf_bndy)
{
	s32	status = _FAIL;
	u32	i;
	u32	Last_Entry_Of_TxPktBuf = LAST_ENTRY_OF_TX_PKT_BUFFER_8830A(padapter);/* 176, 22k */
	HAL_DATA_TYPE *pHalData	= GET_HAL_DATA(padapter);
...
)
{
	...
	if (!AutoloadFail) {
		if (GetRegAmplifierType2G(Adapter) == 0) { /* AUTO*/
			...
			/* PA & LNA Type */
			PA_LNAType_2G = LE_BITS_TO_1BYTE(&PROMContent[EEPROM_RFE_OPTION_8830A], 2, 2); /* 0xCA[3:2] */
			/*
...
)
{
	...
	if (!AutoloadFail) {
		if (GetRegGLNAType(Adapter) == 0) /* AUTO */
			GLNA_type = LE_BITS_TO_1BYTE(&PROMContent[EEPROM_RFE_OPTION_8830A], 4, 3); /* 0xCA[6:4] */
		else
		...
		if (GetRegRFEType(Adapter) != 64) {
			...
			*/
			if (pHalData->rfe_type > RFETypeMaxVal)
				pHalData->rfe_type = EEPROM_DEFAULT_RFE_OPTION_8830A;

		} else if ((0xFF == PROMContent[EEPROM_RFE_OPTION_8830A]) ||
			...
			*/
			pHalData->rfe_type = PROMContent[EEPROM_RFE_OPTION_8830A] & RFETypeMask; /* 0xCA[1:0] */
		}
	} else {
		if (GetRegRFEType(Adapter) != 64) {
			pHalData->rfe_type = GetRegRFEType(Adapter);
	...
	if (Adapter->hw_port == HW_PORT1) {
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			if (!rtw_mi_get_ap_num(Adapter) && !rtw_mi_get_mesh_num(Adapter)) {
				...
				UpdateInterruptMask8830AU(Adapter, _TRUE, 0, IMR_BCNDMAINT0_88E);
#elif defined(CONFIG_SDIO_HCI)
				UpdateInterruptMask8830ASdio(Adapter, 0, SDIO_HIMR_BCNERLY_INT_MSK);
#endif
...
#if defined(CONFIG_USB_HCI)
				UpdateInterruptMask8830AU(Adapter, _TRUE , 0, (IMR_TBDER_88E | IMR_TBDOK_88E));
#elif defined(CONFIG_SDIO_HCI)
				UpdateInterruptMask8830ASdio(Adapter, 0, (SDIO_HIMR_TXBCNOK_MSK | SDIO_HIMR_TXBCNERR_MSK));
#endif
				...
				UpdateInterruptMask8830AE(Adapter, 0, 0, RT_BCN_INT_MASKS, 0);
#endif
			}
...
#ifdef CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT
#if defined(CONFIG_USB_HCI)
			UpdateInterruptMask8830AU(Adapter, _TRUE , IMR_BCNDMAINT0_88E, 0);
#elif defined(CONFIG_SDIO_HCI)
			UpdateInterruptMask8830ASdio(Adapter, SDIO_HIMR_BCNERLY_INT_MSK, 0);
#endif
#endif/* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
#if defined(CONFIG_USB_HCI)
			UpdateInterruptMask8830AU(Adapter, _TRUE , (IMR_TBDER_88E | IMR_TBDOK_88E), 0);
#elif defined(CONFIG_SDIO_HCI)
			UpdateInterruptMask8830ASdio(Adapter, (SDIO_HIMR_TXBCNOK_MSK | SDIO_HIMR_TXBCNERR_MSK), 0);
#endif
#endif/* CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR */
...
#if defined(CONFIG_PCI_HCI)
			UpdateInterruptMask8830AE(Adapter, RT_BCN_INT_MASKS, 0, 0, 0);
#endif
		}
...
u8 SetHwReg8830A(_adapter *adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(adapter);
	...
	case HW_VAR_H2C_FW_PWRMODE: {
		...
		rtl8830a_set_FwPwrMode_cmd(adapter, psmode);
	}
	break;
	case HW_VAR_H2C_FW_JOINBSSRPT: {
		u8	mstatus = (*(u8 *)val);
		rtl8830a_set_FwJoinBssReport_cmd(adapter, mstatus);
	}
	break;
#ifdef CONFIG_P2P_PS
	case HW_VAR_H2C_FW_P2P_PS_OFFLOAD: {
		u8	p2p_ps_state = (*(u8 *)val);
		rtl8830a_set_p2p_ps_offload_cmd(adapter, p2p_ps_state);
	}
	break;
#endif /* CONFIG_P2P_PS */
#ifdef CONFIG_BT_COEXIST
...
void GetHwReg8830A(_adapter *adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(adapter);
	u32 val32;
...
)
{
	...
	case HAL_DEF_RA_DECISION_RATE: {
		u8 MacID = *((u8 *)pValue);
		*((u8 *)pValue) = odm_ra_get_decision_rate_8830a(&(pHalData->odmpriv), MacID);
	}
	break;

	case HAL_DEF_RA_SGI: {
		u8 MacID = *((u8 *)pValue);
		*((u8 *)pValue) = odm_ra_get_sgi_8830a(&(pHalData->odmpriv), MacID);
	}
	break;
#endif
	...
	case HAL_DEF_PT_PWR_STATUS:
		...
		{
			u8 MacID = *((u8 *)pValue);
			*((u8 *)pValue) = odm_ra_get_hw_pwr_status_8830a(&(pHalData->odmpriv), MacID);
		}
...
#ifdef CONFIG_GPIO_API
int rtl8830a_GpioFuncCheck(PADAPTER adapter, u8 gpio_num)
{
        int ret = _SUCCESS;

        if (IS_HARDWARE_TYPE_8830A(adapter) == _FAIL) {
