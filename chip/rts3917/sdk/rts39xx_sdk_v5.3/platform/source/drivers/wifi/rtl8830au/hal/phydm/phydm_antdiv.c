...
void odm_update_rx_idle_ant(void *dm_void, u8 ant)
{
	...
	if (fat_tab->rx_idle_ant != ant) {
		...
		else if (dm->support_ic_type == ODM_RTL8830A)
	...
	void *dm_void)
{
	...
	PHYDM_DBG(dm, DBG_ANT_DIV,
		  "***8830A AntDiv_Init =>  ant_div_type=[CG_TRX_SMART_ANTDIV]\n");
...
/*PHYDM_DBG(dm,DBG_ANT_DIV,
 *	   "[8830A] SetTxAntByTxInfo_WIN: mac_id=%d, antsel_tr_mux=3'b%d%d%d\n",
 *	    mac_id, fat_tab->antsel_c[mac_id], fat_tab->antsel_b[mac_id],
 *	    fat_tab->antsel_a[mac_id]);
 */
#endif
...
#if 0
		/*panic_printk("[%s] [%d]   ******ODM_SetTxAntByTxInfo_8830A******\n",__FUNCTION__,__LINE__);*/
#endif
