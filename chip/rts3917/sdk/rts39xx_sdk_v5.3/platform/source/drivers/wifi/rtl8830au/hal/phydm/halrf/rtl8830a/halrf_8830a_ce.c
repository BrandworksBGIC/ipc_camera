...
void halrf_rf_lna_setting_8830a(struct dm_struct *dm, enum halrf_lna_set type)
{
	/*phydm_disable_lna*/
...
void set_iqk_matrix_8830a(struct dm_struct *dm, u8 OFDM_index, u8 rf_path,
...
void do_iqk_8830a(void *dm_void, u8 delta_thermal_index, u8 thermal_value,
		  u8 threshold)
{
...
void get_delta_swing_table_8830a(void *dm_void, u8 **temperature_up_a,
...
void configure_txpower_track_8830a(struct txpwrtrack_cfg *config)
{
...
boolean
phy_simularity_compare_8830a(struct dm_struct *dm, s32 result[][8], u8 c1,
			     u8 c2, boolean is2t)
{
...
void _phy_iq_calibrate_8830a(struct dm_struct *dm, s32 result[][8], u8 t,
			     boolean is2T)
{
	u32 i;
	...
	for (i = 0; i < retry_count; i++) {
		path_aok = phy_path_a_iqk_8830a(dm, is2T);
		/*		if(path_aok == 0x03){ */
		if (path_aok == 0x01) {
...
void _phy_lc_calibrate_8830a(struct dm_struct *dm, boolean is2T)
{
	u8 tmp_reg;
...
void phy_iq_calibrate_8830a(void *dm_void, boolean is_recovery)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	...
	for (i = 0; i < 3; i++) {
		...
		if (i == 1) {
			is12simular = phy_simularity_compare_8830a(dm, result, 0, 1, false);
			if (is12simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK,
		...
		if (i == 2) {
			is13simular = phy_simularity_compare_8830a(dm, result, 0, 2, false);
			if (is13simular) {
				final_candidate = 0;
				RF_DBG(dm, DBG_RF_IQK,
			...
			is23simular = phy_simularity_compare_8830a(dm, result, 1, 2, false);
			if (is23simular) {
				final_candidate = 1;
				RF_DBG(dm, DBG_RF_IQK,
...
void phy_lc_calibrate_8830a(void *dm_void)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
