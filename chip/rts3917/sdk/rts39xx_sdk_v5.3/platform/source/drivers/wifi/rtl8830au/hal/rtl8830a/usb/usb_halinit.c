...
static BOOLEAN HalUsbSetQueuePipeMapping8830AUsb(
		PADAPTER	pAdapter,
		u8		NumInPipe,
		u8		NumOutPipe
)
{
	...
	_ConfigNormalChipOutEP_8830A(pAdapter, NumOutPipe);
...
void rtl8830au_interface_configure(_adapter *padapter)
{
	HAL_DATA_TYPE	*pHalData	= GET_HAL_DATA(padapter);
	...
	HalUsbSetQueuePipeMapping8830AUsb(padapter,
			  pdvobjpriv->RtNumInPipes, pdvobjpriv->RtNumOutPipes);

}

static u32 _InitPowerOn_8830AU(_adapter *padapter)
{
	u16 value16;
	...
	if (!HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, Rtl8830A_NIC_PWR_ON_FLOW)) {
...
#if (1 == RTL8830A_RX_PACKET_INCLUDE_CRC)
	rcr |= ACRC32;
#endif
...
)
{
	...
	if (IS_WIRELESS_MODE_N_24G(Adapter) || IS_WIRELESS_MODE_N_5G(Adapter)) {
		...
		/* in usbehci.sys. */
		/*  */
		usb_AggSettingRxUpdate_8830A(Adapter);
...
u32 rtl8830au_hal_init(PADAPTER Adapter)
{
	u8	value8 = 0;
	u16  value16;
	u8	txpktbuf_bndy;
...
#define HAL_INIT_PROFILE_TAG(stage) do {} while (0)
	...
	if (pwrctrlpriv->bkeepfwalive) {
		...
		if (pHalData->bIQKInitialized) {
			...
			/*phy_iq_calibrate_8830a(Adapter, _TRUE);*/
			halrf_iqk_trigger(&pHalData->odmpriv, _TRUE);
		} else {
			/*			PHY_IQCalibrate(padapter, _FALSE); */
			/*phy_iq_calibrate_8830a(Adapter, _FALSE);*/
		...
		/*phy_lc_calibrate_8830a(&pHalData->odmpriv);*/
	...
	if (Adapter->registrypriv.mp_mode == 0) {
		status = rtl8830a_FirmwareDownload(Adapter, _FALSE);
		if (status != _SUCCESS) {
			...
			pHalData->fw_ractrl = IS_VENDOR_8830A_I_CUT_SERIES(Adapter) ? _TRUE : _FALSE;
#else
			pHalData->fw_ractrl = _FALSE;
#endif
		}
...
#if (HAL_MAC_ENABLE == 1)
	status = PHY_MACConfig8830A(Adapter);
	if (status == _FAIL) {
...
#if (HAL_BB_ENABLE == 1)
	status = PHY_BBConfig8830A(Adapter);
	if (status == _FAIL) {
...
#if (HAL_RF_ENABLE == 1)
	status = PHY_RFConfig8830A(Adapter);
	if (status == _FAIL) {
	...
	status = rtl8830a_iol_efuse_patch(Adapter);
	if (status == _FAIL) {
		RTW_INFO("%s  rtl8830a_iol_efuse_patch failed\n", __FUNCTION__);
		goto exit;
	}
#endif
	...
	InitBeaconParameters_8830a(Adapter);
...
#if RTL8830A_EARLY_MODE_PKT_NUM_10 == 1
		value8 = value8 | 0x1f;
#else
		value8 = value8 | 0xf;
#endif
...
)
{
	...
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, Rtl8830A_NIC_LPS_ENTER_FLOW);
	...
	HalPwrSeqCmdParsing(Adapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_USB_MSK, Rtl8830A_NIC_DISABLE_FLOW);
...
static void rtl8830au_hw_power_down(_adapter *padapter)
{
...
u32 rtl8830au_hal_deinit(PADAPTER Adapter)
{
	...
	if (pwrctl->bkeepfwalive) {
		...
		if ((pwrctl->bHWPwrPindetect) && (pwrctl->bHWPowerdown))
			rtl8830au_hw_power_down(Adapter);
	} else
#endif
	{
...
unsigned int rtl8830au_inirp_init(PADAPTER Adapter)
{
...
unsigned int rtl8830au_inirp_deinit(PADAPTER Adapter)
{
...
static void
Hal_CustomizeByCustomerID_8830AU(
		PADAPTER		padapter
)
{
#if 0
...
)
{
	...
	Hal_EfuseParsePIDVID_8830AU(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	...
	rtl8830a_EfuseParseChnlPlan(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	Hal_EfuseParseXtal_8830A(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	...
	Hal_ReadPAType_8830A(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	Hal_ReadAmplifierType_8830A(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	Hal_ReadRFEType_8830A(padapter, pHalData->efuse_eeprom_data, pHalData->bautoload_fail_flag);
	...
	Hal_CustomizeByCustomerID_8830AU(padapter);
...
)
{
	...
	readAdapterInfo_8830AU(Adapter);
}
...
static u8 ReadAdapterInfo8830AU(PADAPTER Adapter)
{
	/* Read EEPROM size before call any EEPROM function */
	Adapter->EepromAddressSize = GetEEPROMSize8830A(Adapter);
...
void UpdateInterruptMask8830AU(PADAPTER padapter, u8 bHIMR0 , u32 AddMSR, u32 RemoveMSR)
{
...
u8 SetHwReg8830AU(PADAPTER Adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	u8 ret = _SUCCESS;
	...
	case HW_VAR_SET_RPWM:
		...
		break;
	default:
		ret = SetHwReg8830A(Adapter, variable, val);
		break;
	}
...
void GetHwReg8830AU(PADAPTER Adapter, u8 variable, u8 *val)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);

	switch (variable) {
	default:
		GetHwReg8830A(Adapter, variable, val);
		break;
	}
...
)
{
	...
	case HAL_DEF_RX_STBC:
		*((u8 *)pValue) = 1;
		break;
	default:
		bResult = GetHalDefVar8830A(Adapter, eVariable, pValue);
		break;
	}
...
static void rtl8830au_init_default_value(_adapter *padapter)
{
...
static u8 rtl8830au_ps_func(PADAPTER Adapter, HAL_INTF_PS_FUNC efunc_id, u8 *val)
{
	u8 bResult = _TRUE;
	switch (efunc_id) {
...
void rtl8830au_set_hal_ops(_adapter *padapter)
{
	struct hal_ops	*pHalFunc = &padapter->hal_func;


	pHalFunc->hal_power_on = _InitPowerOn_8830AU;
	pHalFunc->hal_power_off = hal_poweroff_8830au;

	pHalFunc->hal_init = &rtl8830au_hal_init;
	pHalFunc->hal_deinit = &rtl8830au_hal_deinit;

	pHalFunc->inirp_init = &rtl8830au_inirp_init;
	pHalFunc->inirp_deinit = &rtl8830au_inirp_deinit;

	pHalFunc->init_xmit_priv = &rtl8830au_init_xmit_priv;
	pHalFunc->free_xmit_priv = &rtl8830au_free_xmit_priv;

	pHalFunc->init_recv_priv = &rtl8830au_init_recv_priv;
	pHalFunc->free_recv_priv = &rtl8830au_free_recv_priv;
#ifdef CONFIG_RTW_SW_LED
	pHalFunc->InitSwLeds = &rtl8830au_InitSwLeds;
	pHalFunc->DeInitSwLeds = &rtl8830au_DeInitSwLeds;
#endif/* CONFIG_RTW_SW_LED */

	pHalFunc->init_default_value = &rtl8830au_init_default_value;
	pHalFunc->intf_chip_configure = &rtl8830au_interface_configure;
	pHalFunc->read_adapter_info = &ReadAdapterInfo8830AU;
	pHalFunc->set_hw_reg_handler = &SetHwReg8830AU;
	pHalFunc->GetHwRegHandler = &GetHwReg8830AU;
	pHalFunc->get_hal_def_var_handler = &GetHalDefVar8830AUsb;
	pHalFunc->SetHalDefVarHandler = &SetHalDefVar8830AUsb;

	pHalFunc->SetBeaconRelatedRegistersHandler = &SetBeaconRelatedRegisters8830A;

	pHalFunc->hal_xmit = &rtl8830au_hal_xmit;
	pHalFunc->mgnt_xmit = &rtl8830au_mgnt_xmit;
#ifdef CONFIG_RTW_MGMT_QUEUE
	pHalFunc->hal_mgmt_xmitframe_enqueue = &rtl8830au_hal_mgmt_xmitframe_enqueue;
#endif
	pHalFunc->hal_xmitframe_enqueue = &rtl8830au_hal_xmitframe_enqueue;


#ifdef CONFIG_HOSTAPD_MLME
	pHalFunc->hostap_mgnt_xmit_entry = &rtl8830au_hostap_mgnt_xmit_entry;
#endif
	pHalFunc->interface_ps_func = &rtl8830au_ps_func;

#ifdef CONFIG_XMIT_THREAD_MODE
	pHalFunc->xmit_thread_handler = &rtl8830au_xmit_buf_handler;
#endif
#ifdef CONFIG_SUPPORT_USB_INT
	pHalFunc->interrupt_handler = interrupt_handler_8830au;
#endif
	rtl8830a_set_hal_ops(pHalFunc);
