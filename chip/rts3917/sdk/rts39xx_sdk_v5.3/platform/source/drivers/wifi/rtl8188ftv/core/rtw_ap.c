		...
		for (i = 0; i < chk_alive_num; i++) {
			...
			if (pmlmeext->active_keep_alive_check) {
				...
				if (ret == _SUCCESS) {
					...
					rtw_hal_macid_undrop(padapter, psta->cmn.mac_id);
				#endif
				...
				#ifdef RTW_WKARD_KEEP_ALIVE_DROP_MAC
					rtw_hal_macid_drop(padapter, psta->cmn.mac_id);
				#endif
...
chbw_decision:
	...
	for (i = 0; i < pdvobj->iface_nums; i++) {
		...
		rtw_ap_link_regu_forbid_update(GET_PRIMARY_LINK(pdvobj->padapters[i]), false);
	}
...
update_beacon:
	...
	if (!rtw_mi_get_ap_num(padapter))
 		padapter->is_first_ap = _TRUE;

	if (pdvobj->bcn_cnt_trx_switch == TX_BCN_CNT && padapter->is_first_ap)
		rtw_hal_set_hwreg(padapter, HW_VAR_TX_BCN_CNT, (u8 *)(&enable));
#endif
}
...
void stop_ap_mode(_adapter *padapter)
{
	...
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
#if defined(CONFIG_SUPPORT_MULTI_BCN) || defined(CONFIG_BCN_CNT)
	struct dvobj_priv *pdvobj = padapter->dvobj;
#endif
#ifdef CONFIG_SUPPORT_MULTI_BCN
	_irqL irqL;
...
#ifdef CONFIG_BCN_CNT
	if (pdvobj->bcn_cnt_trx_switch == TX_BCN_CNT && padapter->is_first_ap) {
            rtw_hal_set_hwreg(padapter, HW_VAR_TX_BCN_CNT, (u8 *)(&disable));
            rtw_reset_bcn_cnt_info(padapter);
	}

	if (padapter->is_first_ap)
		 padapter->is_first_ap = 0;
#endif
}
...
static u8 rtw_ap_ch_specific_chk(_adapter *adapter, u8 ch, u8 *bw, u8 *offset, const char *caller)
{
	...
	enum band_type band = rtw_get_band_type(ch);
	int ch_idx;
	u8 ret = _SUCCESS;
