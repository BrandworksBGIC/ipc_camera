...
#ifdef DBG_FW_POLL
static int proc_get_fw_poll(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	...
	if (rtw_get_hw_init_completed(adapter)) {
		rtw_run_in_thread_cmd_wait(adapter, ((void *)(rtw_hal_h2c_test)), adapter, 2000);
		ret = hal->h2c_test_ret;
	} else
		ret = RTW_HAL_NOT_READY;
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, ETH_ALEN)  !=  _TRUE)
				...
				RTW_PRINT_SEL(m, "==============================\n");
				_RTW_PRINT_SEL(m, "macaddr=%s\n", get_macaddr_str(mac_addr_str, m, psta->cmn.mac_addr));
...
ssize_t proc_set_btc_reduce_wl_txpwr(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	if ((IS_HARDWARE_TYPE_8822C(padapter)
		...
		|| IS_HARDWARE_TYPE_8723F(padapter))
...
static int proc_get_bcn_time(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct bcn_cnt_priv *bcn_cntpriv = &padapter->bcn_cntpriv;

	if (dvobj->bcn_cnt_trx_switch == RX_BCN_CNT) {
		if (!MLME_IS_STA(padapter)) {
			RTW_PRINT_SEL(m, "Not STA mode\n");
			return 0;
		}

		if (!check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
			RTW_PRINT_SEL(m, "No Linked BSS.\n");
			return 0;
		}
	} else {
		if (!MLME_IS_AP(padapter)) {
			RTW_PRINT_SEL(m, "Not AP mode\n");
			return 0;
		}
	}
	...
	if (bcn_cntpriv) {
		...
		_enter_critical(&bcn_cntpriv->bcn_lock, &irqL);
		bcn_ts = bcn_cntpriv->bcn_ts;
		bcn_tsf = bcn_cntpriv->bcn_tsf;
		_exit_critical(&bcn_cntpriv->bcn_lock, &irqL);

		if (dvobj->bcn_cnt_trx_switch == TX_BCN_CNT)
			RTW_PRINT_SEL(m, "beacon tsf: %llu, \n"
			"beacon tx timestamp (ns): %lld\n", bcn_tsf, bcn_ts);
		else
			RTW_PRINT_SEL(m, "beacon tsf: %llu, \n"
			"beacon rx timestamp (ns): %lld\n", bcn_tsf, bcn_ts);
	}
	return 0;
}

static int proc_get_bcn_cnt_trx(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);

	RTW_PRINT_SEL(m, "%s\n", dvobj->bcn_cnt_trx_switch?"TX beacon count":"RX beacon count");
	return 0;
}

static ssize_t proc_set_bcn_cnt_trx(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	_adapter *tmp_iface = NULL;
	struct mlme_priv *mlmepriv = &adapter->mlmepriv;
	struct sta_info *sta = NULL;
	char tmp[32];
	int enable = 1, disable = 0;
	int bcn_cnt_trx_switch = 0;
	int i = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		int num = sscanf(tmp, "%d",&bcn_cnt_trx_switch);

		if (num < 1)
			return -EINVAL;

		if (bcn_cnt_trx_switch == RX_BCN_CNT || bcn_cnt_trx_switch == TX_BCN_CNT)
			dvobj->bcn_cnt_trx_switch = bcn_cnt_trx_switch;
		else
			return -EINVAL;

		if (rtw_mi_get_assoced_sta_num(adapter) || rtw_mi_get_ap_num(adapter)) {
			for (i = 0; i < dvobj->iface_nums; i++) {
				tmp_iface = dvobj->padapters[i];
				if (tmp_iface->is_first_ap)
					ap_iface = tmp_iface;
				if (tmp_iface->is_first_ld_sta)
					sta_iface = tmp_iface;
			}
		}
		...
		if (dvobj->bcn_cnt_trx_switch == RX_BCN_CNT) {
			if (rtw_mi_get_ap_num(adapter)) {
				rtw_hal_set_hwreg(ap_iface, HW_VAR_TX_BCN_CNT, (u8 *)(&disable));
				rtw_reset_bcn_cnt_info(ap_iface);
			}
			if (rtw_mi_get_assoced_sta_num(adapter)) {
				sta = rtw_get_stainfo(&sta_iface->stapriv, get_bssid(&sta_iface->mlmepriv));
				if (sta) {
#ifdef CONFIG_FW_MULTI_PORT_SUPPORT
				rtw_hal_set_default_port_id_cmd(sta_iface, sta->cmn.mac_id);
#endif
				rtw_hal_set_hwreg(sta_iface, HW_VAR_RX_BCN_CNT, (u8 *)(&enable));
				}
			}

		} else {
			if (rtw_mi_get_assoced_sta_num(adapter)) {
				rtw_hal_set_hwreg(sta_iface, HW_VAR_RX_BCN_CNT, (u8 *)(&disable));
				rtw_reset_bcn_cnt_info(sta_iface);
			}
			if (rtw_mi_get_ap_num(adapter))
				rtw_hal_set_hwreg(ap_iface, HW_VAR_TX_BCN_CNT, (u8 *)(&enable));
		}

		RTW_INFO("bcn_cnt_trx_switch:%d\n", dvobj->bcn_cnt_trx_switch);
...
#ifdef DBG_FW_POLL
	RTW_PROC_HDL_SSEQ("fw_poll", proc_get_fw_poll, NULL),
#endif
...
#ifdef CONFIG_BCN_CNT
	RTW_PROC_HDL_SSEQ("bcn_time", proc_get_bcn_time, NULL),
	RTW_PROC_HDL_SSEQ("bcn_cnt_trx", proc_get_bcn_cnt_trx, proc_set_bcn_cnt_trx),
#endif
};
