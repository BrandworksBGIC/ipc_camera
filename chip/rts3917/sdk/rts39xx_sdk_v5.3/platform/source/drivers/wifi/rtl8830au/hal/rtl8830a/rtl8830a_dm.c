...
static void dm_CheckPbcGPIO(_adapter *padapter)
{
	...
	tmp1byte |= (HAL_8830A_HW_GPIO_WPS_BIT);
	...
	if (tmp1byte & HAL_8830A_HW_GPIO_WPS_BIT)
		bPbcPressed = _TRUE;
#else
	tmp1byte = rtw_read8(padapter, GPIO_IN);
	...
	if ((tmp1byte & HAL_8830A_HW_GPIO_WPS_BIT) == 0)
		bPbcPressed = _TRUE;
#endif
...
void rtl8830a_init_dm_priv(PADAPTER Adapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
...
void rtl8830a_deinit_dm_priv(PADAPTER Adapter)
{
	PHAL_DATA_TYPE	pHalData = GET_HAL_DATA(Adapter);
	struct dm_struct		*podmpriv = &pHalData->odmpriv;
