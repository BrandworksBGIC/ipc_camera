...
void rtw_hal_def_value_init(_adapter *padapter)
{
	...
	if (adapter_to_regsty(padapter)->adaptivity_idle_probability)
...
s32 c2h_handler(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	...
	case C2H_TXBCN_TSF:
		c2h_txbcn_tsf_hdl(adapter, payload, plen);
		break;
#endif
	case C2H_EXTEND:
		sub_id = payload[0];
...
{
	return phy_is_tx_power_limit_needed(dvobj_get_primary_adapter(dvobj));
}
