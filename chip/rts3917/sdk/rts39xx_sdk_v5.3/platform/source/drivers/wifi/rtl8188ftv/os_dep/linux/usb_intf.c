...
static void rtw_dev_shutdown(struct device *dev)
{
	...
	struct sreset_priv *psrtpriv = NULL;
#endif
		...
		if (dvobj) {
			...
			if (adapter) {
				if (!rtw_is_surprise_removed(adapter)) {
					...
					psrtpriv->user_cmd_running = _TRUE;

					if (psrtpriv->silent_reset_inprogress) {
						wait_for_completion_timeout(&psrtpriv->sreset_done, msecs_to_jiffies(3000));
						RTW_INFO("%s - sreset done\n", __func__);
					}
					#endif
					#ifdef CONFIG_WOWLAN
					...
					rtw_hal_switch_gpio_wl_ctrl(adapter, BCN_CNT_GPIO_IDX, _FALSE);
					#endif
					...
					#ifdef DBG_CONFIG_ERROR_RESET
					psrtpriv->user_cmd_running = _FALSE;
					#endif
				}
			}
...
static struct usb_device_id rtw_usb_id_tbl[] = {
	...
	{USB_DEVICE(0x3574, 0x4261), .driver_info = RTL8192F}, /* COMFAST 8192fm */
#endif
	...
	/*=== Customer ID ===*/
	{USB_DEVICE_AND_INTERFACE_INFO(0x3574, 0x5211, 0xff, 0xff, 0xff), .driver_info = RTL8821C}, /* COMFAST 8811CU */
	{USB_DEVICE_AND_INTERFACE_INFO(0x3574, 0x5231, 0xff, 0xff, 0xff), .driver_info = RTL8821C}, /* COMFAST 8821CU */
#endif
	...
	{USB_DEVICE_AND_INTERFACE_INFO(0x3574, 0x5251, 0xff, 0xff, 0xff), .driver_info = RTL8822C}, /* sihaizhonglian 8812cu */
	{USB_DEVICE_AND_INTERFACE_INFO(0x2357, 0x0137, 0xff, 0xff, 0xff), .driver_info = RTL8822C}, /* TPLINK 8812cu */
...
#ifdef CONFIG_BCN_CNT
extern int rtw_bcn_cnt_trx_switch;
#endif
...
#ifdef CONFIG_BCN_CNT
	pdvobjpriv->bcn_cnt_trx_switch = rtw_bcn_cnt_trx_switch;
#endif
