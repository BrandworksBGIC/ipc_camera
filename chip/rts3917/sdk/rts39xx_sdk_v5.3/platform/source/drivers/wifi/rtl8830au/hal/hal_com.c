...
void dump_chip_info(HAL_VERSION	ChipVersion)
{
	...
	if (IS_8830A(ChipVersion))
		cnt += sprintf((buf + cnt), "Chip Version Info: CHIP_8830A_");
...
#if defined(CONFIG_RTL8830A)
	if (rtw_get_chip_type(adapter) == RTL8830A) {
		HAL_DATA_TYPE *hal_data = GET_HAL_DATA(adapter);

		/* 8830A FW doesn't set macid no link, driver does it by self */
		if (opmode)
			...
			if (psecuritypriv->dot11PrivacyAlgrthm == _TKIP_) {
				...
				if(_rtw_wow_chk_cap(adapter, WOW_CAP_TKIP_OL))
					SET_H2CCMD_REMOTE_WAKE_CTRL_TKIP_OFFLOAD_EN(
						u1H2CRemoteWakeCtrlParm, enable);
#endif /* CONFIG_GTK_OL */
				if (IS_HARDWARE_TYPE_8830A(adapter) ||
...
static void rtw_hal_ap_wow_enable(_adapter *padapter)
{
	...
	rtw_msleep_os(2);
	#if defined(CONFIG_RTL8830A)
	if (IS_HARDWARE_TYPE_8830A(padapter))
				  ...
				  RSVDPAGE_LOC *rsvd_page_loc)
{
	...
	if (pwrctl->wowlan_pno_enable == _FALSE) {
		...
		*total_pkt_len = index + rc_len;
		...
		if (IS_HARDWARE_TYPE_8830A(adapter) || IS_HARDWARE_TYPE_8812(adapter)) {
 ...
 *
 * Page Size = 128: 8830a, 8723a/b, 8192c/d,
...
#endif
#if defined(CONFIG_RTL8830A) || defined(CONFIG_RTL8188F) || defined(CONFIG_RTL8188GTV) \
...
void rtw_hal_check_rxfifo_full(_adapter *adapter)
{
	...
	if (regsty->check_hw_status == 1) {
		...
		if (IS_8830A(pHalData->version_id) ||
...
#elif defined(CONFIG_RTL8830A)
	if (value & BIT4 && (registry_par->RegPwrTrimEnable == 1)) {
		RTW_INFO("8830AS Offset RF Gain.\n");
		RTW_INFO("8830AS Offset RF Gain. EEPROMRFGainVal=0x%x\n",
			 pHalData->EEPROMRFGainVal);
...
#ifdef CONFIG_RTS_FULL_BW 
/*
8830A: not support full RTS BW feature(mac REG no define 480[5])
*/
