...
#if(RTL8733B_SUPPORT)
void
phydm_la_mv_data_2_tx_buffer_rtl8733b(void *dm_void,  u32	source, u32	dest, u32 	length)
{
	struct dm_struct *dm = (struct dm_struct *)dm_void;
	struct rt_adcsmp *smp = &dm->adcsmp;
	struct rt_adcsmp_string *buf = &smp->adc_smp_buf;
	...
	if (is_round_up) {
		for(index = 0;index < 4;index++) {
			...
			if((imem_start_addr_offset + 0x8000) >= buf->end_pos) {
				...
				phydm_la_mv_data_2_tx_buffer_rtl8733b(dm, imem_start_addr, txbuff_start_addr, dma_len);
