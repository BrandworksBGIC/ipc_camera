...
int rtw_mbid_cam_cache_dump(void *sel, const char *fun_name, _adapter *adapter)
{
	...
	for (i = 0; i < TOTAL_MBID_CAM_NUM; i++) {
		...
		if (mbid_cam_ctl->bitmap & BIT(i)) {
			...
			_RTW_PRINT_SEL(sel, "MAC Addr:%s\t", get_macaddr_str(mac_addr_str, sel, dvobj->mbid_cam_cache[i].mac_addr));
...
int rtw_mbid_cam_dump(void *sel, const char *fun_name, _adapter *adapter)
{
	...
	for (i = 0; i < TOTAL_MBID_CAM_NUM; i++) {
		...
		_RTW_PRINT_SEL(sel, "MAC Addr:%s\n", get_macaddr_str(mac_addr_str, sel, mac_addr));
	}
	/*_exit_critical_bh(&mbid_cam_ctl->lock, &irqL);*/
	return 0;
}
...
s32 rtw_hal_set_default_port_id_cmd(_adapter *adapter, u8 mac_id)
{
	...
	RTW_INFO("%s ("ADPT_FMT") port_id :%d, mac_id:%d\n",
...
#define MLME_IS_DFT_PORT_CANDIDATE(adapter) (MLME_IS_STA(adapter) && MLME_IS_ASOC(adapter))
/**
 * rtw_select_and_set_dftport() - select and set default port while connected/disconnected
 * @padapter: the adapter which going to connect with or disconnect from AP
 * @is_connected: the caller is going to connect with or disconnect from AP
 *
 * When a STA adapter connect with or disconnect from AP,
 * this function should be used to select and set default port to FW
 * Then, BTC or other module will set its own function default port id with selected default port.
 * Information of default port will be stored in dvobj
 * rtw_set_ps_mode() and rtw_hal_wow_enable() will set default port when needed
 *
 * Return: adapter - this adapter will use fw default port function,
 *                   it may be not the same as the input adapter.
 */
struct _ADAPTER* rtw_select_and_set_dftport(struct _ADAPTER *padapter, bool is_connected)
{
	struct mlme_priv *pmlmepriv = &padapter->mlmepriv;
	struct dvobj_priv *dvobj = adapter_to_dvobj(padapter);
	struct _ADAPTER *padapter_dftport = GET_PRIMARY_ADAPTER(padapter);
	struct mlme_priv *pmlmepriv_dftport = &padapter_dftport->mlmepriv;
	u8 i, reset_to_dft = 0;

	/*
	 * primary_adapter will be designated as padapter_dftport
	 * If padapter_dftport is (STA & connected), set dft_port(padapter_dftport)
	 * Else if padapter is (STA & connected), set dft_port(padapter)
	 * Else, padapter_dftport and padapter are not (STA & connected), then loop all iface
	 * and select the first padapter[i] is (STA & connected), set dft_port(padapter[i])
	 */

#if 0
	RTW_INFO(FUNC_NDEV_FMT " padapter_dftport(%u)\n",
		 FUNC_NDEV_ARG(padapter_dftport->pnetdev), MLME_IS_DFT_PORT_CANDIDATE(padapter_dftport));
	RTW_INFO(FUNC_NDEV_FMT " padapter(%u)\n",
		 FUNC_NDEV_ARG(padapter->pnetdev), MLME_IS_DFT_PORT_CANDIDATE(padapter));
#endif

	/* If primary adapter is on connect/disconnect flow */
	if (padapter == padapter_dftport) {
		if (is_connected) {
			if (MLME_IS_DFT_PORT_CANDIDATE(padapter)) {
				goto exit;
			} else {
				RTW_WARN(FUNC_NDEV_FMT " : unexpected condition, netdev should be(STA & connected)\n",
					 FUNC_NDEV_ARG(padapter->pnetdev));
				goto loop_all_iface;
			}
		} else { /* disconnect flow */
			goto loop_all_iface;
		}
	} else { /* non-primary adapter is on connect/disconnect flow */
		if (is_connected) {
			if (MLME_IS_DFT_PORT_CANDIDATE(padapter_dftport)) {
				goto exit;
			} else {
				if (MLME_IS_DFT_PORT_CANDIDATE(padapter)) {
					padapter_dftport = padapter;
					goto exit;
				} else {
					RTW_WARN(FUNC_NDEV_FMT " : unexpected condition, netdev should be(STA & connected)\n",
						 FUNC_NDEV_ARG(padapter->pnetdev));
...
loop_all_iface:
	/*
	 * padapter_dftport and padapter are not (STA & connected)
	 * Loop all iface, select the first padapter[i] is (STA & connected), set dft_port(padapter[i])
	 * This loop will only happen on disconnect flow
	 * */
	if (DEV_STA_LD_NUM(dvobj) > 0) {
		for (i = 0; i < dvobj->iface_nums; i++) {
			padapter_dftport = dvobj->padapters[i];
			if (padapter_dftport == padapter)
				continue;
	...
	/* No padapter[i] is (STA & connected) */
	reset_to_dft = 1;
exit:
	if (reset_to_dft) {
		/* No linked STA adapter be founded, reset dft_port to PRIMARY_ADAPTER and macid 0 */
		RTW_INFO("%s() No linked STA be found, reset default_port_id to default\n", __func__);
		padapter_dftport = GET_PRIMARY_ADAPTER(padapter);
		/* Use low level function rtw_hal_set_default_port_id_cmd(_adapter *adapter, u8 mac_id) */
		if (rtw_hal_set_default_port_id_cmd(padapter_dftport, 0) != _SUCCESS)
			RTW_ERR("%s() rtw_hal_set_default_port_id_cmd fail\n", __func__);
	} else {
		if (rtw_set_default_port_id(padapter_dftport) != _SUCCESS)
			RTW_ERR("%s() rtw_set_default_port_id fail\n", __func__);
	}
#if 0
	RTW_INFO(FUNC_NDEV_FMT " is padapter_dftport(%u) for function return\n",
		 FUNC_NDEV_ARG(padapter_dftport->pnetdev), MLME_IS_DFT_PORT_CANDIDATE(padapter_dftport));
#endif

	return padapter_dftport;
}
#if 0
...
#if (defined(CONFIG_GPIO_WAKEUP) || defined(CONFIG_BCN_CNT))
void rtw_hal_switch_gpio_wl_ctrl(_adapter *padapter, u8 index, u8 enable)
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);

	/* The WAKEUP_GPIO_IDX pin mux of the 8723f chipset is automatically configured by the FW. */
	/* Therefore, no manual configuration is required in the driver code for this pin. */
...
#ifdef CONFIG_BCN_CNT
		if (dvobj->bcn_cnt_trx_switch == RX_BCN_CNT && adapter->is_first_ld_sta)
			rtw_hal_set_hwreg(adapter, HW_VAR_RX_BCN_CNT, (u8 *)(&enable));
#endif
	}
...
#if defined(CONFIG_TSF_SYNC) || defined(CONFIG_TSF_SYNC_DRV_TRIGGER)
#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_AP_MODE)
...
#endif /* defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_AP_MODE) */
#endif /* defined(CONFIG_TSF_SYNC) || defined(CONFIG_TSF_SYNC_DRV_TRIGGER) */
			...
			while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
				...
				if (psta) {
					if ((_rtw_memcmp(psta->cmn.mac_addr, bc_addr, ETH_ALEN)  !=   _TRUE)
						...
						RTW_PRINT_SEL(sel, "==============================\n");
						RTW_PRINT_SEL(sel, "macaddr =%s\n", get_macaddr_str(mac_addr_str, sel, psta->cmn.mac_addr));
						RTW_PRINT_SEL(sel, "\n");
						RTW_PRINT_SEL(sel, "Beacon:\n");
						...
						for (rf_path = 0; rf_path < hal_spec->rf_reg_path_num; rf_path++) {
							...
							RTW_PRINT_SEL(sel, "RF_PATH_%d  RSSI:%d(dBm)\n",
								 rf_path, translate_percentage_to_dbm(podmpriv->phy_dbg_info.phystatus_statistic_avg.rssi_beacon_avg[rf_path]));
						}
...
#ifdef CONFIG_BCN_CNT
void rtw_hal_set_rx_bcn_cnt(_adapter *padapter, u8 enable, u8 gpio_high_active, u8 gpio_input_en, u8 input_for_low)
{
	u8 bcn_cnt_param[H2C_BCN_CNT_LEN] = {0};
	u8 gpio_num = BCN_CNT_GPIO_IDX;
	u8 gpio_pulse_cnt = 1, gpio_pulse_en = 1;
	u8 gpio_duration = 5, gpio_duration_unit = 1;
	u8 ret = _FAIL;

	SET_H2CCMD_RX_BCN_CNT_EN(bcn_cnt_param, enable);
	SET_H2CCMD_RX_BCN_CNT_ACTIVE(bcn_cnt_param, gpio_high_active);
	SET_H2CCMD_RX_BCN_CNT_INPUT_EN(bcn_cnt_param, gpio_input_en);
	SET_H2CCMD_RX_BCN_CNT_INPUT_LOW(bcn_cnt_param, input_for_low);
	SET_H2CCMD_RX_BCN_CNT_PORTID(bcn_cnt_param, padapter->hw_port);
	SET_H2CCMD_RX_BCN_CNT_GPIO_NUM(bcn_cnt_param, gpio_num);
	SET_H2CCMD_RX_BCN_CNT_PULSE_CNT(bcn_cnt_param, gpio_pulse_cnt);
	SET_H2CCMD_RX_BCN_CNT_PULSE_EN(bcn_cnt_param, gpio_pulse_en);
	SET_H2CCMD_RX_BCN_CNT_GPIO_DURATION(bcn_cnt_param, gpio_duration);
	SET_H2CCMD_RX_BCN_CNT_GPIO_DURATION_UNIT(bcn_cnt_param, gpio_duration_unit);

	ret = rtw_hal_fill_h2c_cmd(padapter, H2C_RX_BCN_CNT, H2C_BCN_CNT_LEN, bcn_cnt_param);

	RTW_INFO("%s: ret %d\n", __func__, ret);
}

void rtw_hal_set_tx_bcn_cnt(_adapter *padapter, u8 enable, u8 gpio_high_active, u8 gpio_input_en, u8 input_for_low)
{
	u8 bcn_cnt_param[H2C_BCN_CNT_LEN] = {0};
	u8 gpio_num = BCN_CNT_GPIO_IDX;
	u8 gpio_pulse_cnt = 1, gpio_pulse_en = 1;
	...
	SET_H2CCMD_TX_BCN_CNT_EN(bcn_cnt_param, enable);
	SET_H2CCMD_TX_BCN_CNT_ACTIVE(bcn_cnt_param, gpio_high_active);
	SET_H2CCMD_TX_BCN_CNT_INPUT_EN(bcn_cnt_param, gpio_input_en);
	SET_H2CCMD_TX_BCN_CNT_INPUT_LOW(bcn_cnt_param, input_for_low);
	SET_H2CCMD_TX_BCN_CNT_MBSSID(bcn_cnt_param, 0);
	SET_H2CCMD_TX_BCN_CNT_GPIO_NUM(bcn_cnt_param, gpio_num);
	SET_H2CCMD_TX_BCN_CNT_PULSE_CNT(bcn_cnt_param, gpio_pulse_cnt);
	SET_H2CCMD_TX_BCN_CNT_PULSE_EN(bcn_cnt_param, gpio_pulse_en);
	SET_H2CCMD_TX_BCN_CNT_GPIO_DURATION(bcn_cnt_param, gpio_duration);
	SET_H2CCMD_TX_BCN_CNT_GPIO_DURATION_UNIT(bcn_cnt_param, gpio_duration_unit);

	ret = rtw_hal_fill_h2c_cmd(padapter, H2C_TX_BCN_CNT, H2C_BCN_CNT_LEN, bcn_cnt_param);

	RTW_INFO("%s: ret %d\n", __func__, ret);
}

void c2h_txbcn_tsf_hdl(_adapter *adapter, u8 *data, u8 len)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		ap_iface = dvobj->padapters[i];
		if(ap_iface->is_first_ap)
			break;
	}

	if (data && ap_iface->is_first_ap) {
		ap_iface->bcn_cntpriv.bcn_ts = rtw_sptime_get_raw();
		ap_iface->bcn_cntpriv.bcn_tsf = RTW_GET_LE64(data);
	}
...
#ifdef DBG_FW_POLL
s32 rtw_hal_h2c_test(_adapter *adapter)
{
	HAL_DATA_TYPE *hal = GET_HAL_DATA(adapter);
	s32 ret;

	if (hal->bFWReady != _TRUE)
		ret = RTW_FW_NOT_READY;
	else {
		/* use H2C_CUSTOMER_STR_REQ with En=0 to test the result of rtw_hal_fill_h2c_cmd */
		u8 h2c_data[H2C_CUSTOMER_STR_REQ_LEN] = {0};

		SET_H2CCMD_CUSTOMER_STR_REQ_EN(h2c_data, 0);
		ret = rtw_hal_fill_h2c_cmd(adapter, H2C_CUSTOMER_STR_REQ, H2C_CUSTOMER_STR_REQ_LEN, h2c_data);
	}

	hal->h2c_test_ret = ret;
