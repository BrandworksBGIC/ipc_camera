...
u8
Hal_ReadRFEType_8703B(PADAPTER Adapter, u8 *PROMContent, BOOLEAN AutoloadFail)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	...
	if (!AutoloadFail) {
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			...
			if (pHalData->rfe_type > EEPROM_MAX_RFE_8703B)
				goto  fail;

		} else if (0xFF == PROMContent[EEPROM_RFE_OPTION_8703B]){
				goto  fail;

		} else {
			pHalData->rfe_type = PROMContent[EEPROM_RFE_OPTION_8703B] & 0x1f; /* 0xCA[4:0] */
		...
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			...
			if (pHalData->rfe_type > EEPROM_MAX_RFE_8703B)
				...
				if(EEPROM_DEFAULT_RFE_OPTION_8703B == PROMContent[EEPROM_RFE_OPTION_8703B])
					goto  fail;
				else
					goto  exit;
		}
...
fail:
	...
	RTW_ERR("cmd: insmod rtl8703b.ko rtw_RFE_type=<rfe_type>\n\n");
	return _FAIL;
}
...
void rtl8703b_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	GetHwReg8703B(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	GetHwReg8703B(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	GetHwReg8703B(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	GetHwReg8703B(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void rtl8703b_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->read_wmmedca_reg = &rtl8703b_read_wmmedca_reg;
}
