...
int rtw_cmd_filter(struct cmd_priv *pcmdpriv, struct cmd_obj *cmd_obj)
{
	...
	if (cmd_obj->cmdcode == CMD_SET_DRV_EXTRA) {
		...
		if (parm->ec_id == POWER_SAVING_CTRL_WK_CID)
			if (parm->type == IPS_CTRL_LEAVE_SET_MODE || parm->type == IPS_CTRL_LEAVE_SCAN)
				return _SUCCESS;
	}
...
#define GEN_MLME_EXT_HANDLER(id, cmd, cb) [id] = { .cmd_hdl = cmd, .callback = cb},
...
static const struct rtw_cmd wlancmds[] = {
	GEN_MLME_EXT_HANDLER(CMD_JOINBSS, join_cmd_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_DISCONNECT, disconnect_hdl, rtw_disassoc_cmd_callback)
	GEN_MLME_EXT_HANDLER(CMD_CREATE_BSS, createbss_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_OPMODE, setopmode_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SITE_SURVEY, sitesurvey_cmd_hdl, rtw_survey_cmd_callback)
	GEN_MLME_EXT_HANDLER(CMD_SET_AUTH, setauth_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_KEY, setkey_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_STAKEY, set_stakey_hdl, rtw_setstaKey_cmdrsp_callback)
	GEN_MLME_EXT_HANDLER(CMD_ADD_BAREQ, add_ba_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_CHANNEL, rtw_set_chbw_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_TX_BEACON, tx_beacon_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_MLME_EVT, mlme_evt_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_DRV_EXTRA, rtw_drvextra_cmd_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_SET_CHANPLAN, rtw_set_chplan_hdl, NULL)
#ifdef CONFIG_RTW_LED_HANDLED_BY_CMD_THREAD
	GEN_MLME_EXT_HANDLER(CMD_LEDBLINK, led_blink_hdl, NULL)
#endif
	GEN_MLME_EXT_HANDLER(CMD_SET_CHANSWITCH, set_csa_hdl, NULL)
#ifdef CONFIG_TDLS
	GEN_MLME_EXT_HANDLER(CMD_TDLS, tdls_hdl, NULL)
#endif
	GEN_MLME_EXT_HANDLER(CMD_CHK_BMCSLEEPQ, chk_bmc_sleepq_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_RUN_INTHREAD, run_in_thread_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_ADD_BARSP, add_ba_rsp_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_RM_POST_EVENT, rm_post_event_hdl, NULL)
#ifdef CONFIG_RTW_MESH
	GEN_MLME_EXT_HANDLER(CMD_SET_MESH_PLINK_STATE, rtw_mesh_set_plink_state_cmd_hdl, NULL)
#endif
	GEN_MLME_EXT_HANDLER(CMD_DO_IQK, rtw_iqk_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_GET_CHANPLAN, rtw_get_chplan_hdl, rtw_get_chplan_callback)
	GEN_MLME_EXT_HANDLER(CMD_WRITE_BCN_LEN, rtw_write_bcnlen_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_AP_CHANSWITCH, set_ap_csa_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_REQ_TXRPT, rtw_reqtxrpt_cmd_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_BCN_CONTROL, bcn_control_hdl, NULL)
	GEN_MLME_EXT_HANDLER(CMD_STA_CSA_UPDATE_CAP, rtw_csa_sta_update_cap_hdl, NULL)
};
...
u8 rtw_sitesurvey_cmd(_adapter *padapter, struct sitesurvey_parm *pparm)
{
	...
	psurveyPara = rtw_malloc(sizeof(struct sitesurvey_parm));
	if (psurveyPara == NULL) {
		rtw_mfree(ph2c, sizeof(struct cmd_obj));
		return _FAIL;
	}
	...
	if (pparm)
		...
		rtw_init_sitesurvey_parm(psurveyPara);
...
{
	bool cis_scan_needed;

	/* SW only dynamic/periodic check even when device is idle */
	cis_scan_needed = rtw_regd_watchdog_hdl(dvobj);

#ifdef CONFIG_80211D
	if (!rtw_hw_is_init_completed(dvobj)) {
		if (cis_scan_needed) {
			/*
			* ENV BSS scan is needed when device idle (IPS-PWROFF)
			* leave directly and trigger ENV BSS scan
			*/
			#ifdef CONFIG_IPS
			rtw_ips_ctrl_wk_cmd(dvobj_get_primary_adapter(dvobj), IPS_CTRL_LEAVE_SCAN, -1, RTW_CMDF_DIRECTLY);
			#endif
			rtw_cis_scan_idle_check(dvobj_to_rfctl(dvobj));
		}
	}
#endif
}
