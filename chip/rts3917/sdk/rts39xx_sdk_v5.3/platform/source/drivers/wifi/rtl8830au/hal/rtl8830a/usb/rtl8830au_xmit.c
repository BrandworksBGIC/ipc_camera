 ...
 *
 *****************************************************************************/
#define _RTL8830A_XMIT_C_

#include <drv_types.h>
#include <rtl8830a_hal.h>


s32	rtl8830au_init_xmit_priv(_adapter *padapter)
{
	...
	tasklet_init(&pxmitpriv->xmit_tasklet,
		     (void(*)(unsigned long))rtl8830au_xmit_tasklet,
...
void	rtl8830au_free_xmit_priv(_adapter *padapter)
{
}
...
void rtl8830au_cal_txdesc_chksum(struct tx_desc	*ptxdesc)
{
 ...
 *   */
void rtl8830a_fill_fake_txdesc(
	PADAPTER	padapter,
	u8		*pDesc,
	u32		BufferLen,
	u8		IsPsPoll,
	...
	u8		bDataFrame)
{
	...
	if (_TRUE == bDataFrame) {
		...
		switch (EncAlg) {
		case _NO_PRIVACY_:
			SET_TX_DESC_SEC_TYPE_8830A(pDesc, 0x0);
			break;
		case _WEP40_:
		case _WEP104_:
		case _TKIP_:
			SET_TX_DESC_SEC_TYPE_8830A(pDesc, 0x1);
			break;
		case _SMS4_:
			SET_TX_DESC_SEC_TYPE_8830A(pDesc, 0x2);
			break;
		case _AES_:
			SET_TX_DESC_SEC_TYPE_8830A(pDesc, 0x3);
			break;
		default:
			SET_TX_DESC_SEC_TYPE_8830A(pDesc, 0x0);
			break;
	...
	rtl8830au_cal_txdesc_chksum(ptxdesc);
#endif
}
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pHalData->fw_ractrl == _FALSE) {
				...
				if (pattrib->ht_en)
					sgi = odm_ra_get_sgi_8830a(&pHalData->odmpriv, pattrib->mac_id);

				data_rate = odm_ra_get_decision_rate_8830a(&pHalData->odmpriv, pattrib->mac_id);

#if (POWER_TRAINING_ACTIVE == 1)
				pwr_status = odm_ra_get_hw_pwr_status_8830a(&pHalData->odmpriv, pattrib->mac_id);
...
agg_end:
	...
	UpdateEarlyModeInfo8830A(pxmitpriv, pxmitbuf);
#endif
...
s32 rtl8830au_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe)
{
 ...
 */
s32 rtl8830au_hal_xmit(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	return pre_xmitframe(padapter, pxmitframe);
}

#ifdef CONFIG_RTW_MGMT_QUEUE
s32 rtl8830au_hal_mgmt_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
static void rtl8830au_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8830au_hostap_mgnt_xmit_entry(_adapter *padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
