...
static void _rtw_bss_nums_count(_adapter *adapter, u8 *pbss_nums)
{
	...
	while (1) {
		...
		if (!pnetwork)
			break;
		chan_idx = rtw_chset_search_ch_include_dis(adapter_to_chset(adapter), pnetwork->network.Configuration.DSConfig);
...
void rtw_acs_get_rst(_adapter *adapter)
{
	...
	chan_idx = rtw_chset_search_ch_include_dis(adapter_to_chset(adapter), cur_chan);
	if ((chan_idx == -1) || (chan_idx >= MAX_CHANNEL_NUM)) {
