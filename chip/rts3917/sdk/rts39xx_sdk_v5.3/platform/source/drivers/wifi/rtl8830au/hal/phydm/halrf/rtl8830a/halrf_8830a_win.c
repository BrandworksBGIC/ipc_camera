...
void halrf_rf_lna_setting_8830a(
	struct dm_struct	*dm,
...
)
{
	...
	RF_DBG(dm, DBG_RF_TX_PWR_TRACK, "===>ODM_TxPwrTrackSetPwr8830A\n");
	...
	if (method == TXAGC) {
...
#if (MP_DRIVER != 1)
		/* phy_set_tx_power_level8830a(dm->adapter, *dm->channel); */
		...
		if (rf_path == RF_PATH_A) {
			set_iqk_matrix_8830a(dm, final_ofdm_swing_index, RF_PATH_A,
		...
		if (final_ofdm_swing_index > pwr_tracking_limit_ofdm) {   /* BBSwing higher then Limit */
			...
			set_iqk_matrix_8830a(dm, pwr_tracking_limit_ofdm, RF_PATH_A,
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][0],
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][1]);
			...
			set_iqk_matrix_8830a(dm, 0, RF_PATH_A,
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][0],
				cali_info->iqk_matrix_reg_setting[channel_mapped_index].value[0][1]);
...
)
{
	...
	if (1 <= channel && channel <= 14) {
		...
		*temperature_up_a = (u8 *)delta_swing_table_idx_2ga_p_8830a;
		*temperature_down_a = (u8 *)delta_swing_table_idx_2ga_n_8830a;
	}
}

void configure_txpower_track_8830a(
...
)
{
	...
	config->rf_path_count = MAX_PATH_NUM_8830A;
	config->thermal_reg_addr = RF_T_METER_88E;

	config->odm_tx_pwr_track_set_pwr = odm_tx_pwr_track_set_pwr88_e;
	config->do_iqk = do_iqk_8830a;
	config->phy_lc_calibrate = halrf_lck_trigger;
	config->get_delta_swing_table = get_delta_swing_table_8830a;
}
...
)
{
	...
	RF_DBG(dm, DBG_RF_IQK, "===> IQK:phy_simularity_compare_8830a c1 %d c2 %d!!!\n", c1, c2);
	...
	for (i = 0; i < bound; i++) {
		...
		if (diff > MAX_TOLERANCE) {
			RF_DBG(dm, DBG_RF_IQK, "IQK:phy_simularity_compare_8830a differnece overflow index %d compare1 0x%x compare2 0x%x!!!\n",  i, result[c1][i], result[c2][i]);
	...
	RF_DBG(dm, DBG_RF_IQK, "IQK:phy_simularity_compare_8830a simularity_bit_map   %d !!!\n", simularity_bit_map);
...
)
{
	...
	if (t == 0) {
		 ...
		 * 			RT_DISP(FINIT, INIT_IQK, ("_phy_iq_calibrate_8830a()==>0x%08x\n",bbvalue)); */
	...
	for (i = 0 ; i < retry_count ; i++) {
		path_aok = phy_path_a_iqk_8830a(dm, is2T, t + i);
	...
	if (is2T) {
		...
		for (i = 0 ; i < retry_count ; i++) {
			path_bok = phy_path_b_iqk_8830a(dm);
	...
	RF_DBG(dm, DBG_RF_IQK, "_phy_iq_calibrate_8830a() <==\n");

}


void
_phy_lc_calibrate_8830a(
	struct dm_struct		*dm,
	boolean		is2T
)
{
...
)
{
...
#endif
	{
		RF_DBG(dm, DBG_RF_INIT, "phy_iq_calibrate_8830a: Return due to is_recovery!\n");
	...
	for (i = 0; i < 3; i++) {
			_phy_iq_calibrate_8830a(dm, result, i, false);

		if (i == 1) {
			is12simular = phy_simularity_compare_8830a(dm, result, 0, 1);
			if (is12simular) {
				final_candidate = 0;
		...
		if (i == 2) {
			is13simular = phy_simularity_compare_8830a(dm, result, 0, 2);
			if (is13simular) {
				final_candidate = 0;
			...
			is23simular = phy_simularity_compare_8830a(dm, result, 1, 2);
			if (is23simular) {
				final_candidate = 1;
...
void
phy_lc_calibrate_8830a(
	void		*dm_void
)
{
	struct dm_struct		*dm = (struct dm_struct *)dm_void;

	_phy_lc_calibrate_8830a(dm, false);
}



void _phy_set_rf_path_switch_8830a(
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct		*dm,
#else
	void	*adapter,
...
)
{
	...
	if (is2T) {	/* 92C */
		...
		/* <20120504, Kordan> [8830A] We should make AntDiversity controlled by HW (0x870[9:8] = 0), */
...
void phy_set_rf_path_switch_8830a(
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct		*dm,
#else
	void	*adapter,
...
)
{
	...
	if (IS_92C_SERIAL(hal_data->VersionID))
		_phy_set_rf_path_switch_8830a(adapter, is_main, true);
	else
#endif
	{
		/* For 88C 1T1R */
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		_phy_set_rf_path_switch_8830a(adapter, is_main, false);
#else
		_phy_set_rf_path_switch_8830a(dm, is_main, false);
#endif
	}
}
...
boolean _phy_query_rf_path_switch_8830a(
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct		*dm,
#else
	void	*adapter,
...
/* return value true => Main; false => Aux */
boolean phy_query_rf_path_switch_8830a(
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	struct dm_struct		*dm
#else
	void	*adapter
...
)
{
	...
	if (IS_2T2R(hal_data->VersionID))
		return _phy_query_rf_path_switch_8830a(adapter, true);
	else
#endif
	{
		/* For 88C 1T1R */
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		return _phy_query_rf_path_switch_8830a(adapter, false);
#else
		return _phy_query_rf_path_switch_8830a(dm, false);
#endif
	}
}
#endif
