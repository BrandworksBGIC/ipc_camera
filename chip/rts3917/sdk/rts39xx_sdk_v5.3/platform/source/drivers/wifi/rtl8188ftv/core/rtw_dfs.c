...
static void rtw_dfs_ch_switch_hdl(struct dvobj_priv *dvobj, u8 band_idx)
{
	...
	rtw_hal_pause_tx_duty(m_iflink->adapter, 1);
#endif
	rtw_hal_macid_sleep_all_used(m_iflink->adapter);
	...
	rtw_hal_macid_wakeup_all_used(m_iflink->adapter);
	rtw_mi_os_xmit_schedule(m_iflink->adapter);
#ifdef CONFIG_TX_DUTY
	rtw_hal_pause_tx_duty(m_iflink->adapter, 0);
#endif
}
...
apply:
		...
		if (needed && rtw_rfctl_radar_detect_supported(rfctl))
			...
			if (needed) {
				...
				#ifdef CONFIG_ECSA
				if (rtw_hal_is_csa_support(m_iflink->adapter) && !rtw_mr_is_ecsa_running(rfctl)) {
					if (!rtw_hal_dfs_trigger_csa(m_iflink->adapter,
