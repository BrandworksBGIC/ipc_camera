...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	bool sta_11b_only = false;
	unsigned char rate_set[NDIS_802_11_LENGTH_RATES_EX];
	u8 rate_num = 0;
#endif

	if (MLME_IS_REGU_FORBID(padapter))
		return _SUCCESS;
	...
	if (!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) &&
		...
		/* In the P2P mode, the driver should not respond to the Probe Request frames
		 * that indicate support for 11b rates only
		 */
		rtw_ies_get_supported_rate(pframe + WLAN_HDR_A3_LEN + _PROBEREQ_IE_OFFSET_,
			len - WLAN_HDR_A3_LEN - _PROBEREQ_IE_OFFSET_, (u8 *)rate_set, &rate_num);

		if (rate_num > 0 && cckratesonly_included(rate_set, rate_num))
			sta_11b_only = true;
...
static void parsing_country_element_ie(struct mlme_ext_priv *pmlmeext, u8* ie, s16 len)
{
	int i = 0;
...
static void parsing_ap_channel_rpt_ie(struct mlme_ext_priv *pmlmeext, u8* ie, s16 len)
{
	int i = 0;
...
static void rtw_parsing_ap_info(_adapter *adapter, u8 *pframe, u32 len)
{
	int left;
	unsigned char *pos;
...
end_auth_null:
		...
		if (((pmlmeinfo->state & 0x03) == WIFI_FW_STATION_STATE) && (pmlmeinfo->state & WIFI_FW_ASSOC_SUCCESS)) {
			...
			if (psta != NULL) {
				...
				rtw_parsing_ap_info(padapter, pframe, len);

#ifdef CONFIG_BCN_CNT
				if (dvobj->bcn_cnt_trx_switch == RX_BCN_CNT &&
				    padapter->is_first_ld_sta) {
					sysptime ts_raw_mono = 0;
					u64 bcn_tsf;
					_irqL irqL;

					ie = pframe + sizeof(struct rtw_ieee80211_hdr_3addr);
					buf = (u32 *)ie;
					ts_raw_mono = rtw_sptime_get_raw();

					bcn_tsf = le32_to_cpu(*(buf + 1));
					bcn_tsf <<= 32;
					bcn_tsf |= le32_to_cpu(*buf);
					_enter_critical(&padapter->bcn_cntpriv.bcn_lock, &irqL);
					padapter->bcn_cntpriv.bcn_ts = ts_raw_mono;
					padapter->bcn_cntpriv.bcn_tsf = bcn_tsf;
					padapter->bcn_cntpriv.seq_num = GetSequence(pframe);
					_exit_critical(&padapter->bcn_cntpriv.bcn_lock, &irqL);
				}
#endif
...
#ifdef CONFIG_LAYER2_ROAMING
	dequeuq_roam_pkt(padapter, _TRUE);
#endif
...
int _issue_probereq(_adapter *padapter, const NDIS_802_11_SSID *pssid, const u8 *da, u8 ch, bool append_wps, int wait_ack)
{
	...
	*(fctrl) = 0;
	...
	if ((pregistrypriv->ht_enable == _TRUE)
		...
		len = rtw_build_vht_cap_ie(padapter, pframe);
		pattrib->pktlen += len;
		pframe += len;
	}
#endif
...
*/
void report_surveydone_event(_adapter *padapter, u8 reason, bool complete, bool acs)
{
	struct cmd_obj *pcmd_obj;
	u8	*pevtcmd;
	u32 cmdsz;
	...
	RTW_INFO("survey done event(%x) reason:0x%x%s for "ADPT_FMT"\n",
...
#ifdef CONFIG_BCN_CNT
	if (self_action == MLME_STA_DISCONNECTED) {
		if (dvobj->bcn_cnt_trx_switch == RX_BCN_CNT && padapter->is_first_ld_sta) {
			u8 enable = 0;

			rtw_hal_set_hwreg(padapter, HW_VAR_RX_BCN_CNT, (u8 *)(&enable));
			rtw_reset_bcn_cnt_info(padapter);
		}
		padapter->is_first_ld_sta = 0;
	}
#endif
}
...
#ifdef CONFIG_BCN_CNT
	if (psta && dvobj->bcn_cnt_trx_switch == RX_BCN_CNT && padapter->is_first_ld_sta) {
...
#ifdef CONFIG_LAYER2_ROAMING
	if ((psecuritypriv->dot11PrivacyAlgrthm == _NO_PRIVACY_) && (pmlmepriv->roam_buf_pkt))
		dequeuq_roam_pkt(padapter, _FALSE);
#endif
...
void roam_status_chk(_adapter *padapter, u8 from_timer)
{
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;
#if defined(CONFIG_LAYER2_ROAMING) && defined(CONFIG_RTW_80211K)
	...
	if (check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
		/* update last_roaming time again to prevent
		 * roaming too aggressive
		 */
	...
	if (rtw_chk_roam_flags(padapter, RTW_ROAM_ACTIVE)) {
		RTW_INFO("signal_strength = %d\n", precvpriv->beacon_strength);

		roam_cur_rssi_threshold = pmlmepriv->roam_rssi_threshold;
		if (precvpriv->beacon_strength < roam_cur_rssi_threshold) {
			pmlmepriv->need_to_roam = _TRUE;
			if (rtw_get_passing_time_ms(pmlmepriv->last_roaming) >= pmlmepriv->roam_scan_int*2000) {
				if (rtw_drv_scan_by_self(padapter, RTW_AUTO_SCAN_REASON_ROAM, false))
					pmlmepriv->last_roaming = rtw_get_current_time();
			}
		} else
...
u8 stop_ap_hdl(_adapter *adapter)
{
	...
	rtw_ap_link_regu_forbid_apply(GET_PRIMARY_LINK(adapter), false, false);
		...
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num, bool include_dis, bool no_sparse)
{
	...
	for (i = 0; i < in_num; i++) {
		...
		chinfo = rtw_chset_get_chinfo_by_bch(chset, rtw_get_band_type(in[i].hw_value), in[i].hw_value, include_dis);
		if (chinfo) {
			if (j >= out_num) {
...
static int rtw_scan_ch_dec(_adapter *padapter, struct rtw_ieee80211_channel *out,
		u32 out_num, struct rtw_ieee80211_channel *in, u32 in_num, bool include_dis, bool no_sparse)
{
...
#ifdef CONFIG_LAYER2_ROAMING
static u8 rtw_is_band_match(u8 ch, u8 band)
{
	if (band == BAND_ON_5G) {
		if (ch >= 36)
			return 1;
...
static void rtw_scan_ch_sort(_adapter *adapter, struct rtw_ieee80211_channel *out, u32 out_num)
{
	int i, j, k, l;
	struct rtw_ieee80211_channel *chs;
	u8 *band_order = adapter->mlmepriv.roam_scan_order;
	...
	for (i = 0; i < 3; i++) {
		if (band_order[i] >= BAND_MAX)
			break;
		for (j = 0; j < out_num; j++) {
			if (!rtw_is_band_match(chs[j].hw_value, band_order[i]))
				continue;
...
static int _rtw_roam_ch_decision(_adapter *padapter, struct rtw_ieee80211_channel *out,
		u32 out_num, struct rtw_ieee80211_channel *in_ch, u32 in_num, bool no_sparse, int reason)
{
...
#endif
		return rtw_scan_ch_decision(padapter, out,
				out_num, in_ch, in_num, no_sparse);
}

static int rtw_roam_ch_decision(_adapter *padapter, struct rtw_ieee80211_channel *out,
		u32 out_num, struct rtw_ieee80211_channel *in_ch, u32 in_num, bool no_sparse, int reason)
{
	...
	if (in_num) {
		...
		if (!roam_ch)
			goto roam_exit;
		roam_ch_num = _rtw_roam_ch_decision(padapter, roam_ch,
					in_num, in_ch, in_num, no_sparse, reason);
#ifdef CONFIG_LAYER2_ROAMING
		rtw_scan_ch_sort(padapter, roam_ch, roam_ch_num);
#endif
	}
	...
	/* get full channel */
	full_ch_num = _rtw_roam_ch_decision(padapter, full_ch,
				RTW_CHANNEL_SCAN_AMOUNT, NULL, 0, no_sparse, reason);

	/* sort full channel */
#ifdef CONFIG_LAYER2_ROAMING
	rtw_scan_ch_sort(padapter, full_ch, full_ch_num);
#endif
	_rtw_memset(out, 0, sizeof(struct rtw_ieee80211_channel) * out_num);
...
static void sitesurvey_res_reset(_adapter *adapter, struct sitesurvey_parm *parm)
{
	...
	if (parm->reason == RTW_AUTO_SCAN_REASON_ROAM ||
	    parm->reason == RTW_AUTO_SCAN_REASON_ROAM_ACTIVE)
		ss->ch_num = rtw_roam_ch_decision(adapter
		...
		ss->ch_num = rtw_scan_ch_dec(adapter
				, ss->ch, RTW_CHANNEL_SCAN_AMOUNT, parm->ch, parm->ch_num
				, parm->reason & RTW_AUTO_SCAN_REASON_CIS_ENV_BSS
				, parm->acs || parm->reason);
	...
	ss->acs = parm->acs;
#ifdef CONFIG_LAYER2_ROAMING
	adapter->mlmepriv.roam_scan_candidate = NULL;
#endif
}
...
operation_by_state:
	...
	case SCAN_PROCESS: {
		...
		if ((ss->reason == RTW_AUTO_SCAN_REASON_ROAM ||
		     ss->reason == RTW_AUTO_SCAN_REASON_ROAM_ACTIVE)) {
		...
		#if defined(CONFIG_RTW_ACS) && defined(CONFIG_RTW_ACS_DBG)
		else if (IS_ACS_ENABLE(padapter) && rtw_is_acs_st_valid(padapter)) {
			scan_ms = rtw_acs_get_adv_st(padapter);
		}
		#endif /*CONFIG_RTW_ACS*/
		else {
		...
		#ifdef DBG_SITESURVEY
			#ifdef CONFIG_P2P
			RTW_INFO(FUNC_ADPT_FMT" %s ch:%u (cnt:%u,idx:%d) at %dms, %c%c%c%c, R=%d(%d ms) %s\n"
				, FUNC_ADPT_ARG(padapter)
	...
	case SCAN_COMPLETE:
		...
		report_surveydone_event(padapter, ss->reason, !pmlmeext->scan_abort, ss->acs);
#ifdef CONFIG_RTW_ACS
		if (IS_ACS_ENABLE(padapter))
...
#define GEN_WLAN_EVT_HDL(id, data, cb) [id] = { .parmsize = sizeof(data), .event_callback = cb},
	...
	if (join_res >= 0) {
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if ((MLME_IS_AP(iface) || MLME_IS_MESH(iface))
				...
				rtw_ap_link_regu_forbid_update(GET_PRIMARY_LINK(iface), false);
