...
void interrupt_handler_8703bu(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
	...
	if (pHalData->IntArray[1]  & IMR_TXERR_8703B)
		RTW_INFO("===> %s Tx Error Flag Interrupt Status\n", __func__);
	if (pHalData->IntArray[1]  & IMR_RXERR_8703B)
		RTW_INFO("===> %s Rx Error Flag INT Status\n", __func__);
	if (pHalData->IntArray[1]  & IMR_TXFOVW_8703B)
		RTW_INFO("===> %s Transmit FIFO Overflow\n", __func__);
	if (pHalData->IntArray[1]  & IMR_RXFOVW_8703B)
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	pkt_cnt = GET_RX_STATUS_DESC_USB_AGG_PKTNUM_8703B(pbuf);
#endif
	...
	do {
		...
		rtl8703b_query_rx_desc_status(precvframe, pbuf);
...
void rtl8703bu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8703bu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8703BU;
	RTW_INFO("CHIP TYPE: RTL8703BU\n");
}
