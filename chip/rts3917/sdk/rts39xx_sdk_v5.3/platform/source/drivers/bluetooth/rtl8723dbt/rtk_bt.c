						...
						struct usb_device *udev)
{
	...
	list_for_each_entry(p, &hdev->le_conn_params, list) {
		...
		RTKBT_DBG("%s(): auto_connect = %d", __func__, p->auto_connect);
		RTKBT_DBG("%s(): addr_type = 0x%02x", __func__, p->addr_type);
		RTKBT_DBG("%s(): addr=%02x:%02x:%02x:%02x:%02x:%02x", __func__,
