...
static int sync_data(void)
{
	...
	if ((board_reset_mode & 0xF) == 0x3) {
		...
		if (reg > 0x62000)
			memcpy((void *)(0x82800000 + 0x5e000), (void *)(0x81000000 + 0x5e000), 0x1E000);
	}
	return 0;
}
...
static int show_dram_config(void)
{
	...
	snprintf(dram_clk, sizeof(dram_clk), " @ %d MHz\n", get_dram_clock());
...
struct init_fnc_a init_sequence_f[] = {
#ifndef CONFIG_FAST_BOOT
	FUNC_I(sync_data),
#endif
	FUNC_I(setup_mon_len),
#ifdef CONFIG_OF_CONTROL
	FUNC_I(fdtdec_setup),
#endif
#ifdef CONFIG_TRACE_EARLY
	FUNC_I(trace_early_init),
#endif
	FUNC_I(initf_malloc),
	FUNC_I(log_init),
	FUNC_I(initf_bootstage),	/* uses its own timer, so does not need DM */
	FUNC_I(event_init),
#ifdef CONFIG_BLOBLIST
	FUNC_I(bloblist_init),
#endif
	FUNC_I(setup_spl_handoff),
#if defined(CONFIG_CONSOLE_RECORD_INIT_F)
	FUNC_I(console_record_init),
#endif
#if defined(CONFIG_HAVE_FSP)
	FUNC_I(arch_fsp_init),
#endif
	FUNC_I(arch_cpu_init),		/* basic arch cpu dependent setup */
	FUNC_I(mach_cpu_init),		/* SoC/machine dependent CPU setup */
#ifndef CONFIG_FAST_BOOT
	FUNC_I(initf_dm),
#endif
#if defined(CONFIG_BOARD_EARLY_INIT_F)
	FUNC_I(board_early_init_f),
#endif
#if defined(CONFIG_PPC) || defined(CONFIG_SYS_FSL_CLK) || defined(CONFIG_M68K)
	/* get CPU and bus clocks according to the environment variable */
	FUNC_I(get_clocks),		/* get CPU and bus clocks (etc.) */
#endif
#if !defined(CONFIG_M68K)
	FUNC_I(timer_init),		/* initialize timer */
#endif
#if defined(CONFIG_BOARD_POSTCLK_INIT)
	FUNC_I(board_postclk_init),
#endif
#ifndef CONFIG_FAST_BOOT
	FUNC_I(env_init),		/* initialize environment */
#endif
	FUNC_I(init_baud_rate),		/* initialze baudrate settings */
	FUNC_I(serial_init),		/* serial communications setup */
	FUNC_I(console_init_f),		/* stage 1 init of console */
	FUNC_I(display_options),	/* say that we are here */
	FUNC_I(display_text_info),	/* show debugging info if required */
	FUNC_I(checkcpu),
#if defined(CONFIG_SYSRESET)
	FUNC_I(print_resetinfo),
#endif
#if defined(CONFIG_DISPLAY_CPUINFO)
	FUNC_I(print_cpuinfo),		/* display cpu info (and speed) */
#endif
#if defined(CONFIG_DTB_RESELECT)
	FUNC_I(embedded_dtb_select),
#endif
#if defined(CONFIG_DISPLAY_BOARDINFO)
	FUNC_I(show_board_info),
#endif
#ifdef INIT_FUNC_WATCHDOG_INIT
	FUNC_I(INIT_FUNC_WATCHDOG_INIT),
#endif
	FUNC_I(misc_init_f),
#ifdef INIT_FUNC_WATCHDOG_RESET
	FUNC_I(INIT_FUNC_WATCHDOG_RESET),
#endif
#if CONFIG_IS_ENABLED(SYS_I2C_LEGACY)
	FUNC_I(init_func_i2c),
#endif
#if defined(CONFIG_VID) && !defined(CONFIG_SPL)
	FUNC_I(init_func_vid),
#endif
	FUNC_I(announce_dram_init),
	FUNC_I(dram_init),		/* configure available RAM banks */
#ifdef CONFIG_POST
	FUNC_I(post_init_f),
#endif
#ifdef INIT_FUNC_WATCHDOG_RESET
	...
	FUNC_I(reserve_pram),
#endif
	FUNC_I(reserve_round_4k),
	FUNC_I(arch_reserve_mmu),
	FUNC_I(reserve_video),
	FUNC_I(reserve_trace),
	FUNC_I(reserve_uboot),
	FUNC_I(reserve_malloc),
	FUNC_I(reserve_board),
	FUNC_I(reserve_global_data),
	FUNC_I(reserve_fdt),
	FUNC_I(reserve_bootstage),
	FUNC_I(reserve_bloblist),
	FUNC_I(reserve_arch),
	FUNC_I(reserve_stacks),
	FUNC_I(dram_init_banksize),
	FUNC_I(show_dram_config),
#ifdef INIT_FUNC_WATCHDOG_RESET
	FUNC_I(INIT_FUNC_WATCHDOG_RESET),
#endif
	FUNC_I(setup_bdinfo),
	FUNC_I(display_new_sp),
#ifdef INIT_FUNC_WATCHDOG_RESET
	FUNC_I(INIT_FUNC_WATCHDOG_RESET),
#endif
	FUNC_I(reloc_fdt),
	FUNC_I(reloc_bootstage),
	FUNC_I(reloc_bloblist),
	FUNC_I(setup_reloc),
#if defined(CONFIG_X86) || defined(CONFIG_ARC)
	FUNC_I(copy_uboot_to_ram),
	FUNC_I(do_elf_reloc_fixups),
#endif
	FUNC_I(clear_bss),
	/*
...
void board_init_f(ulong boot_flags)
{
	...
	extern void ts_delta_init(void);
	ts_delta_init();
