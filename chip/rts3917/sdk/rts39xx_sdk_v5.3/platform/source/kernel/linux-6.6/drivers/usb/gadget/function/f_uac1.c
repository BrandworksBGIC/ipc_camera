...
#ifdef CONFIG_UAC_PLAYBACK
#define EPOUT_EN(_opts) ((_opts)->c_chmask != 0)
#define FUOUT_EN(_opts) ((_opts)->c_mute_present \
			|| (_opts)->c_volume_present)
#else
#define EPOUT_EN(_opts) (0)
#define FUOUT_EN(_opts) (0)
#endif

#ifdef CONFIG_UAC_CAPTURE
#define EPIN_EN(_opts) ((_opts)->p_chmask != 0)
#define FUIN_EN(_opts) ((_opts)->p_mute_present \
			|| (_opts)->p_volume_present)
#else
#define EPIN_EN(_opts) (0)
#define FUIN_EN(_opts) (0)
#endif
...
static char def_assoc[USB_MAX_STRING_LEN_WITH_NULL] =
			"RTSX UAC 1.0";
		...
		const struct usb_ctrlrequest *ctrl)
{
	...
	DBG(cdev, "bRequest 0x%x, w_value 0x%04x, len %d(0x%x), endpoint %d(0x%x)\n",
			ctrl->bRequest, w_value, len, ctrl->wLength, ep, ctrl->wIndex);
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	strings_uac1[STR_ASSOC].s = def_assoc;
...
F_STR_ATTR(uac1_opts, f_uac1_opts, iassoc);
