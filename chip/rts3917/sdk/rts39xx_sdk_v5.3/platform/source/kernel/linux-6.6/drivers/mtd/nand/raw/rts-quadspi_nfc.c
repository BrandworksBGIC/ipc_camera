...
MODULE_PARM_DESC(channels,
	"spi channel mode, dual/quad/fast");

enum mode {
	SPI_NAND_FAST_MODE = 1,
	SPI_NAND_DUAL_MODE = 2,
	SPI_NAND_QUAD_MODE = 4,
};

#define MODE_SHIFT	21
...
enum rts_qspi_nfc_devtype {
	TYPE_FPGA = (1 << 0),
	RTS_QUADSPI_NFC_RTS3917 = (1 << 1),
};
...
struct rts_qspi_nfc {
	...
	enum rts_qspi_nfc_devtype devtype;
};
...
static struct nand_flash_dev spi_nand_flash_ids[] = {
	...
	*/
	{
		.name = "WINBOND SPI NAND 64MiB 3,3V",
		.id = { NAND_MFR_WINBOND, 0xaa, 0x20 },
		.chipsize = 64,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
	...
	{
		.name = "WINBOND SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_WINBOND, 0xaa, 0x22 },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
	...
	{
		.name = "WINBOND SPI NAND 512MiB 3,3V",
		.id = { NAND_MFR_WINBOND, 0xaa, 0x23 },
		.chipsize = 512,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
	...
	{
		.name = "GD5F2GQ5UEYIG SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x52 },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "GD5F1GQ5UEYIG SPI NAND 128MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x51 },
		.chipsize = 128,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "GD5F1GQ4UEYIHY SPI NAND 128MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0xd9 },
		.chipsize = 128,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "GD5F4GM8UEYIG SPI NAND 512MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x95 },
		.chipsize = 512,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "GD5F2GM7UEYIG SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x92 },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "GD5F1GM7UEYIG SPI NAND 128MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x91 },
		.chipsize = 128,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "FM25S01A SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_FUDAN, 0xe4, 0x7f },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
	...
	{
		.name = "FM25S02A SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_FUDAN, 0xe5 },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.oobsize = 64,
	...
	{
		.name = "F50L1G41LB SPI NAND 128MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x01, 0x7f },
		.chipsize = 128,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
	...
	{
		.name = "F50L2G41KA SPI NAND 256MiB 3,3V",
		.id = { NAND_MFR_ESMT, 0x41, 0x7f },
		.chipsize = 256,
		.pagesize = SZ_2K,
		.erasesize = SZ_128K,
		.id_len = 3,
...
struct rts_qspi_nfc_devtype_data {
	enum rts_qspi_nfc_devtype devtype;
};

static struct rts_qspi_nfc_devtype_data rts3917_data = {
	.devtype = RTS_QUADSPI_NFC_RTS3917,
};
					...
					u32 cycle)
{
	...
	if (rqspi_nfc->devtype & RTS_QUADSPI_NFC_RTS3917)
	{
		dummy = baud * cycle * 2 + baud + 4;
	}
	else
		dummy = baud * cycle * 2 + 1;
...
static uint8_t rts_qspi_nfc_read_byte(struct nand_chip *chip)
{
	struct spi_nand *snand = chip->priv;
	uint8_t data;

	data = readb(chip->legacy.IO_ADDR_R + snand->buf_start);
	snand->buf_start++;

	return data;
}

static void rts_qspi_nfc_read_buf(struct nand_chip *chip, u8 *buf, int len)
{
	struct spi_nand *snand = chip->priv;

	/* TODO:if buffer too large, try DMA copy */
	memcpy_fromio(buf, chip->legacy.IO_ADDR_R + snand->buf_start, len);
...
static void rts_qspi_nfc_write_buf(struct nand_chip *chip,
	const uint8_t *buf, int len)
{
	...
	_memcpy_toio(chip->legacy.IO_ADDR_W + snand->buf_start, buf, len);
	snand->buf_start += len;
...
static int rts_qspi_nfc_read_page_ecc(struct nand_chip *chip,
...
static int rts_qspi_nfc_read_page_raw(struct nand_chip *chip,
	uint8_t *buf, int oob_required, int page)
{
	struct spi_nand *snand = chip->priv;
	struct mtd_info *mtd = snand->mtd;
...
static int rts_qspi_nfc_write_page_ecc(struct nand_chip *chip,
...
static int rts_qspi_nfc_write_page_raw(struct nand_chip *chip,
		const uint8_t *buf, int oob_required, int page)
{
	struct mtd_info *mtd = nand_to_mtd(chip);
	/* disable ecc write */
 ...
 */
static int rts_qspi_nfc_read_oob_raw(struct nand_chip *chip, int page)
{
	struct spi_nand *snand = chip->priv;
	struct mtd_info *mtd = snand->mtd;
	struct rts_qspi_nfc *rqspi_nfc = snand->priv;
...
static int rts_qspi_nfc_write_oob_ecc(struct nand_chip *chip, int page)
{
	int status = 0;
	struct mtd_info *mtd = nand_to_mtd(chip);
	...
	chip->legacy.cmdfunc(chip, NAND_CMD_PAGEPROG, mtd->writesize, -1);
...
static int spi_nand_write(struct spi_nand *snand, int page_offset, int length)
{
	 ...
	 * just ignore this bit. But especially for XTX nand flash,
	 * 12th bit of address byte should be 0, otherwise, write
	 * operation will make the flash busy all time.
	 */
...
static int spi_nand_status(struct spi_nand *snand)
{
	 ...
	 */
	data |= NAND_STATUS_WP;
	chip->legacy.write_buf(chip, &data, 1);
...
static void spi_nand_select_chip(struct nand_chip *chip, int ncs)
{
	/* We need this to override the default */
}

static void sw_die_select_chip(struct nand_chip *chip, int chipnr)
{
	struct spi_nand *snand = chip->priv;
	int ret;
...
static void rts_qspi_nfc_cmdfunc(struct nand_chip *chip, unsigned int command,
...
static int rts_qspi_nfc_waitfunc(struct nand_chip *chip)
{
	struct spi_nand *snand = chip->priv;
	int ret;
...
static int rts_qspi_nfc_setup(struct rts_qspi_nfc *rqspi_nfc)
{
	...
	rts_writel(rqspi_nfc, IMR, 0xFF);       /* Disable all interrupts */
...
static void simple_get_flash_id(struct spi_nand *snand)
{
	...
	for (i = 0; i < 8; i++)
		snand->id_data[i] = chip->legacy.read_byte(chip);

}
static int set_quad_mode(struct spi_nand *snand)
{
	int status, maf_id, dev_id;

	maf_id = snand->id_data[0];
	dev_id = snand->id_data[1];
	...
	case NAND_MFR_XTX:
	case NAND_MFR_FUDAN:
		if (dev_id != 0xe4 && dev_id != 0x01) {
		/* Fudan FM25S01A enable quad by default,but FM25S02A not */
...
static int spi_nand_enable_quad(struct spi_nand *snand, int maxchips)
{
	...
	if (chip->options & (SPI_NAND_QUAD_MODE << MODE_SHIFT)) {
		int ret;
	...
	} else if (chip->options & (SPI_NAND_DUAL_MODE << MODE_SHIFT)) {
...
static int rts_channel_map(char *str)
{
	char lower[10] = {0};

	strncpy(lower, str, sizeof(lower) - 1);
	if (!strcmp(lower, "quad"))
		return SPI_NAND_QUAD_MODE;
	else if (!strcmp(lower, "dual"))
		return SPI_NAND_DUAL_MODE;
	else if (!strcmp(lower, "fast"))
		return SPI_NAND_FAST_MODE;

	return -EINVAL;
}

static int qspi_nfc_attach_chip(struct nand_chip *chip)
{
	struct spi_nand *snand = chip->priv;
	struct rts_qspi_nfc *rqspi_nfc = snand->priv;
	int ret = 0;
	...
	ret = spi_nand_init_chip(snand, rqspi_nfc->maxchips);
	if (ret)
		return ret;

	return spi_nand_enable_quad(snand, rqspi_nfc->maxchips);
}

static const struct nand_controller_ops qspi_nfc_controller_ops = {
	.attach_chip = qspi_nfc_attach_chip,
};
...
static struct spi_board_info qspi_nfc_board_info[] = {
	{
		...
		.bus_num		= 0,
		.chip_select		= 0,
		.controller_data	= (void *)SPI_NAND_QUAD_MODE,
	},
};
...
MODULE_DEVICE_TABLE(of, rts_qspi_nfc_dt_ids);
...
static int rts_qspi_nfc_probe(struct platform_device *pdev)
{
	...
	struct rts_qspi_nfc_devtype_data		*qspi_nfc_devtype;
	const struct of_device_id *of_id;
	int				mode;
	const char		*spi_transfer_mode;
	char			spi_transfer_channel;

	of_id = of_match_device(rts_qspi_nfc_dt_ids, &pdev->dev);

	qspi_nfc_devtype = (void *)of_id->data;
	...
	if (!strcmp(spi_transfer_mode, "fast"))
		spi_transfer_channel = SPI_NAND_FAST_MODE;
	else if (!strcmp(spi_transfer_mode, "dual"))
		spi_transfer_channel = SPI_NAND_DUAL_MODE;
	else if (!strcmp(spi_transfer_mode, "quad"))
		spi_transfer_channel = SPI_NAND_QUAD_MODE;
	else {
	...
	board_info->controller_data = (void *)spi_transfer_channel;
	rqspi_nfc->bi = board_info;

	ret = of_property_read_u32(np, "spi-max-frequency",
			&(rqspi_nfc->bi->max_speed_hz));
	if (ret) {
	...
	if (channels) {
		...
		static const char * const channel_str[] = {
			[SPI_NAND_FAST_MODE] = "fast",
			[SPI_NAND_DUAL_MODE] = "dual",
			[SPI_NAND_QUAD_MODE]  = "quad",
			NULL,
		};
		channel_mode = rts_channel_map(channels);
	...
	chip->options |= (mode << MODE_SHIFT);
	rqspi_nfc->devtype = qspi_nfc_devtype->devtype;
	rqspi_nfc->irq = irq;
	rqspi_nfc->regs = regs;
	...
	chip->legacy.IO_ADDR_R = regs + DATA_FIFO;
	chip->legacy.IO_ADDR_W = regs + DATA_FIFO;
	chip->legacy.read_byte = rts_qspi_nfc_read_byte;
	chip->legacy.read_buf = rts_qspi_nfc_read_buf;
	chip->legacy.write_buf = rts_qspi_nfc_write_buf;
	chip->legacy.cmdfunc = rts_qspi_nfc_cmdfunc;
	chip->legacy.waitfunc = rts_qspi_nfc_waitfunc;
	chip->legacy.select_chip = spi_nand_select_chip;
	...
	if (has_die_select(snand)) {
		...
		chip->legacy.select_chip = sw_die_select_chip;
		maxchips = 2;
	} else {
		maxchips = 1;
	}

	rqspi_nfc->maxchips = maxchips;
	chip->legacy.dummy_controller.ops = &qspi_nfc_controller_ops;

	ret = nand_scan_with_ids(chip, maxchips, spi_nand_flash_ids);
	if (ret)
		goto FAIL;

	mtd_set_of_node(snand->mtd, np);
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:rts-quadspi-nfc");
