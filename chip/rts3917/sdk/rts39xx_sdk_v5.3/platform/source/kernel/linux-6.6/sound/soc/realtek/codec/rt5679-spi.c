/*
 * rt5679-spi.c  --  ALC5679 ALSA SoC audio codec driver
 *
...
static struct spi_device *rt5679_spi;

int rt5679_spi_read(u32 addr, unsigned int *val, size_t len)
{
	struct spi_device *spi = rt5679_spi;
	...
	write_buf[0] =
		(len == 4) ? RT5679_SPI_CMD_32_READ : RT5679_SPI_CMD_16_READ;
...
EXPORT_SYMBOL_GPL(rt5679_spi_read);
...
EXPORT_SYMBOL_GPL(rt5679_spi_write);

/**
 * rt5679_spi_burst_read - Read data from SPI by rt5679 dsp memory address.
 * @addr: Start address.
...
{
	u8 spi_cmd = RT5679_SPI_CMD_BURST_READ;
	int status;
	u8 write_buf[8];
	...
	while (offset < len) {
		if (offset + RT5679_SPI_BUF_LEN <= len)
			end = RT5679_SPI_BUF_LEN;
		else
			end = len % RT5679_SPI_BUF_LEN;
		...
		status = spi_sync(rt5679_spi, &message);

		if (status)
			return false;

		offset += RT5679_SPI_BUF_LEN;
	}
...
EXPORT_SYMBOL_GPL(rt5679_spi_burst_read);

/**
 * rt5679_spi_burst_write - Write data to SPI by rt5679 dsp memory address.
 * @addr: Start address.
...
{
	u8 spi_cmd = RT5679_SPI_CMD_BURST_WRITE;
	u8 *write_buf;
	unsigned int i, end, offset = 0;
	int status;

	write_buf = kmalloc(RT5679_SPI_BUF_LEN + 6, GFP_KERNEL);
	...
	while (offset < len) {
		...
		status = spi_write(rt5679_spi, write_buf, end + 6);

		if (status) {
			dev_err(&rt5679_spi->dev, "%s error %d\n", __FUNCTION__,
				status);
			kfree(write_buf);
			return status;
		}
...
EXPORT_SYMBOL_GPL(rt5679_spi_burst_write);

static int rt5679_spi_probe(struct spi_device *spi)
{
	rt5679_spi = spi;
	return 0;
}
...
static const struct of_device_id rt5679_spi_match[] = {
	{ .compatible = "realtek,rt5679", },
	{},
};
MODULE_DEVICE_TABLE(of, rt5679_spi_match);
#endif
...
static struct spi_driver rt5679_spi_driver = {
	.driver = {
			.name = "rt5679",
#if defined(CONFIG_OF)
			.of_match_table = rt5679_spi_match,
#endif
	},
	.probe = rt5679_spi_probe,
};
module_spi_driver(rt5679_spi_driver);

MODULE_DESCRIPTION("ALC5679 SPI driver");
