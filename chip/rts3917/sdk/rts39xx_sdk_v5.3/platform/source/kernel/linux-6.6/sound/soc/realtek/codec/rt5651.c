...
static int rt5651_reg_init(struct snd_soc_component *component)
{
	int i;
...
static int rt5651_index_sync(struct snd_soc_component *component)
{
	...
	for (i = 0; i < RT5651_INIT_REG_LEN; i++)
		if (RT5651_PRIV_INDEX == init_list[i].reg ||
			...
			snd_soc_component_write(component, init_list[i].reg,
					init_list[i].val);
	return 0;
}

static const struct reg_default rt5651_reg[RT5651_DEVICE_ID + 1] = {
	{RT5651_RESET, 0x0000},
...
static int rt5651_reset(struct snd_soc_component *component)
{
	return snd_soc_component_write(component, RT5651_RESET, 0);
}
 ...
 */
static int rt5651_index_write(struct snd_soc_component *component,
		unsigned int reg, unsigned int value)
{
	int ret;

	ret = snd_soc_component_write(component, RT5651_PRIV_INDEX, reg);
	if (ret < 0) {
		dev_err(component->dev, "Failed to set private addr: %d\n", ret);
		goto err;
	}
	ret = snd_soc_component_write(component, RT5651_PRIV_DATA, value);
	if (ret < 0) {
	...
	struct snd_soc_component *component, unsigned int reg)
{
	...
	return snd_soc_component_read(component, RT5651_PRIV_DATA);
}
 ...
 */
static int rt5651_index_update_bits(struct snd_soc_component *component,
	unsigned int reg, unsigned int mask, unsigned int value)
{
	unsigned int old, new;
	int change, ret;

	ret = rt5651_index_read(component, reg);
	if (ret < 0) {
	...
	change = old != new;
	if (change) {
		ret = rt5651_index_write(component, reg, new);
		if (ret < 0) {
...
static bool rt5651_volatile_register(
...
static bool rt5651_readable_register(
	struct device *dev, unsigned int reg)
{
	switch (reg) {
	case RT5651_RESET:
...
static void set_sys_clk(struct snd_soc_component *component, int sys_clk)
{
...
int rt5651_headset_detect(struct snd_soc_component *component, int jack_insert)
{
	...
	if(jack_insert) {
		...
		if (snd_soc_component_read(component, RT5651_IRQ_CTRL2) & 0x8) {
...
void rt5651_config_ovcd_thld(struct snd_soc_component *component,
...
int rt5651_check_jd_status(struct snd_soc_component *component)
{
	return snd_soc_component_read(component, RT5651_INT_IRQ_ST) & 0x0010;//To DO: Check the mask bit
}
EXPORT_SYMBOL(rt5651_check_jd_status);

int rt5651_check_bp_status(struct snd_soc_component *component)
{
	return  snd_soc_component_read(component, RT5651_IRQ_CTRL2) & 0x8;//To DO: Check the mask bit
}
...
void rt5651_enable_ovcd_interrupt(struct snd_soc_component *component,
							bool enable)
{
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[1] = snd_soc_component_read(component, regctl_addr);
	return 0;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if(ucontrol->value.integer.value[1] <= REGVAL_MAX)
		snd_soc_component_write(component, regctl_addr,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	return snd_soc_component_update_bits(component, mc->reg, RT5651_L_VOL_MASK |
			 ...
			 struct snd_soc_dapm_widget *sink)
{
	...
	val = snd_soc_component_read(snd_soc_dapm_to_component(source->dapm),
				RT5651_GLB_CLK);
...
static void hp_amp_power(struct snd_soc_component *component, int on)
{
	...
	if(on) {
		if(hp_amp_power_count <= 0) {
			/* depop parameters */
			snd_soc_component_update_bits(component, RT5651_DEPOP_M2,
				RT5651_DEPOP_MASK, RT5651_DEPOP_MAN);
			snd_soc_component_update_bits(component, RT5651_DEPOP_M1,
				RT5651_HP_CP_MASK | RT5651_HP_SG_MASK |
				RT5651_HP_CB_MASK,
				RT5651_HP_CP_PU | RT5651_HP_SG_DIS |
				RT5651_HP_CB_PU);
			rt5651_index_write(component, RT5651_HP_DCC_INT1, 0x9f00);
			/* headphone amp power on */
			snd_soc_component_update_bits(component, RT5651_PWR_ANLG1,
				RT5651_PWR_FV1 | RT5651_PWR_FV2 , 0);
			snd_soc_component_update_bits(component, RT5651_PWR_VOL,
			...
			snd_soc_component_update_bits(component, RT5651_PWR_ANLG1,
				...
				RT5651_PWR_FV1 | RT5651_PWR_FV2);
			snd_soc_component_update_bits(component, RT5651_CHARGE_PUMP,
				RT5651_PM_HP_MASK, RT5651_PM_HP_HV);
			rt5651_index_update_bits(component, RT5651_CHOP_DAC_ADC,
				0x0200, 0x0200);
			snd_soc_component_update_bits(component, RT5651_DEPOP_M1,
...
static void rt5651_pmu_depop(struct snd_soc_component *component)
{
	hp_amp_power(component, 1);

	/* headphone unmute sequence */
	snd_soc_component_update_bits(component, RT5651_DEPOP_M3,
	...
	rt5651_index_write(component, RT5651_MAMP_INT_REG2, 0xfc00);
	snd_soc_component_update_bits(component, RT5651_DEPOP_M1,
	...
	snd_soc_component_update_bits(component, RT5651_HP_VOL,
	...
	snd_soc_component_update_bits(component, RT5651_DEPOP_M1,
		...
		RT5651_HP_L_SMT_DIS | RT5651_HP_R_SMT_DIS);
	msleep(20);
	snd_soc_component_update_bits(component, RT5651_HP_CALIB_AMP_DET,
		RT5651_HPD_PS_MASK, RT5651_HPD_PS_EN);
}

static void rt5651_pmd_depop(struct snd_soc_component *component)
{
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		rt5651_pmu_depop(component);
		break;

	case SND_SOC_DAPM_PRE_PMD:
		rt5651_pmd_depop(component);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		hp_amp_power(component,1);
		snd_soc_component_update_bits(component, RT5651_LOUT_CTRL1,
			RT5651_L_MUTE | RT5651_R_MUTE, 0);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_update_bits(component, RT5651_PDM_CTL,
			RT5651_M_PDM_L, 0);
		break;
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		snd_soc_component_write(component, RT5651_PLL_MODE_1, 0x9a00);
		snd_soc_component_write(component, RT5651_PLL_MODE_2, 0xf800);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		snd_soc_component_write(component, RT5651_PLL_MODE_1, 0);
		snd_soc_component_write(component, RT5651_PLL_MODE_2, 0);
	default:
		return 0;
	}
...
static int rt5651_set_sysclk(struct snd_soc_component *component,
			     int clk_id, int source, unsigned int freq, int dir)
{
	struct rt5651_priv *rt5651 = snd_soc_component_get_drvdata(component);

	set_sys_clk(component, clk_id);
	if (freq == rt5651->sysclk && clk_id == rt5651->sysclk_src)
		return 0;

	if (freq == 19200000) {
		snd_soc_component_write(component, 0x86, 0x23d7);
		snd_soc_component_write(component, 0x87, 0x23d7);
	}
	rt5651->sysclk = freq;
...
{
	return rt5651_set_sysclk(dai->component, clk_id, 0, freq, dir);
}
			...
			unsigned int freq_in, unsigned int freq_out)
{
	...
	if (!freq_in || !freq_out) {
		...
		rt5651->pll_in = 0;
		rt5651->pll_out = 0;
		set_sys_clk(component, RT5651_SCLK_S_MCLK);
		return 0;
	}
	...
	dev_dbg(component->dev, "bypass=%d m=%d n=%d k=2\n", pll_code.m_bp,
	...
	struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < 0xb4; i++) {
		if (cnt + RT5651_REG_DISP_LEN >= PAGE_SIZE)
			break;
		val = rt5651_index_read(component, i);
		if (!val)
			continue;
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_OFF:
		set_sys_clk(component, RT5651_SCLK_S_RCCLK);
...
static void do_enable_push_button_int(struct work_struct *work)
{
	...
	snd_soc_component_update_bits(rt5651_codec, RT5651_IRQ_CTRL2,
...
static int rt5651_probe(struct snd_soc_component *component)
{
	...
	if (0x6281 != snd_soc_component_read(component, RT5651_DEVICE_ID)) {
		dev_err(component->dev, "Can't find 5651 codec\n");
		return -ENODEV;
	}

	rt5651_reset(component);
	snd_soc_component_update_bits(component, RT5651_PWR_ANLG1,
		RT5651_PWR_VREF1 | RT5651_PWR_MB |
		RT5651_PWR_BG | RT5651_PWR_VREF2,
		RT5651_PWR_VREF1 | RT5651_PWR_MB |
	...
	rt5651_reg_init(component);
	snd_soc_component_update_bits(component, RT5651_PWR_ANLG1,
		RT5651_PWR_LDO_DVO_MASK, RT5651_PWR_LDO_DVO_1_2V);

	rt5651_set_bias_level(component, SND_SOC_BIAS_OFF);
	...
	realtek_ce_init_hwdep(component);
#endif
#endif
...
static void rt5651_remove(struct snd_soc_component *component)
{
...
{
	rt5651_set_bias_level(component, SND_SOC_BIAS_STANDBY);
	return 0;
}
#else
...
#define RT5651_STEREO_RATES SNDRV_PCM_RATE_8000_192000
...
static struct snd_soc_component_driver soc_codec_dev_rt5651 = {
	.probe = rt5651_probe,
...
static int rt5651_i2c_probe(struct i2c_client *i2c)
{
	...
	if (!rt5651->regmap) {
		printk("Failed to allocate register map\n");
		return -1;
	}

	ret = devm_snd_soc_register_component(&i2c->dev, &soc_codec_dev_rt5651,
...
static void rt5651_i2c_remove(struct i2c_client *i2c)
{
