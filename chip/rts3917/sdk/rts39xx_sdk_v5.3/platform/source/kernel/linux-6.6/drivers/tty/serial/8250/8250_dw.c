...
#define DW_UART_QUIRK_REALTEK		BIT(4)
...
static int dw8250_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(data->pctrl)) {
		err = PTR_ERR(data->pctrl);
		return err;
	}

	default_state = pinctrl_lookup_state(data->pctrl,
					     PINCTRL_STATE_DEFAULT);
	if (IS_ERR(default_state)) {
		dev_warn(&pdev->dev, "have not set default pinctrl state\n");
	} else {
		err = pinctrl_select_state(data->pctrl, default_state);
		if (err) {
...
static const struct of_device_id dw8250_of_match[] = {
	...
	{ .compatible = "realtek,rts-uart", .data = &dw8250_realtek_data },
	{ /* Sentinel */ }
};
