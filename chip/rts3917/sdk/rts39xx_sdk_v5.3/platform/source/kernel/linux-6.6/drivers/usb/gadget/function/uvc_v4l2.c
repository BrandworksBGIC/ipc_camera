...
{
	char guid[2][16] = {UVC_GUID_FORMAT_MJPEG, UVC_GUID_FORMAT_H264};
	...
	case UVCG_UNCOMPRESSED:
		...
		memcpy(guid[0], unc->desc.guidFormat, sizeof(guid));
		break;
	case UVCG_H265:
		return ERR_PTR(-EINVAL);
	case UVCG_H264:
	...
	format = uvc_format_by_guid(guid[idx]);
	if (!format)
		return ERR_PTR(-EINVAL);
...
static const struct uvc_format_desc *to_uvc_format(struct uvcg_format *uformat)
{
	...
	case UVCG_H264:
	case UVCG_H265:
		h26x = to_uvcg_h26x(&uformat->group.cg_item);
		if (!h26x)
			return ERR_PTR(-EINVAL);

		memcpy(guid, h26x->desc.guidFormat, sizeof(guid));
		break;
	default:
		break;
	}
...
static struct uvcg_frame *find_closest_frame_by_size(struct uvc_device *uvc,
					   int idx, struct uvcg_format *uformat,
					   u16 rw, u16 rh)
{
	struct uvc_video *video = &uvc->video[idx];
	struct uvcg_format_ptr *format;
	struct uvcg_frame_ptr *frame;
	struct uvcg_frame *uframe = NULL;
	unsigned int d, maxd;
...
uvc_send_response_int(struct uvc_device *uvc, struct uvc_request_data *data)
{
	...
	usb_ep_dequeue(uvc->interrupt_ep, req);

	if (data->length < 0) {
		usb_ep_set_halt(uvc->interrupt_ep);
		return 0;
	}
	...
	return usb_ep_queue(uvc->interrupt_ep, req, GFP_KERNEL);
}
...
uvc_v4l2_get_format(struct file *file, void *fh, struct v4l2_format *fmt)
{
	struct video_device *vdev = video_devdata(file);
	struct uvc_device *uvc = video_get_drvdata(vdev);
	struct uvc_video *video = &uvc->video[vdev->num - STREAM_BASE_NUM];
...
uvc_v4l2_try_format(struct file *file, void *fh, struct v4l2_format *fmt)
{
	struct video_device *vdev = video_devdata(file);
	struct uvc_device *uvc = video_get_drvdata(vdev);
	int strm_idx = vdev->num - STREAM_BASE_NUM;
	struct uvc_video *video = &uvc->video[strm_idx];
	struct uvcg_format *uformat;
	struct uvcg_frame *uframe;
	u8 *fcc;
	...
	uframe = find_closest_frame_by_size(uvc, strm_idx, uformat,
				fmt->fmt.pix.width, fmt->fmt.pix.height);
	if (!uframe)
		return -EINVAL;
...
uvc_v4l2_set_format(struct file *file, void *fh, struct v4l2_format *fmt)
{
	...
	struct uvc_video *video = &uvc->video[vdev->num - STREAM_BASE_NUM];
	int ret;
	unsigned int isize = fmt->fmt.pix.sizeimage;
	...
	video->imagesize = video->bpp ? fmt->fmt.pix.sizeimage : isize;
	video->maximagesize = fmt->fmt.pix.sizeimage;
	return ret;
}
...
uvc_v4l2_enum_format(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	...
	if (f->index >= uvc->header->num_fmt[strm_idx])
		return -EINVAL;
...
uvc_v4l2_qbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	if (uvc->state[strm_idx] == UVC_STATE_STREAMING)
...
uvc_v4l2_streamon(struct file *file, void *fh, enum v4l2_buf_type type)
{
	struct video_device *vdev = video_devdata(file);
	struct uvc_device *uvc = video_get_drvdata(vdev);
	int stream = vdev->num - STREAM_BASE_NUM;
	struct uvc_video *video = &uvc->video[stream];
	int ret;
	...
	if (uvc->state[stream] == UVC_STATE_STREAMING) {
		rlxprintk(RLX_TRACE_STATUS, "%s already streaming\n",
				__func__);
		return 0;
	}
	...
	if (uvc->state[stream] == UVC_STATE_STOPPING) {
		rlxprintk(RLX_TRACE_STATUS, "%s already stop\n",
				__func__);
		return -ECONNRESET;
	}

	rlxprintk(RLX_TRACE_STATUS, "%s change %d to streaming\n",
			__func__, uvc->state[stream]);
	uvc->state[stream] = UVC_STATE_STREAMING;
...
uvc_v4l2_streamoff(struct file *file, void *fh, enum v4l2_buf_type type)
{
	...
	rlxprintk(RLX_TRACE_STATUS, "%s change %d to connected\n",
			__func__, uvc->state[strm_idx]);
	uvc->state[strm_idx] = UVC_STATE_CONNECTED;
...
static void uvc_v4l2_disable(struct uvc_device *uvc, int idx)
{
	uvc_function_disconnect(uvc);
	uvcg_video_enable(&uvc->video[idx], 0);
	uvcg_free_buffers(&uvc->video[idx].queue);
			   ...
			   const struct v4l2_event_subscription *sub)
{
	...
	if (sub->type == UVC_EVENT_SETUP && handle->is_uvc_app_handle) {
		uvc_v4l2_disable(uvc, fh->vdev->num
				- STREAM_BASE_NUM);
...
uvc_v4l2_open(struct file *file)
{
	...
	handle->device = &uvc->video[vdev->num - STREAM_BASE_NUM];
...
uvc_v4l2_release(struct file *file)
{
	...
	if (handle->is_uvc_app_handle)
		uvc_v4l2_disable(uvc, vdev->num - STREAM_BASE_NUM);
...
uvc_v4l2_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	return uvcg_queue_mmap(
		&uvc->video[vdev->num - STREAM_BASE_NUM].queue, vma);
}
...
uvc_v4l2_poll(struct file *file, poll_table *wait)
{
	...
	return uvcg_queue_poll(
		&uvc->video[vdev->num - STREAM_BASE_NUM].queue, file, wait);
}
		...
		unsigned long flags)
{
	...
	return uvcg_queue_get_unmapped_area(
		&uvc->video[vdev->num - STREAM_BASE_NUM].queue, pgoff);
}
#endif
