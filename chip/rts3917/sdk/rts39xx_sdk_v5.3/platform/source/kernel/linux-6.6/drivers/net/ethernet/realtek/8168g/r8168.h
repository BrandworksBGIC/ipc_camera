...
#define RTL8168_VERSION                                                        \
	"8.048.03" NAPI_SUFFIX FIBER_SUFFIX REALWOW_SUFFIX DASH_SUFFIX
...
#define ETH_MISC_R32(reg) ((unsigned long)readl(eth_misc_addr + (reg)))
#define ETH_MISC_W32(reg, val32) (writel((val32), eth_misc_addr + (reg)))
...
#define RTL_SET_GSO_MAX_SIZE(dev, size)                                        \
	netif_set_gso_max_size(dev, size)
#else
#define RTL_SET_GSO_MAX_SIZE(dev, size)                                        \
	netif_set_tso_max_size(dev, size)
#endif
...
//const char RTL8168_MAC = "44:77:39:3a:9b:01";
struct rtl8168_private {
	...
	struct reset_control *fephy_reset;
	struct reset_control *eth_mem_up;
#ifdef CONFIG_R8168_NAPI
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 24)
	...
	int chipset;
	int extern_phy;
	struct gpio_desc *extern_phy_reset_gpio;
	u32 mcfg;
...
void rtl8168_mdio_prot_write(struct rtl8168_private *tp, u32 RegAddr,
			     u32 value);
