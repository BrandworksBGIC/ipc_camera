...
/**
 * config_ep_by_speed_and_alt_idx() - configures the given endpoint
 ...
 */
int config_ep_by_speed_and_alt_idx(struct usb_gadget *g,
...
EXPORT_SYMBOL_GPL(config_ep_by_speed_and_alt_idx);
...
int config_ep_by_speed_and_alt(struct usb_gadget *g,
				...
				u8 alt)
{
	return config_ep_by_speed_and_alt_idx(g, f, _ep, alt, 0);
}
...
void usb_remove_interface_id(struct usb_configuration *config, unsigned int id)
{
	if (id < MAX_CONFIG_INTERFACES && config->interface[id]) {
		config->interface[id] = NULL;
		if (id < config->next_interface_id)
			config->next_interface_id = id;
	}
}
EXPORT_SYMBOL_GPL(usb_remove_interface_id);
...
static __u8 MS_Capability_ID[16] = {
	0xDF, 0x60, 0xDD, 0xD8, 0x89, 0x45, 0xC7, 0x4C,
	0x9C, 0xD2, 0x65, 0x9D, 0x9E, 0x64, 0x8A, 0x9F};
static __u8 dwWindowsVersion_Win8[4] = {0x00, 0x00, 0x03, 0x06};
static __u8 WINUSB_CompatibleID[8] = {
	0X57, 0X49, 0X4E, 0X55, 0X53, 0X42, 0x00, 0x00};
static __u8 msos_property_name[40] = {
	...
	0x44, 0x00, 0x00, 0x00};
static __u8 msos_property_data[78] = {
...
static int msbos_desc(struct usb_composite_dev *cdev)
{
	struct usb_bos_descriptor       *bos = cdev->req->buf;
	struct usb_msos_platform_capability_desc_header *msos_header;
	struct usb_msos_descriptor_setinfo *msos_setinfo;
	...
	msos_header = cdev->req->buf + le16_to_cpu(bos->wTotalLength);
	le16_add_cpu(&bos->wTotalLength, USB_DT_MSOS_HEAD_SIZE);
	msos_header->bLength = USB_DT_MSOS_HEAD_SIZE;
	msos_header->bDescriptorType = USB_DEVICE_CAPABILITY_DESCRIPTOR_TYPE;
	msos_header->bDevCapabilityType = USB_DEVICE_CAPABILITY_PLATFORM;
	msos_header->bReserved = 0;
	memcpy(msos_header->MS_OS_20_Platform_Capability_ID,
			MS_Capability_ID, 16);

	msos_setinfo = cdev->req->buf + le16_to_cpu(bos->wTotalLength);
	le16_add_cpu(&bos->wTotalLength, USB_DT_MSOS_SET_INFO_SIZE);
	msos_header->bLength += USB_DT_MSOS_SET_INFO_SIZE;
	memcpy(msos_setinfo->dwWindowsVersion, dwWindowsVersion_Win8, 4);
	msos_setinfo->wMSOSDescriptorSetTotalLength =
		cpu_to_le16(USB_DT_MSOS_DESC_SET_HEADER_SIZE
				+ USB_DT_COMPATIBLE_ID_SIZE
				+ USB_DT_REGISTRY_PROPERTY_SIZE);
	if (cdev->get_dfu_interface_id && cdev->get_dfu_interface_id())
		le16_add_cpu(&msos_setinfo->wMSOSDescriptorSetTotalLength,
			     USB_DT_MSOS_FUNCTION_SUBSET_HEADER_SIZE);

	msos_setinfo->bMS_VendorCode = MS_VendorCode;
	msos_setinfo->bAltEnumCode = 0;

	return le16_to_cpu(bos->wTotalLength);
}

static int get_msos_desc(struct usb_composite_dev *cdev)
{
	struct usb_msos_desc_set_header *desc_set_header = cdev->req->buf;
	struct usb_msos_function_subset_header *function_subset_header;
	struct usb_msos_compatible_id_desc *compatible_id;
	struct usb_msos_registry_property_desc *registry_property;

	desc_set_header->wLength =
		cpu_to_le16(USB_DT_MSOS_DESC_SET_HEADER_SIZE);
	desc_set_header->wDescriptorType =
		cpu_to_le16(MS_OS_20_SET_HEADER_DESCRIPTOR);
	memcpy(desc_set_header->dwWindowsVersion, dwWindowsVersion_Win8, 4);
	desc_set_header->wTotalLength =
		cpu_to_le16(USB_DT_MSOS_DESC_SET_HEADER_SIZE);

	if (cdev->get_dfu_interface_id && cdev->get_dfu_interface_id()) {
		function_subset_header = cdev->req->buf +
			le16_to_cpu(desc_set_header->wTotalLength);
		le16_add_cpu(&desc_set_header->wTotalLength,
			     USB_DT_MSOS_FUNCTION_SUBSET_HEADER_SIZE);
		function_subset_header->wLength =
			cpu_to_le16(USB_DT_MSOS_FUNCTION_SUBSET_HEADER_SIZE);
		function_subset_header->wSubsetLength =
			cpu_to_le16(USB_DT_MSOS_FUNCTION_SUBSET_HEADER_SIZE);
		function_subset_header->wDescriptorType =
			cpu_to_le16(MS_OS_20_SUBSET_HEADER_FUNCTION);
		function_subset_header->bFirstInterface =
			cdev->get_dfu_interface_id();
		function_subset_header->bReserved = 0;
	}

	compatible_id = cdev->req->buf +
		le16_to_cpu(desc_set_header->wTotalLength);
	le16_add_cpu(&desc_set_header->wTotalLength, USB_DT_COMPATIBLE_ID_SIZE);
	compatible_id->wLength = cpu_to_le16(USB_DT_COMPATIBLE_ID_SIZE);
	compatible_id->wDescriptorType =
		cpu_to_le16(MS_OS_20_FEATURE_COMPATBLE_ID);
	memcpy(compatible_id->CompatibleID, WINUSB_CompatibleID, 8);
	memset(compatible_id->SubCompatibleID, 0, 8);
	if (cdev->get_dfu_interface_id && cdev->get_dfu_interface_id())
		le16_add_cpu(&function_subset_header->wSubsetLength,
			     USB_DT_COMPATIBLE_ID_SIZE);

	registry_property = cdev->req->buf +
		le16_to_cpu(desc_set_header->wTotalLength);
	le16_add_cpu(&desc_set_header->wTotalLength,
			USB_DT_REGISTRY_PROPERTY_SIZE);
	registry_property->wLength = cpu_to_le16(USB_DT_REGISTRY_PROPERTY_SIZE);
	registry_property->wDescriptorType =
		cpu_to_le16(MS_OS_20_FEATURE_REG_PROPERTY);
	registry_property->wPropertyDataType = cpu_to_le16(REG_SZ);
	registry_property->wPropertyNameLength = cpu_to_le16(40);
	memcpy(registry_property->PropertyName, msos_property_name, 40);
	registry_property->wPropertyDataLength = cpu_to_le16(78);
	memcpy(registry_property->PropertyData, msos_property_data, 78);
	if (cdev->get_dfu_interface_id && cdev->get_dfu_interface_id())
		le16_add_cpu(&function_subset_header->wSubsetLength,
			     USB_DT_REGISTRY_PROPERTY_SIZE);

	return le16_to_cpu(desc_set_header->wTotalLength);
}
...
int composite_filteroutdata(struct usb_gadget *gadget,
