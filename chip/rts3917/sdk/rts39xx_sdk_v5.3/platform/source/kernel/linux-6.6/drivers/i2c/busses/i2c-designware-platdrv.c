...
static int dw_i2c_plat_probe(struct platform_device *pdev)
{
	...
	dev->rst = devm_reset_control_get_optional(&pdev->dev, "reset-i2c");
	if (IS_ERR(dev->rst))
	...
	dev->clk = devm_clk_get_optional(&pdev->dev, "i2c_ck");
	if (IS_ERR(dev->clk)) {
		ret = PTR_ERR(dev->clk);
		goto exit_reset;
	}
	...
	if (ret) {
		dev_err(&pdev->dev, "Enable i2c clk failed\n");
		goto exit_reset;
	}
