...
static int proc_get_eth_phy(struct seq_file *m, void *v)
{
	...
	spin_lock_irqsave(&tp->lock, flags);
	seq_puts(m, "\n####################page 0##################\n ");
	rtl8168_mdio_write(tp, 22, 0); //rtl8168_mdio_write(tp, 0x1f, 0);
				    ...
				    int count, int *eof, void *data)
{
	...
	len += snprintf(
		...
		tp->HwSuppMagicPktVer, tp->HwSuppCheckPhyDisableModeVer,
		tp->HwPkgDet, tp->HwSuppGigaForceMode, tp->random_mac,
		tp->org_mac_addr,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 13)
		dev->perm_addr,
#endif
			    ...
			    int *eof, void *data)
{
	...
	len += snprintf(page + len, count - len,
			"\n####################page 0##################\n");
	rtl8168_mdio_write(tp, 22, 0); //rtl8168_mdio_write(tp, 0x1f, 0x0000);
	for (n = 0; n < max;) {
		len += snprintf(page + len, count - len, "\n0x%02x:\t", n);
...
void rtl8168_mdio_write(struct rtl8168_private *tp, u32 RegAddr, u32 value)
{
	if (tp->rtk_enable_diag)
		return;
	//printk(KERN_INFO "write phy reg %d = 0x%04x", RegAddr, value);
...
static void rtl8168_set_dash_other_fun_dev_pci_cmd_register(
	struct rtl8168_private *tp, u8 pci_cmd_reg, u8 multi_fun_sel_bit)
{
	u32 clearmask;
	u32 setmask;
	...
	rtl8168_other_fun_dev_pci_setting(tp, 0x04, clearmask, setmask,
					  multi_fun_sel_bit);
}
...
static void rtl8168_driver_start(struct rtl8168_private *tp)
{
	...
	case CFG_METHOD_33:
		...
		if (tp->DASH) {
			rtl8168_set_dash_other_fun_dev_state_change(tp, 0,
								    0x85);
			rtl8168_set_dash_other_fun_dev_pci_cmd_register(
				tp, 0x07, 0x85);
		} else {
			rtl8168_set_dash_other_fun_dev_state_change(tp, 3,
								    0x85);
		}
		break;
	}
...
static void rtl8168_nic_reset(struct net_device *dev)
{
	...
	case CFG_METHOD_24:
	case CFG_METHOD_25:
		rtl8168_mac_ocp_write(tp, 0xE404,
				      (rtl8168_mac_ocp_read(tp, 0xE404) &
				       ~(0x300)) |
					      (1 << 9));
		//printk(KERN_INFO "after setting, WDT2 reg = 0x%04x", rtl8168_mac_ocp_read(tp, 0xE404));//printkrtl8168_mac_ocp_read(tp, 0xE404);
		break;
	}
...
static unsigned int rtl8168_xmii_link_ok(struct net_device *dev)
{
	...
	////////////////////////////FD Add//get PHY status
	switch (tp->mcfg) {
	case CFG_METHOD_24:
	case CFG_METHOD_25:
		//	rtl8168_mdio_read(tp, MII_BMSR);
		//	retval = rtl8168_mdio_read(tp, MII_BMSR);
		//	if((retval & BMSR_LSTATUS) == 0x4)//LinkON
		//	{
		//		rtl8168_mac_ocp_write(tp, 0xDE40,  rtl8168_mac_ocp_read(tp, 0xDE40) & (~0x3C) | 0x2C);//FORCE link OK//SPEED = 1000BASET FULL
		//				//printk(KERN_INFO "after setting, LINKSTATUS reg = 0x%04x", rtl8168_mac_ocp_read(tp, 0xDE40));
		//	}
		break;
	}
	///////////////////////////FD Add/get PHY Status
		...
		if (link_status_on) {
			...
			if (tp->mcfg == CFG_METHOD_21 ||
				...
				if (!(RTL_R8(tp, PHYstatus) & FullDup)) {
					rtl8168_mdio_write(
						tp, 22,
						0x0000); //rtl8168_mdio_write(tp, 0x1F, 0x0000);
					rtl8168_mdio_write(
						tp, MII_ADVERTISE,
						rtl8168_mdio_read(
							tp, MII_ADVERTISE) &
...
static void rtl8168_powerdown_pll(struct net_device *dev)
{
	...
	if (tp->wol_enabled == WOL_ENABLED || tp->DASH ||
		...
		rtl8168_mdio_write(
			tp, 22, 0x0000); //rtl8168_mdio_write(tp, 0x1F, 0x0000);
				  ...
				  u8 duplex, u32 adv)
{
	...
	if (autoneg == AUTONEG_ENABLE) {
		...
		rtl8168_mdio_write(tp, 22,
				   0); //rtl8168_mdio_write(tp, 0x1f, 0x0000);
		rtl8168_mdio_write(tp, MII_ADVERTISE, auto_nego);
		rtl8168_mdio_write(tp, MII_CTRL1000, giga_ctrl);
		rtl8168_phy_restart_nway(dev);
		mdelay(20);
...
static int rtl_nway_reset(struct net_device *dev)
{
	...
	rtl8168_mdio_write(tp, 22, 0x0000);
	; //rtl8168_mdio_write(tp, 0x1F, 0x0000);
	bmcr = rtl8168_mdio_read(tp, MII_BMCR);
...
static void rtl8168_get_mac_version(struct rtl8168_private *tp)
{
	...
	// printk(KERN_INFO "++011111++\n");
	val32 = RTL_R32(tp, TxConfig);
	reg = val32 & 0x7c800000;
	ICVerID = val32 & 0x00700000;

	// printk(KERN_INFO "++0222222++\n");
	//printk(KERN_INFO "TxConfig = 0x%08x, reg = 0x%x, ICver = 0x%x\n", val32, reg, ICVerID);
	switch (reg) {
	case 0x30000000:
...
static void rtl8168_print_mac_version(struct rtl8168_private *tp)
{
	...
	printk(KERN_INFO "mac_version == Unknown\n");
...
rtl8168_phy_ram_code_check(struct net_device *dev)
{
		...
		rtl8168_mdio_write(tp, 22, 0x0000);//rtl8168_mdio_write(tp, 0x1F, 0x0000);
...
static void rtl8168_hw_phy_config(struct net_device *dev)
{
	...
	if (tp->mcfg == CFG_METHOD_1) {
		...
		int i = 0, temp = 0;
		rtl8168_mdio_write(tp, 22, 0);
		unsigned int phyid = rtl8168_mdio_read(tp, 0x3);
		phyid = (phyid << 16) + rtl8168_mdio_read(tp, 0x2);
		printk(KERN_INFO "This Phy ID = 0x%08x\n", phyid);
		//for(i = 0; i < 32; i ++)
		//{
		//printk(KERN_INFO "This Phy reg %d = 0x%08x\n", i, rtl8168_mdio_read(tp, i));
		//msleep(10);
		//}
		//rtl8168_mdio_read(tp, 0x13);
		//msleep(10);
		//printk(KERN_INFO "This Phy reg 19 = 0x%08x\n", phyid);
		rtl8168_mac_ocp_write(tp, 0xE404,
				      (rtl8168_mac_ocp_read(tp, 0xE404) &
				       ~(0x300)) |
					      (1 << 9));
		...
		rtl8168_mac_ocp_write(
			tp, 0xEA34,
			(rtl8168_mac_ocp_read(tp, 0xEA34) & (~0x3)) | BIT_1);
		//printk(KERN_INFO "after setting, GMII ENABLE reg = 0x%04x", rtl8168_mac_ocp_read(tp, 0xEA34));

		rtl8168_mdio_write(tp, 22, 0);
		// rtl8168_mdio_write(tp, 0, 0x9140);
		// temp = rtl8168_mdio_read(tp, 0x1);
		// do{
		//		 temp = rtl8168_mdio_read(tp, 0x1);
		// }while((temp & 0x4) == 0x0);
		//
		rtl8168_mac_ocp_write(
			tp, 0xDE40,
			rtl8168_mac_ocp_read(tp, 0xDE40) & (~0x3C) |
				0x2C); //FORCE link OK//SPEED = 1000BASET FULL
		//printk(KERN_INFO "after setting, LINKSTATUS reg = 0x%04x", rtl8168_mac_ocp_read(tp, 0xDE40));
		msleep(10);
		printk(KERN_INFO "This Phy check over\n");
	...
	if (dev && dev->dev.parent) {
		if (of_device_is_compatible(dev->dev.parent->of_node,
			...
			rtl8168_mdio_write(tp, 0x1F, 0x0002);
			rtl8168_mdio_write(tp, 0x19,
					   (rtl8168_mdio_read(tp, 0x19) &
					    0xff00) |
						   0xff73);
		} else if (of_device_is_compatible(dev->dev.parent->of_node,
						   "realtek, rts3901-r8168")) {
			rtl8168_mdio_write(tp, 0x1F, 0x0001);
			rtl8168_mdio_write(tp, 0x13, 0x7400);
		} else if (of_device_is_compatible(dev->dev.parent->of_node,
						   "realtek, rle0745-r8168")) {
			/* Adjust built-in phy RX SNR */
			rtl8168_mdio_write(tp, 0x1F, 0x0001);
			rtl8168_mdio_write(tp, 0x1D, 0x000F);
			rtl8168_mdio_write(tp, 0x1F, 0x0001);
			rtl8168_mdio_write(tp, 0x10, 0x7181);
		} else if (of_device_is_compatible(
				   dev->dev.parent->of_node,
				   "realtek,sheipa-r8168")) { //add
		}
	}
	...
	if (HW_HAS_WRITE_PHY_MCU_RAM_CODE(tp)) {
		if (tp->eee_enabled) {
			if (tp->extern_phy == 1)
				rtl8168_enable_external_EEE(tp);
			else
				rtl8168_enable_EEE(tp);
		} else {
			if (tp->extern_phy == 1)
				rtl8168_disable_external_EEE(tp);
			else
				rtl8168_disable_EEE(tp);
		}
	}
...
static void rtl8168_netpoll(struct net_device *dev)
{
	...
	disable_irq(dev->irq);
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 19)
	rtl8168_interrupt(dev->irq, dev, NULL);
#else
...
static void rtl8168_init_software_variable(struct net_device *dev)
{
	...
	case CFG_METHOD_29:
	case CFG_METHOD_30:
		tp->HwSuppCheckPhyDisableModeVer = 0; //2;
		break;
	case CFG_METHOD_23:
	case CFG_METHOD_27:
	case CFG_METHOD_28:
...
static int rtl8168_get_mac_address(struct net_device *dev)
{
	...
	//printk(KERN_INFO "#########get mac address start!\n");

	for (i = 0; i < MAC_ADDR_LEN; i++) {
		// printk(KERN_INFO "#########get mac[%d] = ", i);
		mac_addr[i] = RTL_R8(tp, MAC0 + i);
		mdelay(10);
		// printk(KERN_INFO "0x%02x\n", mac_addr[i]);
	}
	//printk(KERN_INFO "#########get mac address start0!\n");
	if (!is_valid_ether_addr(mac_addr)) {
		//printk(KERN_INFO "#########get mac address valid!\n");
	...
	//printk(KERN_INFO "#########get mac address start1!\n");

	rtl8168_rar_set(tp, mac_addr);
	//printk(KERN_INFO "#########get mac address start2!\n");

	for (i = 0; i < MAC_ADDR_LEN; i++) {
		/* keep the original MAC address */
		tp->org_mac_addr[i] = RTL_R8(tp, MAC0 + i);
		mdelay(10);
	}

	eth_hw_addr_set(dev, tp->org_mac_addr);

	//printk(KERN_INFO "#########get mac address over01!\n");
#if LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 13)
	memcpy(dev->perm_addr, dev->dev_addr, dev->addr_len);
#endif
	//printk(KERN_INFO "#########get mac address over!\n");
...
{
	void __iomem *ioaddr;
	void __iomem *eth_misc_addr;
	struct net_device *dev;
	struct rtl8168_private *tp;
	int rc = -ENOMEM, eth_misc_re, i = 0;
	struct clk *clk;
	int irq;
	struct resource *addr_res, *eth_misc_res;
	...
	if (device_property_read_bool(dev->dev.parent, "external_phy"))
		tp->extern_phy = 1;
	...
	tp->mmio_addr = ioaddr;
	// printk(KERN_INFO "mmio addr = 0x%08x\n", addr_res->start);
	...
	/* chose mii/rmii */
	if (tp->extern_phy) {
		//ETH_MISC_W32(ETH_MISC_CFG, PHY_SEL);
		//ETH_MISC_W32(ETH_MISC_CFG2, LDO_FEPHY_POW);

		tp->extern_phy_reset_gpio = devm_gpiod_get_index(
			&tp->dev->dev, "extern_phy_reset", 0, GPIOD_OUT_HIGH);
		if (IS_ERR(tp->extern_phy_reset_gpio))
			return PTR_ERR(tp->extern_phy_reset_gpio);
		gpiod_set_value(tp->extern_phy_reset_gpio, 0);
		mdelay(30);
		gpiod_set_value(tp->extern_phy_reset_gpio, 1);
		mdelay(30);
	}
	...
	//#ifdef CONFIG_RTL8168_TARGET_FPGA
	//		u32 tmp1;
	...
	//printk(KERN_INFO "++333nefor hry, ac++\n");
	/* Identify chip attached to board */
	rtl8168_get_mac_version(tp);
	//printk(KERN_INFO "++333333++\n");
...
err_out_misc_res:
	...
	//			if (of_device_is_compatible(dev->dev.parent->of_node,
	//						"realtek, rts3915-r8168"))
	//				release_mem_region(eth_misc_res->start, sizeof(struct rtl8168_private));
	//		else if(of_device_is_compatible(dev->dev.parent->of_node,
	//					"realtek,sheipa-r8168")) {//add
	//		release_mem_region(eth_misc_res->start, sizeof(struct rtl8168_private));
	//		}
...
static int rtl8168_init_mac_address(struct net_device *dev, const char *mac)
{
	struct rtl8168_private *tp = netdev_priv(dev);
	unsigned long flags;
	int addr[ETH_ALEN];
	...
	if (sscanf(mac, "%02x:%02x:%02x:%02x:%02x:%02x", &addr[0], &addr[1],
	...
	rtl8168_rar_set(tp, dev->dev_addr);
	//printk(KERN_INFO "#########init mac address over!\n");
	spin_unlock_irqrestore(&tp->lock, flags);
	// printk(KERN_INFO "#########init mac address over----1!\n");
	return 0;
}
...
static int __devinit rtl8168_init_one(struct platform_device *pdev)
{
	...
	// printk(KERN_INFO "+++++++++++\n");
	rc = rtl8168_init_board(pdev, &dev, &ioaddr, &irq);
	if (rc)
		return rc;
	...
	if (tp->mcfg != CFG_METHOD_DEFAULT) {
		...
		if ((tp->mcfg == CFG_METHOD_1) || (tp->mcfg == CFG_METHOD_2) ||
			...
			RTL_SET_GSO_MAX_SIZE(dev, LSO_32K);
			...
			RTL_SET_GSO_MAX_SIZE(dev, LSO_64K);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 18, 0)
...
#endif
	// printk(KERN_INFO "#########here01!\n");
	rtl8168_exit_oob(dev);
	// printk(KERN_INFO "#########here02!\n");

	rtl8168_hw_init(dev);
	// printk(KERN_INFO "#########here03!\n");

	rtl8168_hw_reset(dev);
	// printk(KERN_INFO "#########here04!\n");
	...
	rc = of_property_read_string(pdev->dev.of_node, "mac-address",
				     &mac_str);
	if (rc) {
		dev_err(&pdev->dev, "Failed to read mac address from DT\n");
		return -EINVAL;
	}
	rtl8168_init_mac_address(dev, mac_str);
	...
	rtl8168_disable_rxdvgate(dev);
	//printk(KERN_INFO "#########here6!\n");

	eth_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(eth_pinctrl))
		return PTR_ERR(eth_pinctrl);
	//printk(KERN_INFO "#########here7!\n");

	default_state =
		pinctrl_lookup_state(eth_pinctrl, PINCTRL_STATE_DEFAULT);
...
static int rtl8168_open(struct net_device *dev)
{
	...
	/*
		 ...
		 */
	tp->TxDescArray =
		dma_alloc_coherent(&tp->platform_dev->dev, R8168_TX_RING_BYTES,
				   &tp->TxPhyAddr, GFP_KERNEL);
	if (!tp->TxDescArray)
		goto err_free_all_allocated_mem;

	tp->RxDescArray =
		dma_alloc_coherent(&tp->platform_dev->dev, R8168_RX_RING_BYTES,
...
static void rtl8168_hw_start(struct net_device *dev)
{
	...
	////////////////////////////////////FD Add Enable GMII for GMAC
	rtl8168_mac_ocp_write(tp, 0xEA34,
			      (rtl8168_mac_ocp_read(tp, 0xEA34) & (~0x3)) |
				      BIT_1);
	//printk(KERN_INFO "after setting, GMII ENABLE reg = 0x%04x", rtl8168_mac_ocp_read(tp, 0xEA34));
	/////////////////////////////////////FD Add
					...
					u32 opts2)
{
	...
	paddr = tp->ShortPacketEmptyBufferPhy;
	status = rtl8168_get_txd_opts1(opts1, len, entry);
	status |= LastFrag;

	txd->addr = cpu_to_le64(paddr);
...
static const struct of_device_id rtl8168_dt_ids[] = {
	...
	{
		.compatible = "realtek,sheipa-r8168",
	},
	{ /* sentinel */ },
};
