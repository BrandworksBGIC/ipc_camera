...
struct uvcg_streaming_header {
	...
	unsigned int                            num_fmt[UVC_STREAMING_NUMS];
};
...
struct uvcg_frame {
	...
	} __attribute__((packed)) h26x_frame;
	};
#else
	struct {
		u8	b_length;
	...
	struct {
		...
		u8	bm_supported_usages[4];
		u8	bm_capabilities[2];
		u8	bm_svc_capabilities[4];
		u8	bm_mvc_capabilities[4];
		u32	dw_min_bit_rate;
		u32	dw_max_bit_rate;
		u32	dw_default_frame_interval;
		u8	b_frame_interval_type;
	} __attribute__((__packed__)) h26x_frame;
#endif
...
struct uvcg_h26x {
	struct uvcg_format		fmt;
	struct uvc_format_h26x		desc;
};

static inline struct uvcg_h26x *to_uvcg_h26x(struct config_item *item)
{
	return container_of(
		container_of(to_config_group(item), struct uvcg_format, group),
		struct uvcg_h26x, fmt);
}
