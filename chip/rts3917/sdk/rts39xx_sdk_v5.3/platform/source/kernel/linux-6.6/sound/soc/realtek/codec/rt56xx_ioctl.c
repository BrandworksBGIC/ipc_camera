...
{
	struct snd_soc_component *component = hw->private_data;
				     ...
				     unsigned long arg)
{
	...
	if (copy_from_user(&rt56xx, _rt56xx, sizeof(rt56xx))) {
		dev_err(component->dev, "copy_from_user faild\n");
		return -EFAULT;
	}
	dev_dbg(component->dev, "%s(): rt56xx.number=%d, cmd=%d\n",
	...
	case RT_READ_CODEC_REG_IOCTL:
		for (p = buf; p < buf + rt56xx.number / 2; p++)
			*(p + rt56xx.number / 2) = snd_soc_component_read(component, *p);
	...
	case RT_WRITE_CODEC_REG_IOCTL:
		for (p = buf; p < buf + rt56xx.number / 2; p++)
			snd_soc_component_write(component, *p, *(p + rt56xx.number / 2));
		break;
	...
	case RT_READ_CODEC_INDEX_IOCTL:
		...
		for (p = buf; p < buf + rt56xx.number / 2; p++)
			*(p + rt56xx.number / 2) =
			    rt56xx_ioctl_ops.index_read(component, *p);
		if (copy_to_user(rt56xx.buf, buf, sizeof(*buf) * rt56xx.number))
			goto err;
		break;
	...
	case RT_WRITE_CODEC_INDEX_IOCTL:
		...
		for (p = buf; p < buf + rt56xx.number / 2; p++)
			rt56xx_ioctl_ops.index_write(component, *p,
						     *(p + rt56xx.number / 2));
		break;
				 ...
				 struct file *file, unsigned long arg)
{
	...
	struct rt56xx_cmd __user *_rt56xx = (struct rt56xx_cmd *)arg;
	struct rt56xx_cmd rt56xx;
	int i, *buf, number = component->driver->reg_cache_size;

	dev_dbg(component->dev, "enter %s, number = %d\n", __func__, number);
	if (copy_from_user(&rt56xx, _rt56xx, sizeof(rt56xx)))
		return -EFAULT;
	...
	for (i = 0; i < number / 2; i++) {
		buf[i] = i << 1;
		buf[i + number / 2] = component->read(component, buf[i]);
	}
...
int realtek_ce_init_hwdep(struct snd_soc_component *component)
{
	struct snd_hwdep *hw;
