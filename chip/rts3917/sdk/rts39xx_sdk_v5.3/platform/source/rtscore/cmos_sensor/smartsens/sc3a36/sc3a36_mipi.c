...
static struct sc3a36_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc3a36_fps_info[] = {
	{20, 3000, 118800000},
};

//sensor setting MIPI@30FPS
static struct rts_isp_i2c_reg g_sc3a36_i2c_init_regs[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x3039, 0x80},
	{0x3029, 0x80},
...
static int sc3a36_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_sc3a36_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc3a36_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_sc3a36_fps_info); i++)
		if (fps == g_sc3a36_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc3a36_fps_info))
		return NULL;

	return &g_sc3a36_fps_info[i];
}

static int sc3a36_get_init_info(uint32_t isp_id,
				 const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct sc3a36_status *status;
	uint16_t half_hts;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	debug("sc3a36 get fps %.1f init info\n", mode->fps);

	status = &g_status[isp_id];
	fps_info = sc3a36_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0], g_sc3a36_i2c_init_regs, 0);
...
static int sc3a36_start(uint32_t isp_id)
{
	struct sc3a36_status *status;
...
static int sc3a36_get_tuned_again(uint32_t isp_id,
...
static int sc3a36_get_tuned_dgain(uint32_t isp_id,
...
static int sc3a36_get_exposure_gain_info(uint32_t isp_id,
		...
		struct rts_isp_sync_regs *regs)
{
	...
	if (exp_gain->analog_gain[0] < 2) {
		...
		set_sync_i2c(&reg[i++], 0x3301, 0x40);
		set_sync_i2c(&reg[i++], 0x3630, 0x19);
		set_sync_i2c(&reg[i++], 0x3633, 0x55);
...
static int sc3a36_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops sc3a36_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "sc3a36",
	.get_info = sc3a36_get_info,
	.get_init_info = sc3a36_get_init_info,
	.start = sc3a36_start,
	.get_tuned_again = sc3a36_get_tuned_again,
	.get_tuned_dgain = sc3a36_get_tuned_dgain,
	.get_exposure_gain_info = sc3a36_get_exposure_gain_info,
	.check = sc3a36_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc3a36_ops)
