 ...
 * Copyright (C) 2025 Wei Mo <wei_mo@apowertec.com>
 */
...
static struct sc2331_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc2331_fps_info[] = {
	{30, 2200, 79200000},
};

static struct rts_isp_i2c_reg g_sc2331_30fps_i2c_init_regs[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
...
static int sc2331_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_sc2331_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc2331_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_sc2331_fps_info); i++)
		if (fps == g_sc2331_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc2331_fps_info))
		return NULL;

	return &g_sc2331_fps_info[i];
}

static int sc2331_get_init_info(uint32_t isp_id,
				const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct sc2331_status *status;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	debug("sc2331 get fps %.1f init info\n", mode->fps);

	status = &g_status[isp_id];
	fps_info = sc2331_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0],
		g_sc2331_30fps_i2c_init_regs, 0);
...
static int sc2331_start(uint32_t isp_id)
{
	struct sc2331_status *status;
...
static int sc2331_get_tuned_again(uint32_t isp_id,
...
static int sc2331_get_tuned_dgain(uint32_t isp_id,
...
static int sc2331_get_exposure_gain_info(uint32_t isp_id,
		...
		struct rts_isp_sync_regs *regs)
{
	...
	exp_reg_value_float =
		1.0 * exp_gain->exposure[0] / (status->exp_step / 2) + 0.5f;
	exp_reg_value =
		clip_d_word(exp_reg_value_float, 2, (2 * total_line - 13));
...
static int sc2331_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops sc2331_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "sc2331",
	.get_info = sc2331_get_info,
	.get_init_info = sc2331_get_init_info,
	.start = sc2331_start,
	.get_tuned_again = sc2331_get_tuned_again,
	.get_tuned_dgain = sc2331_get_tuned_dgain,
	.get_exposure_gain_info = sc2331_get_exposure_gain_info,
	.check = sc2331_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc2331_ops)
