...
static const struct fps_info g_sc2310_fps_info[] = {
	{30, 2200, 74250000},
};

static struct rts_isp_i2c_reg g_sc2310_i2c_init_regs[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
...
static int sc2310_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_sc2310_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc2310_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_sc2310_fps_info); i++)
		if (fps == g_sc2310_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc2310_fps_info))
		return NULL;

	return &g_sc2310_fps_info[i];
}
			       ...
			       struct rts_isp_sensor_init_info *info)
{
	...
	fps_info = sc2310_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0], g_sc2310_i2c_init_regs, 0);
