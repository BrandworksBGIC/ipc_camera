 ...
 * Copyright (C) 2023 benno_ma <benno_ma@realsil.com.cn>
 */
...
static struct sc4336p_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc4336p_fps_info[] = {
	{30, 2800, 126000000},
};

static struct rts_isp_i2c_reg g_sc4336p_30fps_i2c_init_regs[] = {
	{0x0103, 0x01},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	{0x301f, 0x05},
...
static int sc4336p_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_sc4336p_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc4336p_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_sc4336p_fps_info); i++)
		if (fps == g_sc4336p_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc4336p_fps_info))
		return NULL;

	return &g_sc4336p_fps_info[i];
}

static int sc4336p_get_init_info(uint32_t isp_id,
				 const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct sc4336p_status *status;


	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	debug("sc4336p get fps %.1f init info\n", mode->fps);

	status = &g_status[isp_id];
	fps_info = sc4336p_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0],
		g_sc4336p_30fps_i2c_init_regs, 0);
...
static int sc4336p_start(uint32_t isp_id)
{
	struct sc4336p_status *status;
...
static int sc4336p_get_tuned_again(uint32_t isp_id,
...
static int sc4336p_get_tuned_dgain(uint32_t isp_id,
...
static int sc4336p_get_exposure_gain_info(uint32_t isp_id,
...
static int sc4336p_get_mirror_flip(uint32_t isp_id,
...
static int sc4336p_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops sc4336p_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "sc4336p",
	.get_info = sc4336p_get_info,
	.get_init_info = sc4336p_get_init_info,
	.start = sc4336p_start,
	.get_tuned_again = sc4336p_get_tuned_again,
	.get_tuned_dgain = sc4336p_get_tuned_dgain,
	.get_exposure_gain_info = sc4336p_get_exposure_gain_info,
	.get_mirror_flip = sc4336p_get_mirror_flip,
	.check = sc4336p_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc4336p_ops)
