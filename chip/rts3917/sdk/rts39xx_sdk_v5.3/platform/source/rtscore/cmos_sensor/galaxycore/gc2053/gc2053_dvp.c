...
static const struct fps_info g_gc2053_fps_info_dvp[] = {
	{30, 2200, 74250000},
};

static struct rts_isp_i2c_reg g_gc2053_i2c_init_regs_dvp[] = {
	/*system*/
	...
	{0x32, 0xf8}, //0xd8  //{0x32[7]=1,0xd3[7]=1 rsth=vref
	{0x37, 0x03}, //[3:2]eqc2sel=0
	{0x39, 0x15}, //0x17  //0//[3:0]rsgl
	{0x43, 0x07},//vclamp
	...
	{0x77, 0x01}, //0x00 //dacin offset x31
	{0x78, 0x00}, //0x20 //dacin offset
	...
	{0x94, 0x01},
	{0x95, 0x04},
	{0x96, 0x40}, //[10:0]out_height
	{0x97, 0x07},
	{0x98, 0x88}, //[11:0]out_width
	/*ISP*/
	{0xfe, 0x01},
	{0x01, 0x05}, //[3]dpc blending mode
...
static int gc2053_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_gc2053_fps_info_dvp[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *gc2053_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_gc2053_fps_info_dvp); i++)
		if (fps == (g_gc2053_fps_info_dvp)[i].fps)
			break;
	if (i == ARRAY_SIZE((g_gc2053_fps_info_dvp)))
		return NULL;

	return &(g_gc2053_fps_info_dvp)[i];
				...
				struct rts_isp_sensor_init_info *info)
{
	...
	set_init_i2c_regs(info->sensor_regs[0], g_gc2053_i2c_init_regs_dvp, 0);
