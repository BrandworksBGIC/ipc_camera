...
static struct jxf22_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_jxf22_fps_info[] = {
	{30, 2400, 81000000},
};

static struct rts_isp_i2c_reg g_jxf22_i2c_init_regs[] = {
	{0x12, 0x40}, {0x0E, 0x11}, {0x0F, 0x00}, {0x10, 0x36}, {0x11, 0x80},
	{0x5F, 0x01}, {0x60, 0x0A}, {0x19, 0x20}, {0x48, 0x05}, {0x20, 0xB0},
	{0x21, 0x04}, {0x22, 0x65}, {0x23, 0x04}, {0x24, 0xC4}, {0x25, 0x40},
	{0x26, 0x43}, {0x27, 0xC9}, {0x28, 0x18}, {0x29, 0x01}, {0x2A, 0xC0},
	{0x2B, 0x21}, {0x2C, 0x04}, {0x2D, 0x01}, {0x2E, 0x15}, {0x2F, 0x44},
	{0x41, 0xCC}, {0x42, 0x03}, {0x39, 0x90}, {0x1D, 0xFF}, {0x1E, 0x1F},
	{0x6C, 0x90}, {0x30, 0x8C}, {0x31, 0x0C}, {0x32, 0xF0}, {0x33, 0x0C},
	{0x34, 0x1F}, {0x35, 0xE3}, {0x36, 0x0E}, {0x37, 0x34}, {0x38, 0x13},
	{0x3A, 0x08}, {0x3B, 0x30}, {0x3C, 0xC0}, {0x3D, 0x00}, {0x3E, 0x00},
	{0x3F, 0x00}, {0x40, 0x00}, {0x6F, 0x03}, {0x0D, 0x14}, {0x56, 0x32},
	{0x5A, 0x20}, {0x5B, 0xB3}, {0x5C, 0xF7}, {0x5D, 0xF0}, {0x62, 0x80},
	{0x63, 0x82}, {0x64, 0x00}, {0x67, 0x75}, {0x68, 0x04}, {0x6A, 0x4D},
	{0x8F, 0x18}, {0x91, 0x04}, {0x0C, 0x00}, {0x59, 0x97}, {0x4A, 0x05},
	{0x49, 0x11}, {0x50, 0x02}, {0x47, 0x22}, {0x7E, 0xCD}, {0x7F, 0x52},
	{0x7B, 0x57}, {0x7C, 0x28}, {0x80, 0x00}, {0x13, 0x81}, {0x12, 0x00},
	{0x93, 0x5C}, {0x45, 0x89},
};

static int jxf22_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_jxf22_fps_info[0].fps;
	info->modes.num = 1;
	...
	set_power_item(&up->items[i++], SNR_IO_POWER, PWR_2V8, 100);
...
static const struct fps_info *jxf22_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_jxf22_fps_info); i++)
		if (fps == g_jxf22_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_jxf22_fps_info))
		return NULL;

	return &g_jxf22_fps_info[i];
}

static int jxf22_get_init_info(uint32_t isp_id,
			       const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct jxf22_status *status;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	debug("jxf22 get fps %.1f init info\n", mode->fps);

	status = &g_status[isp_id];
	fps_info = jxf22_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0], g_jxf22_i2c_init_regs, 0);
	...
	info->interface.dvp.sample_rising = 0;
...
static int jxf22_start(uint32_t isp_id)
{
	struct jxf22_status *status;
...
static int jxf22_get_tuned_again(uint32_t isp_id,
...
static int jxf22_get_tuned_dgain(uint32_t isp_id,
...
static int jxf22_get_exposure_gain_info(uint32_t isp_id,
...
static int jxf22_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops jxf22_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "jxf22",
	.get_info = jxf22_get_info,
	.get_init_info = jxf22_get_init_info,
	.start = jxf22_start,
	.get_tuned_again = jxf22_get_tuned_again,
	.get_tuned_dgain = jxf22_get_tuned_dgain,
	.get_exposure_gain_info = jxf22_get_exposure_gain_info,
	.check = jxf22_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(jxf22_ops)
