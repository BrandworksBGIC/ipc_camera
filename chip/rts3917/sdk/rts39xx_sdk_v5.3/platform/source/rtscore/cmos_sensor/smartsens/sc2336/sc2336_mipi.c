 ...
 * Copyright (C) 2021 Bob Yin <bob_yin@realsil.com.cn>
 */
...
static struct sc2336_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc2336_fps_info[] = {
	{30, 2250, 81000000},
};

static struct rts_isp_i2c_reg g_sc2336_30fps_i2c_init_regs[] = {
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
...
static int sc2336_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_sc2336_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc2336_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_sc2336_fps_info); i++)
		if (fps == g_sc2336_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc2336_fps_info))
		return NULL;

	return &g_sc2336_fps_info[i];
}
			       ...
			       struct rts_isp_sensor_init_info *info)
{
	...
	debug("sc2336 get fps %.1f init info\n", mode->fps);
	...
	set_init_i2c_regs(info->sensor_regs[0],
		g_sc2336_30fps_i2c_init_regs, 0);
					...
					struct rts_isp_sync_regs *regs)
{
	...
	exp_reg_value =
		clip_d_word(exp_reg_value_float, 1, (1 * total_line - 4));
...
static int sc2336_get_mirror_flip(uint32_t isp_id,
...
static const struct rts_isp_sensor_ops sc2336_ops = {
	...
	.get_mirror_flip = sc2336_get_mirror_flip,
	.check = sc2336_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc2336_ops)
