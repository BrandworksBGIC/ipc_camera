...
struct jxf38p_status {
	float exp_step;
	float last_exposure;
	uint16_t min_vts;
};

static struct jxf38p_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_jxf38p_fps_info_asic[] = {
	{30, 2560, 86400000},
};

static struct rts_isp_i2c_reg g_jxf38p_i2c_init_regs_asic[] = {
	{0x12, 0x40},
	{0x48, 0x8A},
	{0x48, 0x0A},
	{0x0E, 0x11},
...
static int jxf38p_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_jxf38p_fps_info_asic[0].fps;
	info->modes.num = 1;
	...
	set_power_item(&up->items[i++], SNR_RST_GPIO, GPIO_LOW, 12000);
	set_power_item(&up->items[i++], SNR_RST_GPIO, GPIO_HIGH, 1000);
...
static const struct fps_info *jxf38p_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_jxf38p_fps_info_asic); i++)
		if (fps == g_jxf38p_fps_info_asic[i].fps)
			break;
	if (i == ARRAY_SIZE(g_jxf38p_fps_info_asic))
		return NULL;

	return &g_jxf38p_fps_info_asic[i];

}

static int jxf38p_get_init_info(uint32_t isp_id,
				const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct jxf38p_status *status;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	status = &g_status[isp_id];
	fps_info = jxf38p_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;

	set_init_i2c_regs(info->sensor_regs[0], g_jxf38p_i2c_init_regs_asic, 0);
...
static int jxf38p_start(uint32_t isp_id)
{
	struct jxf38p_status *status;
...
static int jxf38p_get_tuned_again(uint32_t isp_id,
...
static int jxf38p_get_tuned_dgain(uint32_t isp_id,
...
static int jxf38p_get_exposure_gain_info(uint32_t isp_id,
...
static int jxf38p_get_mirror_flip(uint32_t isp_id,
...
static int jxf38p_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops jxf38p_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "jxf38p",
	.get_info = jxf38p_get_info,
	.get_init_info = jxf38p_get_init_info,
	.start = jxf38p_start,
	.get_tuned_again = jxf38p_get_tuned_again,
	.get_tuned_dgain = jxf38p_get_tuned_dgain,
	.get_exposure_gain_info = jxf38p_get_exposure_gain_info,
	.get_mirror_flip = jxf38p_get_mirror_flip,
	.check = jxf38p_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(jxf38p_ops)
