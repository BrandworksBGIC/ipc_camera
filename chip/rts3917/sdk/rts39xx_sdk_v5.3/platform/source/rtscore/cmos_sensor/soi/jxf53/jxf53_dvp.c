...
static const struct fps_info g_jxf53_fps_info[] = {
	{30, 2560, 86400000},
};

static struct rts_isp_i2c_reg g_jxf53_30fps_i2c_init_regs[] = {
	{0x12, 0x40}, {0x48, 0x85}, {0x48, 0x05}, {0x0E, 0x1D},
	{0x0F, 0x04}, {0x10, 0x48}, {0x11, 0x80}, {0x46, 0x01},
	{0x47, 0x62}, {0x0D, 0xA2}, {0x57, 0x6A}, {0x58, 0x22},
	...
	{0x42, 0x33}, {0x47, 0x42}, {0x76, 0x60}, {0x77, 0x09},
	{0x80, 0x01}, {0xAF, 0x22}, {0xAB, 0x00}, {0x1D, 0xFF},
	{0x1E, 0x9F}, {0x6C, 0xC0}, {0x9E, 0xF8}, {0x31, 0x10},
	{0x32, 0x18}, {0x33, 0xE8}, {0x34, 0x5E}, {0x35, 0x5E},
	{0x3A, 0xAF}, {0x3B, 0x00}, {0x3C, 0xFF}, {0x3D, 0xFF},
	{0x3E, 0xFF}, {0x3F, 0xBB}, {0x40, 0xFF}, {0x56, 0x92},
	{0x59, 0xAF}, {0x5A, 0x47}, {0x61, 0x18}, {0x6F, 0x04},
	{0x85, 0x5F}, {0x8A, 0x44}, {0x91, 0x13}, {0x94, 0xA0},
	{0x9B, 0x83}, {0x9C, 0xE1}, {0xA4, 0x80}, {0xA6, 0x22},
	{0xA9, 0x1C}, {0x5B, 0xE7}, {0x5C, 0x28}, {0x5D, 0x67},
	{0x5E, 0x11}, {0x62, 0x21}, {0x63, 0x0F}, {0x64, 0xD0},
	{0x65, 0x02}, {0x67, 0x49}, {0x66, 0x00}, {0x68, 0x04},
	{0x69, 0x72}, {0x6A, 0x12}, {0x7A, 0x00}, {0x82, 0x20},
	{0x8D, 0x47}, {0x8F, 0x90}, {0x45, 0x01}, {0x97, 0x20},
	{0x13, 0x81}, {0x96, 0x84}, {0x4A, 0x01}, {0xB1, 0x00},
	{0xA1, 0x0F}, {0xBE, 0x00}, {0x7E, 0x48}, {0xB5, 0xC0},
	{0x50, 0x02}, {0x49, 0x10}, {0x7F, 0x57}, {0x90, 0x00},
	{0x7B, 0x4A}, {0x7C, 0x07}, {0x8C, 0xFF}, {0x8E, 0x00},
	{0x8B, 0x01}, {0x0C, 0x00}, {0xBC, 0x11}, {0x19, 0x20},
	{0x1B, 0x4F}, {0x12, 0x00}, {0x00, 0x10},
};
...
static int jxf53_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_jxf53_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *jxf53_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_jxf53_fps_info); i++)
		if (fps == g_jxf53_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_jxf53_fps_info))
		return NULL;

	return &g_jxf53_fps_info[i];
}
			       ...
			       struct rts_isp_sensor_init_info *info)
{
	...
	fps_info = jxf53_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0], g_jxf53_30fps_i2c_init_regs, 0);
