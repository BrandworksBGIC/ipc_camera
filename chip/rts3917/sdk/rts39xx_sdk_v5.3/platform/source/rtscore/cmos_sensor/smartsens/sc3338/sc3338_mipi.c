 ...
 * Copyright (C) 2021 Eric Yang <eric_yang@realsil.com.cn>
 */
...
struct sc3338_status {
	float exp_step;
	float last_exposure;
	uint16_t min_vts;
};

static struct sc3338_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc3338_fps_info[] = {
	{30, 2500, 102000000},
};

static struct rts_isp_i2c_reg g_sc3338_i2c_init_regs[] = {
...
//pclk = 2500 * 1360 * 30 = 102,000,000
	{0x0103, 0x01}, {0x36e9, 0x80}, {0x37f9, 0x80}, {0x301f, 0x0b},
	{0x30b8, 0x44}, {0x3200, 0x00}, {0x3201, 0x02}, {0x3202, 0x00},
	{0x3203, 0x00}, {0x3204, 0x09}, {0x3205, 0x0d}, {0x3206, 0x05},
	{0x3207, 0x1b}, {0x3208, 0x09}, {0x3209, 0x00}, {0x320a, 0x05},
	{0x320b, 0x10}, {0x320e, 0x05}, {0x320f, 0x50}, {0x3210, 0x00},
	{0x3211, 0x08}, {0x3212, 0x00}, {0x3213, 0x07}, {0x3253, 0x10},
	...
	{0x3631, 0x84}, {0x3633, 0x32}, {0x363b, 0x03}, {0x363c, 0x08},
	{0x3641, 0x38}, {0x3670, 0x4e}, {0x3674, 0xc0}, {0x3675, 0xc0},
	{0x3676, 0xc0}, {0x3677, 0x84}, {0x3678, 0x8a}, {0x3679, 0x8c},
	{0x367c, 0x48}, {0x367d, 0x49}, {0x367e, 0x4b}, {0x367f, 0x5f},
	{0x3690, 0x33}, {0x3691, 0x33}, {0x3692, 0x44}, {0x369c, 0x4b},
	{0x369d, 0x5f}, {0x36b0, 0x87}, {0x36b1, 0x90}, {0x36b2, 0xa1},
	{0x36b3, 0xd8}, {0x36b4, 0x49}, {0x36b5, 0x4b}, {0x36b6, 0x4f},
	{0x36ea, 0x11}, {0x36eb, 0x0d}, {0x36ec, 0x1c}, {0x36ed, 0x26},
	{0x370f, 0x01}, {0x3722, 0x09}, {0x3724, 0x41}, {0x3725, 0xc1},
	{0x3771, 0x09}, {0x3772, 0x09}, {0x3773, 0x05}, {0x377a, 0x48},
	{0x377b, 0x5f}, {0x37fa, 0x11}, {0x37fb, 0x33}, {0x37fc, 0x11},
	{0x37fd, 0x08}, {0x3904, 0x04}, {0x3905, 0x8c}, {0x391d, 0x04},
	{0x3921, 0x20}, {0x3926, 0x21}, {0x3933, 0x80}, {0x3934, 0x0a},
	{0x3935, 0x00}, {0x3936, 0x2a}, {0x3937, 0x6a}, {0x3938, 0x6a},
	{0x39dc, 0x02}, {0x3e01, 0x54}, {0x3e02, 0x80}, {0x3e09, 0x00},
	{0x4509, 0x20}, {0x5ae0, 0xfe}, {0x5ae1, 0x40}, {0x5ae2, 0x38},
	{0x5ae3, 0x30}, {0x5ae4, 0x28}, {0x5ae5, 0x38}, {0x5ae6, 0x30},
	{0x5ae7, 0x28}, {0x5ae8, 0x3f}, {0x5ae9, 0x34}, {0x5aea, 0x2c},
	{0x5aeb, 0x3f}, {0x5aec, 0x34}, {0x5aed, 0x2c}, {0x36e9, 0x54},
	{0x37f9, 0x47}, {0x0100, 0x01}, {0x440e, 0x02}, {0x30b8, 0x33},
	{0x3632, 0x64}, {0x3221, 0x66},
};

static int sc3338_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_sc3338_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc3338_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_sc3338_fps_info); i++)
		if (fps == g_sc3338_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc3338_fps_info))
		return NULL;

	return &g_sc3338_fps_info[i];
}

static int sc3338_get_init_info(uint32_t isp_id,
				const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct sc3338_status *status;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	debug("sc3338 get fps %.1f init info\n", mode->fps);

	status = &g_status[isp_id];
	fps_info = sc3338_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;
	...
	set_init_i2c_regs(info->sensor_regs[0], g_sc3338_i2c_init_regs, 0);
...
static int sc3338_start(uint32_t isp_id)
{
	struct sc3338_status *status;
...
static int sc3338_get_tuned_again(uint32_t isp_id,
...
static int sc3338_get_tuned_dgain(uint32_t isp_id,
...
static int sc3338_get_exposure_gain_info(uint32_t isp_id,
...
static int sc3338_get_mirror_flip(uint32_t isp_id,
...
static int sc3338_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops sc3338_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "sc3338",
	.get_info = sc3338_get_info,
	.get_init_info = sc3338_get_init_info,
	.start = sc3338_start,
	.get_tuned_again = sc3338_get_tuned_again,
	.get_tuned_dgain = sc3338_get_tuned_dgain,
	.get_exposure_gain_info = sc3338_get_exposure_gain_info,
	.get_mirror_flip = sc3338_get_mirror_flip,
	.check = sc3338_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc3338_ops)
