...
static struct sc3336_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_sc3336_fps_info[] = {
	{30, 2500, 102000000},
};

static struct rts_isp_i2c_reg g_sc3336_i2c_init_regs[] = {
...
static int sc3336_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_sc3336_fps_info[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *sc3336_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_sc3336_fps_info); i++)
		if (fps == g_sc3336_fps_info[i].fps)
			break;
	if (i == ARRAY_SIZE(g_sc3336_fps_info))
		return NULL;

	return &g_sc3336_fps_info[i];
}
			       ...
			       struct rts_isp_sensor_init_info *info)
{
	...
	debug("sc3336 get fps %.1f init info\n", mode->fps);
	...
	set_init_i2c_regs(info->sensor_regs[0], g_sc3336_i2c_init_regs, 0);
...
static int sc3336_get_mirror_flip(uint32_t isp_id,
...
static const struct rts_isp_sensor_ops sc3336_ops = {
	...
	.get_mirror_flip = sc3336_get_mirror_flip,
	.check = sc3336_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(sc3336_ops)
