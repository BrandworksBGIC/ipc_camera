...
static const struct fps_info g_gc2083_fps_info_dvp[] = {
	{30, 2200, 74250000},
};

static struct rts_isp_i2c_reg g_gc2083_i2c_init_regs_dvp[] = {
	/*system*/
	{0x03fe, 0xf0},
	{0x03fe, 0xf0},
	{0x03fe, 0xf0},
...
static int gc2083_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	...
	info->modes.mode[0].fps = g_gc2083_fps_info_dvp[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *gc2083_get_fps_info(uint16_t fps)
{
	...
	for (i = 0; i < ARRAY_SIZE(g_gc2083_fps_info_dvp); i++)
		if (fps == (g_gc2083_fps_info_dvp)[i].fps)
			break;
	if (i == ARRAY_SIZE((g_gc2083_fps_info_dvp)))
		return NULL;

	return &(g_gc2083_fps_info_dvp)[i];
				...
				struct rts_isp_sensor_init_info *info)
{
	...
	set_init_i2c_regs(info->sensor_regs[0], g_gc2083_i2c_init_regs_dvp, 0);
