...
struct jxf37p_status {
	float exp_step;
	float last_exposure;
	uint16_t min_vts;
};

static struct jxf37p_status g_status[SUPPORTED_ISP_NUM];

static const struct fps_info g_jxf37p_fps_info_asic[] = {
	{30, 2560, 86400000},
};

static struct rts_isp_i2c_reg g_jxf37p_i2c_init_regs_asic[] = {
	{0x12, 0x40},
	{0x48, 0x8A},
	{0x48, 0x0A},
	{0x0E, 0x19},
...
static int jxf37p_get_info(uint32_t isp_id, struct rts_isp_sensor_info *info)
{
	int i;
	...
	info->modes.mode[0].fps = g_jxf37p_fps_info_asic[0].fps;
	info->modes.num = 1;
...
static const struct fps_info *jxf37p_get_fps_info(uint16_t fps)
{
	int i;

	for (i = 0; i < ARRAY_SIZE(g_jxf37p_fps_info_asic); i++)
		if (fps == g_jxf37p_fps_info_asic[i].fps)
			break;
	if (i == ARRAY_SIZE(g_jxf37p_fps_info_asic))
		return NULL;

	return &g_jxf37p_fps_info_asic[i];

}

static int jxf37p_get_init_info(uint32_t isp_id,
				const struct rts_isp_sensor_mode *mode,
			       struct rts_isp_sensor_init_info *info)
{
	const struct fps_info *fps_info;
	struct jxf37p_status *status;

	if (isp_id >= SUPPORTED_ISP_NUM || !info)
		return -RTS_ISP_EINVAL;

	status = &g_status[isp_id];
	fps_info = jxf37p_get_fps_info(mode->fps);
	if (!fps_info)
		return -RTS_ISP_EINVAL;

	set_init_i2c_regs(info->sensor_regs[0], g_jxf37p_i2c_init_regs_asic, 0);
...
static int jxf37p_start(uint32_t isp_id)
{
	struct jxf37p_status *status;
...
static int jxf37p_get_tuned_again(uint32_t isp_id,
...
static int jxf37p_get_tuned_dgain(uint32_t isp_id,
...
static int jxf37p_get_exposure_gain_info(uint32_t isp_id,
...
static int jxf37p_get_mirror_flip(uint32_t isp_id,
...
static int jxf37p_check(uint32_t isp_id)
{
	int ret;
	int id;
...
static const struct rts_isp_sensor_ops jxf37p_ops = {
	.api_version = SENSOR_API_VERSION,
	.name = "jxf37p",
	.get_info = jxf37p_get_info,
	.get_init_info = jxf37p_get_init_info,
	.start = jxf37p_start,
	.get_tuned_again = jxf37p_get_tuned_again,
	.get_tuned_dgain = jxf37p_get_tuned_dgain,
	.get_exposure_gain_info = jxf37p_get_exposure_gain_info,
	.get_mirror_flip = jxf37p_get_mirror_flip,
	.check = jxf37p_check,
};

RTS_ISP_DEFINE_SENSOR_PLUGIN(jxf37p_ops)
