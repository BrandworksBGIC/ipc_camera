 ...
 *
 * rtscamutils/rts_test_crypt.c
 *
 * Copyright (C) 2016      joe xiao<joe_xiao@realsil.com.cn>
 */
...
static uint8_t key[CRYPT_KEY_MAX] = {
...
static uint8_t iv[CRYPT_IV_MAX] = {
			...
			enum RTS_CIPHER_PADDING_MODE padding_mode)
{
	...
	if (ret) {
		printf("rts cipher set keysize fail, ret = %d\n", ret);
		goto err;
	}
	printf("rts cipher set keysize ok\n");
	...
	if (!direction) {
		...
		ret = rts_cipher_encrypt(handle,
				(uint8_t *)input_text, inlen,
				(uint8_t *)output_text, (uint32_t *)&outlen);
		if (ret) {
