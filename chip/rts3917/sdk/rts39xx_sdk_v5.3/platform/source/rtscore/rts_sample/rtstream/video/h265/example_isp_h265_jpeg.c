...
void print_help_info(void)
{
	...
	fprintf(stdout, "\texample for h265 and jpeg\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_isp_h265_jpeg [option]...");
	fprintf(stdout, "\n");
	fprintf(stdout, "-h, --help\thelp\n");
	fprintf(stdout, "-n, --num\tframe number\n");
	fprintf(stdout, "-s, --save\tsave frame at <dir>\n");
	fprintf(stdout, "example:\n");
	fprintf(stdout, "\texample_isp_h265_jpeg -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}
...
static int __init_sys_vmem(void)
{
	...
	/* h26x */
	cfg.stream[0].module[1].type = RTS_AV_ID_H265;
...
int test_stream(void)
{
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/out.h265",
					save_dir);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file out.h265 fail\n");
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	}
