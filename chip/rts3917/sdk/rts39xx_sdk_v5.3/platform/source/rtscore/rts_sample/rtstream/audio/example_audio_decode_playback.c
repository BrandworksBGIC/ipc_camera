/*
 *  Copyright (C) 2019 Realtek Semiconductor Corp.
 *  All Rights Reserved
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation.
 */

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <unistd.h>
#include <errno.h>
#include <string.h>
#include <signal.h>
#include <rtscamkit.h>
#include <rts_queue.h>
#include <rtsavapi.h>
#include <rtsaudio.h>
#include <getopt.h>

/* code flow
                           +------------+    +------------+    +------------+
                           |            |    |            |    |            |
test.ulaw (g711 header)===>| decode_ch  |===>| resample_ch|===>| playback_ch|===>speaker
                           |            |    |(PCM option)|    |   (PCM)    |
                           +------------+    +------------+    +------------+
*/

struct option longopts[] = {
	{"help", no_argument, NULL, 'h'},
	{0, 0, 0, 0}
};

static int g_exit;
static void Termination(int sign)
{
	g_exit = 1;
}

void print_help_info(void)
{
	fprintf(stdout, "DESCRIPTION:\n");
	fprintf(stdout, "\tdecode ulaw audio file\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_audio_decode <in-file>\n");
	fprintf(stdout, "EXAMPLE:\n");
	fprintf(stdout, "\texample_audio_decode test.ulaw\n");
}

void recycle_buffer(void *master, struct rts_av_buffer *buffer)
{
	if (g_exit)
		return;

	buffer->bytesused = 0;
	*((int *)master) = 0;
}

int test_stream(char **argv)
{
	int ret;
	int finish = 0;
	int decode = -1;
	int resample = -1;
	int playback = -1;
	struct rts_av_profile profile;
	uint32_t chunk_bytes = 512;
	FILE *fin = NULL;
	struct rts_av_buffer *inbuf = NULL;
	int buf_state = 0;
	struct rts_audio_attr attr;
	int len;

	fin = fopen(argv[1], "rb");
	if (!fin) {
		RTS_ERR("fail to open %s: %s\n",
			argv[1], strerror(errno));
		ret = RTS_RETURN(RTS_E_OPEN_FAIL);
		goto exit;
	}
	fseek(fin, 0x3a, SEEK_SET);

	inbuf = rts_av_new_buffer(chunk_bytes);
	if (!inbuf) {
		RTS_ERR("fail to alloc buffer\n");
		ret = RTS_RETURN(RTS_E_NO_MEMORY);
		goto exit;
	}

	ret = rts_av_set_buffer_callback(inbuf, &buf_state, recycle_buffer);
	if (ret) {
		RTS_ERR("fail to set buffer callback\n");
		goto exit;
	}
	buf_state = 0;

	decode = rts_av_create_audio_decode_chn();
	if (decode < 0) {
		RTS_ERR("fail to create decode chn, ret = %d\n", decode);
		ret = RTS_RETURN(RTS_E_OPEN_FAIL);
		goto exit;
	}

	profile.fmt = RTS_A_FMT_ULAW;
	profile.audio.samplerate = 8000;
	profile.audio.bitfmt = 16;
	profile.audio.channels = 1;
	ret = rts_av_set_profile(decode, &profile);
	if (ret) {
		RTS_ERR("set decode fail, ret = %d\n", ret);
		goto exit;
	}

	memset(&attr, 0, sizeof(attr));
	snprintf(attr.dev_node, sizeof(attr.dev_node), "hw:0,1");
	attr.channels = 2;
	attr.rate = 16000;
	attr.format = 16;
	resample = rts_av_create_audio_resample_chn(attr.rate,
					attr.format, attr.channels);
	if (resample < 0) {
		RTS_ERR("fail to create resample chn, ret = %d\n", resample);
		ret = RTS_RETURN(RTS_E_OPEN_FAIL);
		goto exit;
	}

	playback = rts_av_create_audio_playback_chn(&attr);
	if (playback < 0) {
		RTS_ERR("fail to create playback chn, ret = %d\n", playback);
		ret = RTS_RETURN(RTS_E_OPEN_FAIL);
		goto exit;
	}

	ret = rts_av_bind(decode, resample);
	if (ret) {
		RTS_ERR("fail to bind decode and resample, ret = %d\n", ret);
		goto exit;
	}

	ret = rts_av_bind(resample, playback);
	if (ret) {
		RTS_ERR("fail to bind resample and playback, ret = %d\n", ret);
		goto exit;
	}

	rts_av_enable_chn(decode);
	rts_av_enable_chn(resample);
	rts_av_enable_chn(playback);
	rts_av_start_send(decode);

	while (!g_exit) {
		if (!buf_state && !feof(fin)) {
			len = fread(inbuf->vm_addr, 1,
					chunk_bytes, fin);
			if (len > 0) {
				inbuf->bytesused = len;
				buf_state = 1;
				rts_av_send(decode, inbuf);
			}
		}

		if (feof(fin) && rts_av_is_idle(playback))
			finish++;
		else
			finish = 0;

		if (finish >= 30) {
			usleep(1000 * 1000 * 1);
			RTS_INFO("finish\n");
			break;
		}

		usleep(1000);
	}

	rts_av_stop_send(decode);
	rts_av_disable_chn(decode);
	rts_av_disable_chn(resample);
	rts_av_disable_chn(playback);
	rts_av_unbind(decode, resample);
	rts_av_unbind(resample, playback);
exit:
	if (decode >= 0) {
		rts_av_destroy_chn(decode);
		decode = -1;
	}
	if (resample >= 0) {
		rts_av_destroy_chn(resample);
		resample = -1;
	}
	if (playback >= 0) {
		rts_av_destroy_chn(playback);
		playback = -1;
	}
	if (inbuf) {
		rts_av_set_buffer_callback(inbuf, NULL, NULL);
		rts_av_delete_buffer(inbuf);
	}
	RTS_SAFE_RELEASE(fin, fclose);

	return ret;
}

int main(int argc, char **argv)
{
	int ret;
	int c;

	while ((c = getopt_long(argc, argv,
				":h", longopts, NULL)) != -1) {
		switch (c) {
		case 'h':
			print_help_info();
			return 0;
		case '?':
			printf("invalid param: -%c\n", optopt);
			return -1;
		default:
			break;
		}
	}

	if (argc != 2) {
		printf("use -h to get help info\n");
		return -1;
	}

	rts_set_log_mask(RTS_LOG_MASK_CONS);

	signal(SIGINT, Termination);
	signal(SIGTERM, Termination);

	ret = rts_av_init();
	if (ret) {
		RTS_ERR("rts_av_init fail\n");
		return ret;
	}

	ret = test_stream(argv);

	rts_av_release();

	if (ret)
		printf("Fail\n");
	else
		printf("Success\n");

	return ret;
}
