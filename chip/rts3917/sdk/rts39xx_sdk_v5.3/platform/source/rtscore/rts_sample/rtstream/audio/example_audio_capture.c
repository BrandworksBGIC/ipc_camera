...
int test_stream(void)
{
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/audio_out.pcm",
					save_dir);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open audio file audio_out.pcm fail\n");
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	}
	...
	while (!g_exit) {
		...
		if (rts_av_recv_block(capture, &buffer, 100))
			continue;
