...
+---------+      +---------+
|         |      |         |
|  vin_ch | ===> | h265_ch | ===> rts_av_query_h265_crop ===> rts_av_set_h265_crop ===> crop_out.h265
|         |      |         |
+---------+      +---------+
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\texample for h265 crop\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_h265_crop [option]...");
	fprintf(stdout, "\n");
	...
	fprintf(stdout, "\texample_h265_crop -x 0 -y 0 -X 256 -Y 128 -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}

int test_stream(struct rts_h265_crop *crop)
{
	...
	struct rts_h265_crop *pcrop = NULL;
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/crop_out.h265",
					save_dir);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file crop_out.h265 fail\n");
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	}

	ret = rts_av_query_h265_crop(h265, &pcrop);
	if (ret) {
	...
	ret = rts_av_set_h265_crop(pcrop);
	if (ret) {
		RTS_ERR("set h265 crop fail, ret = %d\n", ret);
		goto exit;
	}
...
exit:
	RTS_SAFE_RELEASE(pcrop, rts_av_release_h265_crop);
	if (vin >= 0) {
