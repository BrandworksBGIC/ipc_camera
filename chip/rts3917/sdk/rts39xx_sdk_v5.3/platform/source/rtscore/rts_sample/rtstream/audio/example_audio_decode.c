...
/* code flow
                           +------------+
                           |            |
test.ulaw (g711 header)===>| decode_ch  |===>target.wav (PCM no_header)
                           |            |
                           +------------+
*/
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\texample_audio_decode <in-file> <out-file>\n");
	fprintf(stdout, "EXAMPLE:\n");
	fprintf(stdout, "\texample_audio_decode source.ulaw target.wav\n");
}
...
int test_stream(char **argv)
{
	...
	while (!g_exit) {
		...
		if (feof(fin) && rts_av_is_idle(decode))
			finish++;
		else
			finish = 0;
		...
		if (rts_av_recv_block(decode, &buffer, 100))
			continue;
