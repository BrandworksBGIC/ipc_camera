...
+---------+         +---------+
|         |         |         |
|  vin_ch |========>|encode_ch|========>receive_block=====>fov_out.h265
|         |         |         |
+---------+         +---------+
...
void print_help_info(void)
{
	...
	fprintf(stdout,
		"\texample to enable FOV mode\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_fov_mode [option]...\n");
	fprintf(stdout, "\n");
	fprintf(stdout, "-h, --help\thelp\n");
	fprintf(stdout, "-v, --val\t0-disable fov mode, 1-enable fov mode\n");
	fprintf(stdout, "-e, --enc\tencode type (h264 h265)\n");
	fprintf(stdout, "-s, --save\tsave frame at <dir>\n");
	fprintf(stdout, "-n, --num\tframe number\n");
	fprintf(stdout, "example:\n");
	fprintf(stdout, "\texample_fov_mode -e h265/h264 -v 1 -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}

int test_fov_mode(int vin, int value)
{
	int ret = 0;
	int enable;

	ret = rts_av_get_isp_fov_mode(vin, &enable);
	if (ret) {
		RTS_ERR("get fov mode of channel %d fail, ret = %d\n",
			vin, ret);
		goto exit;
	}
	RTS_INFO("fov mode of channel %d is %d\n", vin, enable);

	ret = rts_av_set_isp_fov_mode(vin, value);
	if (ret) {
		RTS_ERR("set fov mode of channel %d fail, ret = %d\n",
			vin, ret);
		goto exit;
	}
...
int test_stream(int value)
{
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/fov_out.%s",
					save_dir, rts_enc_name);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file fov_out.%s fail\n",
				rts_enc_name);
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
