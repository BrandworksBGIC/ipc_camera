...
+---------+      +---------+
|         |      |         |
|  vin_ch | ===> | h264_ch | ===> rts_av_query_h264_crop ===> rts_av_set_h264_crop ===> crop_out.h264
|         |      |         |
+---------+      +---------+
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\texample for h264 crop\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_h264_crop [option]...");
	fprintf(stdout, "\n");
	...
	fprintf(stdout, "\texample_h264_crop -x 0 -y 0 -X 256 -Y 128 -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}

int test_stream(struct rts_h264_crop *crop)
{
	...
	struct rts_h264_crop *pcrop = NULL;
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/crop_out.h264",
					save_dir);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file crop_out.h264 fail\n");
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	}

	ret = rts_av_query_h264_crop(h264, &pcrop);
	if (ret) {
		RTS_ERR("Failed to query crop\n");
		goto exit;
	}

	pcrop->start_x = crop->start_x;
	pcrop->start_y = crop->start_y;
	pcrop->end_x = crop->end_x;
	pcrop->end_y = crop->end_y;
	ret = rts_av_set_h264_crop(pcrop);
	if (ret) {
		RTS_ERR("set h264 crop fail, ret = %d\n", ret);
		goto exit;
	}

	RTS_INFO("crop:start_x = %d,start_y = %d,end_x = %d,end_y = %d\n",
		pcrop->start_x, pcrop->start_y, pcrop->end_x, pcrop->end_y);
...
exit:
	RTS_SAFE_RELEASE(pcrop, rts_av_release_h264_crop);
	if (vin >= 0) {
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv,
				":hx:y:X:Y:n:s:", longopts, NULL)) != -1) {
		switch (c) {
		case 'h':
			print_help_info();
			return 0;
		case 'x':
			crop.start_x = (uint32_t)strtol(optarg, NULL, 0);
			break;
		case 'y':
			crop.start_y = (uint32_t)strtol(optarg, NULL, 0);
			break;
		case 'X':
			crop.end_x = (uint32_t)strtol(optarg, NULL, 0);
			break;
		case 'Y':
			crop.end_y = (uint32_t)strtol(optarg, NULL, 0);
			break;
		case 's':
			save_dir = optarg;
			break;
