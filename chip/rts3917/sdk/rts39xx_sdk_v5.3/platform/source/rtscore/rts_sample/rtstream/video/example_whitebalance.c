...
+---------+         +---------+
|         |         |         |
|  vin_ch |========>|encode_ch|========>receive_block=====>whitebalance_out.h265
|         |         |         |
+---------+         +---------+
...
|                   |          |                    |
| thread-           |          |                    |
| test_whitebalance |=========>|rts_av_set_isp_ctrl |
|                   |          |                    |
+-------------------+          +--------------------+
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\ta example to change whitebalance\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_whitebalance [OPTION]...\n");
	fprintf(stdout, "\n");
	fprintf(stdout, "-h, --help\thelp\n");
	fprintf(stdout, "-v, --val\tvalue\n");
	fprintf(stdout, "-e, --enc\tencode type (h264 h265)\n");
	fprintf(stdout, "-s, --save\tsave frame at <dir>\n");
	fprintf(stdout, "-n, --num\tframe number\n");
	fprintf(stdout, "example:\n");
	fprintf(stdout, "\texample_whitebalance -e h265/h264 -v 4500 -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}

static int test_get_whitebalance(void)
{
	struct rts_isp_control ctrl;
	int ret;

	ret = rts_av_get_isp_ctrl(RTS_ISP_CTRL_ID_AWB_CTRL, &ctrl);
	if (ret) {
		RTS_ERR("get auto white balance fail, ret = %d\n", ret);
		return ret;
	}
	RTS_INFO("%s min = %d, max = %d, step = %d, default = %d, cur = %d\n",
		 ctrl.name, ctrl.minimum, ctrl.maximum,
		 ctrl.step, ctrl.default_value, ctrl.current_value);

	/* white balance is not in auto mode */
	if (!ctrl.current_value) {
		ret = rts_av_get_isp_ctrl(
				      RTS_ISP_CTRL_ID_WB_TEMPERATURE, &ctrl);
		if (ret) {
			RTS_ERR("get auto white balance fail, ret = %d\n", ret);
			return ret;
		}
...
static int test_set_whitebalance(int value)
{
	struct rts_isp_control ctrl;
	int ret;

	ret = rts_av_get_isp_ctrl(
			      RTS_ISP_CTRL_ID_AWB_CTRL, &ctrl);
	if (ret) {
		RTS_ERR("get auto white balance fail, ret = %d\n", ret);
		return ret;
	}
	...
	if (value == 0) {
		/* want to set it to auto, but now is manual */
		if (ctrl.current_value == 0) {
			ctrl.current_value = 1;
			ret = rts_av_set_isp_ctrl(RTS_ISP_CTRL_ID_AWB_CTRL,
						  &ctrl);
			if (ret) {
				RTS_ERR("set whitebalance fail, ret = %d\n",
					ret);
				return ret;
			}
		}
	} else {
		/* want to set it to manual, but now is auto */
		if (ctrl.current_value) {
		...
		ret = rts_av_get_isp_ctrl(
				      RTS_ISP_CTRL_ID_WB_TEMPERATURE, &ctrl);
		if (ret) {
			RTS_ERR("get whitebalance fail, ret = %d\n", ret);
			return ret;
		}
		RTS_INFO("%s min = %d, max = %d, step = %d, default = %d, cur = %d\n",
			 ctrl.name, ctrl.minimum, ctrl.maximum,
		...
		if (value <= ctrl.maximum && value >= ctrl.minimum)
			...
			ctrl.current_value = ctrl.default_value;
		ret = rts_av_set_isp_ctrl(
				      RTS_ISP_CTRL_ID_WB_TEMPERATURE, &ctrl);
		if (ret) {
			RTS_ERR("set whitebalance fail, ret = %d\n", ret);
			return ret;
...
static void *test_whitebalance(void *arg)
{
	int value;
	int ret;
	...
	ret = test_get_whitebalance();
	if (ret)
		goto exit;

	ret = test_set_whitebalance(value);
	if (ret)
		goto exit;
	...
	while (!g_exit)
		usleep(1000);
exit:
	RTS_INFO("quit test whitebalance thread\n");
	rts_ret = ret;
...
int test_stream(int value)
{
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/whitebalance_out.%s",
					save_dir, rts_enc_name);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");

		if (!pfile) {
			RTS_ERR("open encode file whitebalance_out.%s fail\n",
				rts_enc_name);
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	...
	pthread_create(&tid, NULL, test_whitebalance, &value);
