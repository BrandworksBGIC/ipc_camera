...
+---------+         +---------+
|         |         |         |========>receive_block=====>fps_out.h265
|  vin_ch |========>|encode_ch|
|  (YUV)  |         |         |========>change_fps (when recv number=50)
+---------+         +---------+
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\tprovide a example to change fps\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_change_fps [OPTION]...\n");
	fprintf(stdout, "\n");
	...
	fprintf(stdout, "\texample_change_fps -e h265/h264 -f 25 -s /mnt\n");
}

int change_fps(int chn, uint8_t fps)
{
	...
	tmp = profile.video.denominator / profile.video.numerator;
	...
	RTS_INFO("[change fps]%d->%d\n", tmp,
			profile.video.denominator / profile.video.numerator);
...
int test_stream(uint8_t fps)
{
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/fps_out.%s",
				save_dir, rts_enc_name);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file fps_out.%s fail\n",
				rts_enc_name);
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
