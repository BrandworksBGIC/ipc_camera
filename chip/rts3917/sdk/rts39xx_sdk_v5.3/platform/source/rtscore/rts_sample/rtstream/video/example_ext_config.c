...
+---------+         +---------+
|         |         |         |
|  vin_ch |========>|encode_ch|========>receive_block=====>out.h265
|         |         |         |
+---------+         +---------+
...
void print_help_info(void)
{
	...
	fprintf(stdout, "\texample to alloc and config isp buffer externally\n");
	fprintf(stdout, "USAGE:\n");
	fprintf(stdout, "\texample_ext_config [option]...\n");
	fprintf(stdout, "\n");
	...
	fprintf(stdout, "\texample_ext_config -e h265/h264 -s /mnt -n 10\n");
	fprintf(stdout, "\n");
}
...
int test_stream(void)
{
	...
	buffer = rts_av_new_buffer(0);
	if (!buffer) {
		RTS_ERR("new buffer fail\n");
		goto exit;
	}

	buffer->length = WIDTH * HEIGHT *  3 / 2;
	ret = rts_av_sys_vrm_alloc(buffer);
	if (ret) {
		RTS_ERR("sys vrm alloc failed, ret = %d\n", ret);
		goto exit;
	}

	buffer->addr_offset[0] = 0;
	buffer->addr_offset[1] = WIDTH * HEIGHT;
	ret = rts_av_config_outbuf_info(vin, &buffer, 1);
	if (ret) {
		RTS_ERR("config vin output buffer failed, ret = %d\n", ret);
		goto exit;
	}
	...
	if (save_dir) {
		snprintf(outfile, sizeof(outfile), "%s/out.%s",
					save_dir, rts_enc_name);
		RTS_INFO("save to %s\n", outfile);
		pfile = fopen(outfile, "wb");
		if (!pfile) {
			RTS_ERR("open encode file out.%s fail\n",
				rts_enc_name);
			ret = RTS_RETURN(RTS_E_OPEN_FAIL);
			goto exit;
		}
	...
	while (!g_exit) {
		struct rts_av_buffer *buffer1 = NULL;

		if (rts_av_recv_block(enc, &buffer1, 100))
			continue;

		if (buffer1) {
			if (pfile)
				fwrite(buffer1->vm_addr, 1,
					buffer1->bytesused, pfile);
			number++;
			rts_av_put_buffer(buffer1);
		}
