...
{
    ERROR("%s: usage: %s <-a | -n mnt_point | -u> <fstab_file>\n", me, me);
    exit(1);
}
...
int main(int argc, char *argv[])
{
	...
	parse_options(argc, argv, &a_flag, &u_flag, &n_flag, &n_name);
	...
	if (a_flag) {
		...
		ret = fs_mgr_do_mount(fstab, n_name, 0);
	} else if (u_flag) {
		ret = fs_mgr_unmount_all(fstab);
	} else {
