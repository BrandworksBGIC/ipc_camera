...
static struct flag_list fs_mgr_flags[] = {
    ...
    { "ubifs_auth",  MF_UBIFS_AUTH },
    { "fscrypt",     MF_FSCRYPT },
    { "dm-integrity", MF_INTEGRITY },
    { "defaults",    0 },
    { 0,             0 },
};
...
static int parse_device(char *in, char **mtd_num, char **dev)
{
	char *p, *t;
	char *savep;
...
#define FS_OPTIONS_LEN 1024
	...
	if (!strcmp(fstab->recs[cnt].type, "nand")) {
		ret = parse_device(p, &fstab->recs[cnt].mtd_num,
					&fstab->recs[cnt].blk_device);
		if (ret)
			goto err;
	} else {
...
void fs_mgr_free_fstab(struct fstab *fstab)
{
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        free(fstab->recs[i].type);
        free(fstab->recs[i].mtd_num);
 ...
 */
static struct fstab_rec *fs_mgr_get_entry_for_mount_point_after(struct fstab_rec *start_rec, struct fstab *fstab, const char *path)
{
    int i;
    if (!fstab) {
        return NULL;
