...
#define E2FSCK_BIN      "/usr/sbin/e2fsck"
#define F2FS_FSCK_BIN  "/usr/sbin/fsck.f2fs"
...
static int ubi_attach(const char *dev, char *mtd_num)
{
	int ret = 0;
	char *cmd[] = {
		UBIATTACH_BIN,
	...
	DEBUG("cmd: %s - %s\n", *cmd, mtd_num);
	ret = execvp_cmd(ARRAY_SIZE(cmd), cmd);
	if (ret != 0)
		ERROR("Failed attach ubi to mtd %s, ret=%d\n",
					mtd_num, ret);
...
static void check_fs(char *blk_device, char *fs_type, char *target)
{
    ...
    if (!strcmp(fs_type, "ext2") || !strcmp(fs_type, "ext3") || !strcmp(fs_type, "ext4")) {
	    ...
	    ret = execvp_cmd(ARRAY_SIZE(e2fsck_argv), e2fsck_argv);
            if (ret < 0) {
	    ...
	    ret = execvp_cmd(ARRAY_SIZE(f2fs_fsck_argv), f2fs_fsck_argv);
        if (ret < 0) {
            /* No need to check for error in fork, we can't really handle it now */
            ERROR("Failed trying to run %s\n", F2FS_FSCK_BIN);
        }
...
/*
 * Get the given block device size in bytes, using the BLKGETSIZE64 ioctl.
 * Return 0 on success, and -1 on error.
 */
int fs_mgr_get_blk_size(const char *blockdev, uint64_t *size)
{
	int fd;
	int rc = -1;
...
static int get_resize_flag(struct fstab *fstab, int index)
{
	if (fstab->recs[index].flags & MS_RDONLY)
		return 0;
	if (strcmp(fstab->recs[index].type, "emmc"))
		return 0;
	if (strcmp(fstab->recs[index].fs_type, "ext2") &&
	    strcmp(fstab->recs[index].fs_type, "ext3") &&
	    strcmp(fstab->recs[index].fs_type, "ext4"))
		return 0;
	return 1;
}

static void __do_resize_ext4_partition(struct fstab *fstab, int index)
{
	char cmd[64];
	int ret;

	sprintf(cmd, "resize2fs %s", fstab->recs[index].blk_device);
	INFO("%s\n", cmd);
	ret = execle_cmd(cmd);
	if (ret != 0)
...
static void resize_ext4_partition(struct fstab *fstab, int index)
{
	uint64_t partition_size, fs_size = 0;

	/* get ext4 fs size */
	get_fs_size(fstab->recs[index].fs_type,
		    fstab->recs[index].blk_device, &fs_size);

	/* get emmc partition real size */
	fs_mgr_get_blk_size(fstab->recs[index].blk_device, &partition_size);

	/* resize only once */
	if (partition_size > fs_size) {
		INFO("resize partition %s\n", fstab->recs[index].blk_device);
		__do_resize_ext4_partition(fstab, index);
	}
}
...
int fs_mgr_mount_all(struct fstab *fstab)
{
	...
	/* type */
        if (!strcmp(fstab->recs[i].type, "nand")) {
                if (access(fstab->recs[i].blk_device, F_OK)) {
			mret = ubi_attach(fstab->recs[i].blk_device,
					fstab->recs[i].mtd_num);
			if (mret)
				continue;
		}
	}
	...
	if (fstab->recs[i].fs_mgr_flags & MF_FSCRYPT) {
		int rc = fs_mgr_setup_fscrypt(&fstab->recs[i]);
		if (rc != FS_MGR_SETUP_FSCRYPT_SUCCESS) {
			ERROR("Could not set up fscrypted partition, skipping!\n");
			continue;
		}
	} else if (fstab->recs[i].fs_mgr_flags & MF_CRYPT) {
		int rc = fs_mgr_setup_crypt(&fstab->recs[i]);
		if (rc != FS_MGR_SETUP_CRYPT_SUCCESS) {
			ERROR("Could not set up crypted partition, skipping!\n");
			continue;
		}
	}

	if ((fstab->recs[i].fs_mgr_flags & MF_INTEGRITY) && device_is_secure()) {
		int rc = fs_mgr_setup_integrity(&fstab->recs[i]);

		if (rc != FS_MGR_SETUP_INTEGRITY_SUCCESS) {
			ERROR("Could not set up integrity partition, skip!\n");
			continue;
		}
	...
	if (get_resize_flag(fstab, i))
		resize_ext4_partition(fstab, i);

        mret = mount_with_alternatives(fstab, i, &last_idx_inspected, &attempted_idx);
	if (mret < 0)
		ERROR("mount_with_alternatives, mret=%d, i=%d, errno=%d\n",
					mret, i, errno);
    }
	return mret;
}
 ...
 */
int fs_mgr_do_mount(struct fstab *fstab, char *n_name, char *tmp_mount_point)
{
    int i = 0;
    ...
    for (i = 0; i < fstab->num_entries; i++) {
        ...
        if (!strcmp(fstab->recs[i].fs_type, "swap") ||
            ...
            ERROR("Cannot mount filesystem of type %s on %s\n",
                  fstab->recs[i].fs_type, fstab->recs[i].blk_device);
            goto out;
        }
	...
	if ((fstab->recs[i].fs_mgr_flags & MF_INTEGRITY) && device_is_secure()) {
        ...
        if (__mount(fstab->recs[i].blk_device, m, &fstab->recs[i])) {
    ...
    if (mount_errors) {
        ERROR("Cannot mount filesystem at %s. error: %s\n",
            m, strerror(first_mount_errno));
...
int fs_mgr_swapon_all(struct fstab *fstab)
{
    ...
    for (i = 0; i < fstab->num_entries; i++) {
	    ...
	    err = execvp_cmd(ARRAY_SIZE(mkswap_argv), mkswap_argv);
        if (err) {
