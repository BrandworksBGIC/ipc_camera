...
static CRYPTO_ONCE engine_rts = CRYPTO_ONCE_STATIC_INIT;
DEFINE_RUN_ONCE_STATIC(ossl_init_engine_rts)
{
#  ifdef OPENSSL_INIT_DEBUG
    fprintf(stderr, "OPENSSL_INIT: ossl_init_engine_rts: "
                    "ENGINE_load_rts()\n");
#  endif
    ENGINE_load_rts();
    return 1;
...
# ifndef OPENSSL_NO_HW_RTS
    if ((opts & OPENSSL_INIT_ENGINE_RTS)
            && !RUN_ONCE(&engine_rts, ossl_init_engine_rts))
        return 0;
# endif
