...
static void print_weight_node(const void *ptr, VISIT order, int level)
{
    ...
    if (order == postorder || order == leaf)  {
        printf("weight: (%ld)  ", p->num_weights);
...
#define OUT_D16C(X,N) fprintf(fp,"\t%10d, /* %8lu %s */\n", X, collate_pos++, N);
#define OUT_U16C(X,N) fprintf(fp,"\t%10ld, /* %8lu %s */\n", X, collate_pos++, N);
...
static void dump_collate(FILE *fp)
{
	...
	OUT_D16C(MAX_COLLATION_WEIGHTS, "max collation weights");
