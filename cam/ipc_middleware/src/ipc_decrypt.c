...
static ipc_decrypt_f _get_decrypt_func(pv8 so_file)
{
    v8 sign_file[256];
...
static ipc_decrypt_tag_info_p _g_decrypt_tag_info;
...
static s32 _decrypt(void)
{
    ...
    f_decrypt               = _get_decrypt_func(DECRYPT_LIB_PATH);
    if (f_decrypt == NULL) {
    ...
    if (msg->version == 2) {
        if (cloud_platform_info_len + 1 + 1 > sizeof(buff) - sizeof(struct id_msg)) {
            ipcfatal("Tag info out of bounds!");
            ipc_free(_g_decrypt_info);
            return IPC_VERIFY_FAILED;
        }
        s32 tag_len        = msg->cloud_platform_info[cloud_platform_info_len];
        s32 tag_buffer_len = sizeof(*_g_decrypt_tag_info) + tag_len + 1;

        _g_decrypt_tag_info = ipc_malloc(tag_buffer_len, tag_buffer_len);
        if (_g_decrypt_tag_info == NULL) {
            ipcfatal("Malloc failed!");
            ipc_free(_g_decrypt_info);
            _g_decrypt_info = NULL;
            return IPC_NOMEM;
        }

        _g_decrypt_tag_info->len = tag_len;

        memcpy(_g_decrypt_tag_info->info, msg->cloud_platform_info + cloud_platform_info_len + 1, tag_len);
    }
...
ipc_decrypt_tag_info_p ipc_decrypt_tag_info(void)
{
    pthread_mutex_lock(&mutex);
    s32 ret = _decrypt();
    pthread_mutex_unlock(&mutex);
